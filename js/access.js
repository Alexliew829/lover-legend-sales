...
function unlockAccessLock(options = {}) {
  ...
  // V12.0: keep a replayable unlock flag. On desktop Refresh, access.js may
  ...
  button.addEventListener("click", async () => {
    ...
    try {
      ...
      status.textContent = /Unknown action:\s*saveAccessSettings/i.test(message)
        ? "密码同步失败：Google Apps Script 仍是旧部署，请重新部署 V12.0 Code.gs"
        : "密码同步失败：" + message;
    } finally {
