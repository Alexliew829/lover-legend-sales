...
function showPage(name,el){
  ...
  // V30.0: page "sales" uses the sales-card type "daily".
  ...
  // V30.0: every time Live is opened, start from today's date.
  ...
  // V30.0: page switching never waits for or triggers cloud sync.
...
// V30.0: Top 5 business performance. Uses rows already loaded in memory only;
...
function readHistoricalHighsCache(){
  ...
  // V30.0: a changed/deleted turnover invalidates the old historical-high cache immediately.
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V30.0: historical record is lazy. Top 5 opens instantly from local rows;
...
function renderTodayCompanyStatus(){
  ...
  // V30.0: status colour represents actual business amount, not merely
...
function applyCloudCommissionSettings(settings){
  ...
  // V30.0: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V30.0: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V30.0: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V30.0: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V30.0: local Fair values are direct replacements, never additions. The server
...
// V30.0: paint Home immediately, restore local cache, then perform only a
...
// V30.0 cache-first startup: show local data immediately and perform only the
// lightweight revision/current-month check. Historical year data is NOT warmed
// automatically; it is fetched only when the user explicitly opens Monthly Summary.
...
function collectLiveHosts(){
  ...
  // V30.0: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V30.0: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V30.0 Import Cost System product search ================= */
...
function unorderedImportProductMatchV214(sourceValue,queryValue){
  ...
  if(isExactProductCodeQueryV214(query)){
    // V30.0: code search supports case-insensitive prefix/partial entry.
...
function setupImportProductSearchV214(item,nameInput,resultsBox,closeButton){
  // V30.0: iPhone/iOS safe search state.
  ...
  const render=async()=>{
    ...
    try{
      ...
      matches.forEach(record=>{
        ...
        // V30.0: distinguish an intentional tap from list scrolling on iPhone/iOS.
  ...
  if(closeButton){
    ...
    closeButton.addEventListener("click",e=>{
      ...
      // V30.0: X means CLEAR CURRENT PRODUCT SELECTION.
...
/* ================= V30.0 Sales Card unsaved-change protection ================= */
...
function hasUnsavedSalesCardChangesV238(type){
  ...
  // V30.0: use the same transaction-level dirty signal that Live already maintains.
...
/* ================= V30.0 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V30.0: Sales Cards and Daily Profit are mutually exclusive.
    ...
    // V30.0: saved cards first. Do not flash a fake 0.00 card while cloud data loads.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V30.0 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V30.0 button bindings reliable.
...
/* ================= V30.0 multi-product Sales Card ================= */
...
function buildProductSubItemV239(type,card,data={},order=1){
  ...
  [c,p].forEach(el=>el.addEventListener("blur",()=>{
    ...
    // V30.0: blur fires before the Save button click. Re-mark the card dirty here so
...
function bindSingleTapDeleteProductV254(button,handler){
  ...
  // V30.0: for touch/pen, pointerup is the one and only real activation.
...
系统会保留 saleId 的删除记录，让 Import Cost System 知道这张销售已经取消。`))return;
  try{
    ...
    // V30.0: refresh profit view from active backend links after card deletion.
    try{
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  // V30.0: Sales only reminds. Inventory confirmation is handled in Import Cost System.
  ...
  setTimeout(()=>{
    // V30.0: initialization may finish after a very fast first edit.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V30.0 sales card container missing",type);return false}
...
/* ================= V30.0 Sales Card unit-price + stable optimistic UI ================= */
...
async function saveProductLinksV206(type){
  ...
  // V30.0 optimistic local cache: keep the current UI stable; do not redraw from an older async response.
...
/* ================= V30.0 same-day linked bonsai profit summary ================= */
...
async function toggleProductProfitSummaryV216(type,button){
  ...
  try{
    // V30.0: opening Daily Profit closes Sales Cards first, then paints local cache immediately.
    const pre=productLinkPreV208(type),box=document.getElementById(pre+"ProductLinkBox"),body=document.getElementById(pre+"ProductLinkBody");
...
/* ================= V30.0 expandable yearly monthly breakdown + profit ================= */
...
function renderAll(){
  // V30.0: one complete render path. This replaces the older partial duplicate
...
// V30.0: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V30.0: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V30.0: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
/* ================= V30.0 pre-change sales-card context guard ================= */
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V30.0: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V30.0: deleting the last special commission rule leaves candidate and
...
/* ================= V30.0 Sales Card transaction integrity / instant cache ================= */
...
/* ================= V30.0 Live optimistic revision guard ================= */
...
async function refreshOpenSalesChangeLogAfterSaveV243(type,date){
  ...
  }catch(e){console.warn("V30.0 修改/销售记录后台刷新失败",e)}
}
...
saveLiveSales=async function(){
  ...
  // V30.0 preflight: do not even mutate local turnover when saved cards exceed the new amount.
  try{
  ...
  // Critical V30.0 guard: any cloud request started before this local mutation
...
/* ================= V30.0 Profit list copy + compact totals ================= */
...
/* ================= V30.0 profit layout + weighted average margin ================= */
...
/* ================= V30.0 cross-date inventory reminders =================
...
const INVENTORY_PENDING_LOCAL_KEY_V300="lover_inventory_pending_cache_v300";
try{
  const saved=JSON.parse(localStorage.getItem(INVENTORY_PENDING_LOCAL_KEY_V300)||"null");
  if(saved&&Array.isArray(saved.items)){inventoryPendingCacheV250=saved.items;inventoryPendingLastAtV250=Number(saved.at||0)}
}catch(_){}
...
function openImportCostSystemV266(target={}){
  // V30.0: Sales must stay open; Import always opens in a separate tab/window.
...
async function refreshInventoryPendingV250(force=false){
  ...
  inventoryPendingLoadingV250=(async()=>{
    try{
      ...
      try{localStorage.setItem(INVENTORY_PENDING_LOCAL_KEY_V300,JSON.stringify({at:inventoryPendingLastAtV250,items:inventoryPendingCacheV250}))}catch(_){}
...
// V30.0: Sales-side inventory confirmation removed. Import is the only inventory authority.
...
showPage=function(name,el){
  ...
  if(name==="sales"||name==="live"||name==="fair"){renderInventoryPendingGlobalV250();setTimeout(()=>refreshInventoryPendingV250(false),80)}
  return result;
};
window.showPage=showPage;

// Cross-day reminder also initializes shortly after startup.
setTimeout(()=>{renderInventoryPendingGlobalV250();refreshInventoryPendingV250(false)},1800);

// V30.0: after the user processes inventory in the separate Import tab/app,
...
function scheduleInventoryPendingResumeRefreshV265(){
  ...
  inventoryPendingResumeTimerV265=setTimeout(()=>{
    ...
    refreshInventoryPendingV250(false);
  },500);
}
...
renderFairMonthlyList=function(){
  ...
  container.innerHTML=locations.map(group=>{const sales=group.rows.reduce((s,r)=>s+Number(r.amount||0),0),commission=sales*fairRate;return `<div class="month-record-group month-profit-group-v294" data-profit-group="${escapeChangeLogHtmlV200(group.name)}"><div class="month-record-group-title fair-location-title-v298"><span>${escapeChangeLogHtmlV200(group.name)}</span><b>佣金 ${money(commission)}</b></div>${profitHeadV294()}${group.rows.map(r=>fairProfitRowV294(r,0)).join('')}<div class="fair-location-commission-v298"><span>这场 Fair 佣金 ${fairRatePct}%</span><b>${money(commission)}</b></div><div class="month-profit-row-v294 month-profit-total-v294"><span>这场 Fair 总数</span><strong>${money(sales)}</strong><strong class="profit-value-v294">0.00</strong><strong class="profit-rate-v294">0.00%</strong></div></div>`}).join('')+`<div id="fairProfitGrandV294" class="profit-grand-v294"><strong>Fair 全部地点总计</strong><div><span>营业额</span><b>${money(total)}</b></div><div><span>总佣金 ${fairRatePct}%</span><b>${money(fairCommissionTotal)}</b></div><div><span>利润</span><b>0.00</b></div><div><span>整体利润率</span><b>0.00%</b></div></div>`;
  renderFairPageTop3();
  Promise.resolve(loadAllSalesProductLinksV203({force:false,maxAgeMs:120000})).then(links=>{
    ...
    container.innerHTML=locations.map(group=>{
      ...
      return `<div class="month-record-group month-profit-group-v294"><div class="month-record-group-title fair-location-title-v298"><span>${escapeChangeLogHtmlV200(group.name)}</span><b>佣金 ${money(groupCommission)}</b></div>${profitHeadV294()}${rowsHtml}<div class="fair-location-commission-v298"><span>这场 Fair 佣金 ${fairRatePct}%</span><b>${money(groupCommission)}</b></div><div class="month-profit-row-v294 month-profit-total-v294"><span>这场 Fair 总数</span><strong>${money(groupSales)}</strong><strong class="profit-value-v294">${money(groupProfit)}</strong><strong class="profit-rate-v294">${marginTextV294(groupProfit,groupSales)}</strong></div></div>`;
    }).join('');
    const totalProfit=locations.reduce((sum,g)=>sum+g.rows.reduce((s,r)=>s+profitByDayV294(links,'fair',g.name,r.date),0),0);
    container.insertAdjacentHTML('beforeend',`<div id="fairProfitGrandV294" class="profit-grand-v294"><strong>Fair 全部地点总计</strong><div><span>营业额</span><b>${money(total)}</b></div><div><span>总佣金 ${fairRatePct}%</span><b>${money(fairCommissionTotal)}</b></div><div><span>利润</span><b>${money(totalProfit)}</b></div><div><span>整体利润率</span><b>${marginTextV294(totalProfit,total)}</b></div></div>`);
  }).catch(e=>console.warn('V30.0 Fair 利润读取失败',e));
};
...
renderLiveMonthlyList=function(){
  ...
  }).catch(e=>console.warn('V30.0 Live 利润读取失败',e));
};
