...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4070",restoreGeneration:0};
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  // V40.7: confirmation belongs to the sales-card transaction, not only to the
  ...
  // V40.7: this is the active Sales / Fair / Live card builder. Keep each
...
function markDraftSavedLocallyV314(type,ctx,dirty,items,dirtyIds){
  ...
  const local=items.map(x=>{
    ...
    let status='DRAFT';
    if(wasConfirmed){
      // V40.7: preserve each already-confirmed row, but a NEW product added to an
...
async function validateSalesInventoryAvailabilityV325(type,items,saveMode='draft'){
  ...
  // V40.7: keep the same authoritative Import oversell guard, but avoid the old
...
saveProductLinksV206=async function(type,saveMode='confirm',button=null){
  ...
  // V40.7: a draft is local/cloud sales-card data only. Import availability is
  ...
  SALES_CONFIRMATION_IN_FLIGHT_V407=true;
  try{
...
// V40.7: the pending endpoint is the smallest authoritative ACK check. When a
...
// V40.7: do not wait for the full Sales cloud-resume cycle. Import ACK is a
