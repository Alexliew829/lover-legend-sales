...
function companyDailyRowsV186(company){
 const month=selectedMonth(), map={};
 rows.filter(r=>r.type==="daily"&&r.company===company)
 .filter(r=>String(displayToISO(r.date)).slice(0,7)===month)
 .forEach(r=>{map[r.date]=(map[r.date]||0)+Number(r.amount||0);});
 return Object.keys(map).sort((a,b)=>displayToISO(b).localeCompare(displayToISO(a))).map(d=>({date:d,amount:map[d]}));
}
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"1807"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=Array.isArray(state.closedMonths)?state.closedMonths:[];systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"1807"}updateReadOnlyMode()}
