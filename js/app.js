...
function showPage(name,el){
  ...
  // V17.3: every time Live is opened, start from today's date.
  ...
  // V17.3: page switching never waits for or triggers cloud sync.
...
// V17.3: Top 5 business performance. Uses rows already loaded in memory only;
...
const HISTORY_HIGH_CACHE_KEY="lover_sales_history_high_v173";
...
function historyRecordHtml(record,kind,state="ready"){
  if(state==="loading"){
    return '<div class="history-record history-loading"><span>🏆 历史最高纪录</span><small>正在读取…</small></div>';
  }
  if(!record||!Number(record.amount||0)){
    return '<div class="history-record"><span>🏆 历史最高纪录</span><small>暂无历史记录</small></div>';
  }
  ...
  return `<div class="history-record">
    <div class="history-record-left"><span class="history-record-label">🏆 历史最高纪录</span><span class="history-record-detail">${detail}</span></div>
...
function renderHistoricalRecordForPanel(elId,state="ready"){
  ...
  host.innerHTML=historyRecordHtml(data&&data.highs?data.highs[kind]:null,kind);
}
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V17.3: historical record is lazy. Top 5 opens instantly from local rows;
...
function applyCloudCommissionSettings(settings){
  ...
  // V17.3: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V17.3: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V17.3: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V17.3 mobile-safe save: return control immediately after the local durable
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V17.3: local Fair values are direct replacements, never additions. The server
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"17.3"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=Array.isArray(state.closedMonths)?state.closedMonths:[];systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"17.3"}updateReadOnlyMode()}
...
// V17.3: paint Home immediately, restore local cache, then perform only a
...
// V17.3: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V17.3: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V17.3: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
function renderAll(){
  // V17.3: one complete render path. This replaces the older partial duplicate
...
// V17.3: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V17.3: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V17.3: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V17.3: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V17.3: deleting the last special commission rule leaves candidate and
