...
function showPage(name,el){
  ...
  // V25.6: every time Live is opened, start from today's date.
  ...
  // V25.6: page switching never waits for or triggers cloud sync.
...
// V25.6: Top 5 business performance. Uses rows already loaded in memory only;
...
function renderTodayCompanyStatus(){
  ...
  // V25.6: status colour represents actual business amount, not merely
...
function applyCloudCommissionSettings(settings){
  ...
  // V25.6: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V25.6: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V25.6: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V25.6: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V25.6: local Fair values are direct replacements, never additions. The server
...
// V25.6: paint Home immediately, restore local cache, then perform only a
...
// V25.6: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V25.6: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V25.6: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V25.6 Import Cost System product search ================= */
...
function unorderedImportProductMatchV214(sourceValue,queryValue){
  ...
  if(isExactProductCodeQueryV214(query)){
    // V25.6: code search supports case-insensitive prefix/partial entry.
...
function setupImportProductSearchV214(item,nameInput,resultsBox,closeButton){
  // V25.6: iPhone/iOS safe search state.
  ...
  const render=async()=>{
    ...
    try{
      ...
      matches.forEach(record=>{
        ...
        // V25.6: distinguish an intentional tap from list scrolling on iPhone/iOS.
  ...
  if(closeButton){
    ...
    closeButton.addEventListener("click",e=>{
      ...
      // V25.6: X means CLEAR CURRENT PRODUCT SELECTION.
...
/* ================= V25.6 Sales Card unsaved-change protection ================= */
...
/* ================= V25.6 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V25.6: Sales Cards and Daily Profit are mutually exclusive.
    ...
    // V25.6: saved cards first. Do not flash a fake 0.00 card while cloud data loads.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V25.6 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V25.6 button bindings reliable.
...
/* ================= V25.6 multi-product Sales Card ================= */
...
function bindSingleTapDeleteProductV254(button,handler){
  ...
  // V25.6: for touch/pen, pointerup is the one and only real activation.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V25.6 sales card container missing",type);return false}
...
/* ================= V25.6 Sales Card unit-price + stable optimistic UI ================= */
...
async function saveProductLinksV206(type){
  ...
  // V25.6 optimistic local cache: keep the current UI stable; do not redraw from an older async response.
...
/* ================= V25.6 same-day linked bonsai profit summary ================= */
...
async function toggleProductProfitSummaryV216(type,button){
  ...
  try{
    // V25.6: opening Daily Profit closes Sales Cards first, then paints local cache immediately.
    const pre=productLinkPreV208(type),box=document.getElementById(pre+"ProductLinkBox"),body=document.getElementById(pre+"ProductLinkBody");
...
/* ================= V25.6 expandable yearly monthly breakdown ================= */
...
function yearBreakdownRowsV224(kind){
  // V25.6: Balakong / Belimbing / Fair / Live show months inside the selected
...
function renderAll(){
  // V25.6: one complete render path. This replaces the older partial duplicate
...
// V25.6: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V25.6: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V25.6: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V25.6: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V25.6: deleting the last special commission rule leaves candidate and
...
/* ================= V25.6 Sales Card transaction integrity / instant cache ================= */
...
/* ================= V25.6 Live optimistic revision guard ================= */
...
async function refreshOpenSalesChangeLogAfterSaveV243(type,date){
  ...
  }catch(e){console.warn("V25.6 修改/销售记录后台刷新失败",e)}
}
...
saveLiveSales=async function(){
  ...
  // Critical V25.6 guard: any cloud request started before this local mutation
...
/* ================= V25.6 Profit list copy + compact totals ================= */
...
/* ================= V25.6 profit layout + weighted average margin ================= */
...
/* ================= V25.6 cross-date inventory reminders ================= */
