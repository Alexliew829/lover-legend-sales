...
function showPage(name,el){
  ...
  // V28.6: page "sales" uses the sales-card type "daily".
  ...
  // V28.6: every time Live is opened, start from today's date.
  ...
  // V28.6: page switching never waits for or triggers cloud sync.
...
// V28.6: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V28.6: historical record is lazy. Top 5 opens instantly from local rows;
...
function renderTodayCompanyStatus(){
  ...
  // V28.6: status colour represents actual business amount, not merely
...
function applyCloudCommissionSettings(settings){
  ...
  // V28.6: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V28.6: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V28.6: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V28.6: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V28.6: local Fair values are direct replacements, never additions. The server
...
// V28.6: paint Home immediately, restore local cache, then perform only a
...
// V28.6: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V28.6: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V28.6: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V28.6 Import Cost System product search ================= */
...
function unorderedImportProductMatchV214(sourceValue,queryValue){
  ...
  if(isExactProductCodeQueryV214(query)){
    // V28.6: code search supports case-insensitive prefix/partial entry.
...
function setupImportProductSearchV214(item,nameInput,resultsBox,closeButton){
  // V28.6: iPhone/iOS safe search state.
  ...
  const render=async()=>{
    ...
    try{
      ...
      matches.forEach(record=>{
        ...
        // V28.6: distinguish an intentional tap from list scrolling on iPhone/iOS.
  ...
  if(closeButton){
    ...
    closeButton.addEventListener("click",e=>{
      ...
      // V28.6: X means CLEAR CURRENT PRODUCT SELECTION.
...
/* ================= V28.6 Sales Card unsaved-change protection ================= */
...
function hasUnsavedSalesCardChangesV238(type){
  ...
  // V28.6: use the same transaction-level dirty signal that Live already maintains.
...
/* ================= V28.6 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V28.6: Sales Cards and Daily Profit are mutually exclusive.
    ...
    // V28.6: saved cards first. Do not flash a fake 0.00 card while cloud data loads.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V28.6 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V28.6 button bindings reliable.
...
async function removeProductLinkItemV206(type,id){
  const currentCard=document.querySelector(`[data-product-link-item="${id}"]`)?.closest('.sales-card-wrapper,.product-link-card,[data-transaction-id]');
  if(currentCard && typeof salesCardStatusV285==='function' && ['confirmed','modified'].includes(salesCardStatusV285(currentCard))){
    alert('已确认销售卡不能删除产品。\n\n如顾客换产品、数量或价格，请直接修改产品资料后点击「保存修改」。系统只会提醒手动处理库存差异。');
    return;
  }
...
/* ================= V28.6 multi-product Sales Card ================= */
...
function buildProductSubItemV239(type,card,data={},order=1){
  ...
  [c,p].forEach(el=>el.addEventListener("blur",()=>{
    ...
    // V28.6: blur fires before the Save button click. Re-mark the card dirty here so
...
function bindSingleTapDeleteProductV254(button,handler){
  ...
  // V28.6: for touch/pen, pointerup is the one and only real activation.
...
系统会保留 saleId 的删除记录，让 Import Cost System 知道这张销售已经取消。`))return;
  try{
    ...
    // V28.6: refresh profit view from active backend links after card deletion.
    try{
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  // V28.6: Sales only reminds. Inventory confirmation is handled in Import Cost System.
  ...
  setTimeout(()=>{
    // V28.6: initialization may finish after a very fast first edit.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V28.6 sales card container missing",type);return false}
...
/* ================= V28.6 Sales Card unit-price + stable optimistic UI ================= */
...
async function saveProductLinksV206(type){
  ...
  // V28.6 optimistic local cache: keep the current UI stable; do not redraw from an older async response.
...
/* ================= V28.6 same-day linked bonsai profit summary ================= */
...
async function toggleProductProfitSummaryV216(type,button){
  ...
  try{
    // V28.6: opening Daily Profit closes Sales Cards first, then paints local cache immediately.
    const pre=productLinkPreV208(type),box=document.getElementById(pre+"ProductLinkBox"),body=document.getElementById(pre+"ProductLinkBody");
...
function renderAll(){
  // V28.6: one complete render path. This replaces the older partial duplicate
...
// V28.6: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V28.6: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V28.6: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
/* ================= V28.6 pre-change sales-card context guard ================= */
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V28.6: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V28.6: deleting the last special commission rule leaves candidate and
...
/* ================= V28.6 Sales Card transaction integrity / instant cache ================= */
...
/* ================= V28.6 Live optimistic revision guard ================= */
...
async function refreshOpenSalesChangeLogAfterSaveV243(type,date){
  ...
  }catch(e){console.warn("V28.6 修改/销售记录后台刷新失败",e)}
}
...
saveLiveSales=async function(){
  ...
  // V28.6 preflight: do not even mutate local turnover when saved cards exceed the new amount.
  try{
  ...
  // Critical V28.6 guard: any cloud request started before this local mutation
...
/* ================= V28.6 Profit list copy + compact totals ================= */
...
/* ================= V28.6 profit layout + weighted average margin ================= */
...
/* ================= V28.6 cross-date inventory reminders =================
...
function openImportCostSystemV266(target={}){
  // V28.6: Sales must stay open; Import always opens in a separate tab/window.
...
// V28.6: Sales-side inventory confirmation removed. Import is the only inventory authority.
...
// V28.6: after the user processes inventory in the separate Import tab/app,
...
/* ================= V28.6 Draft -> Confirmed sales-card workflow ================= */
function salesCardStatusV285(card){
  const raw=String(card?.dataset?.cardStatus||"").toLowerCase();
  if(raw)return raw;
  return salesCardIsSavedV241(card)?"confirmed":"draft"; // legacy V28.4 saved cards are confirmed
}
function applySalesCardStateV285(card, dataList=[]){
  if(!card)return card;
  const list=Array.isArray(dataList)?dataList:[];
  const explicit=String(list.find(x=>x.cardStatus)?.cardStatus||"").toLowerCase();
  const status=explicit||(list.some(x=>x.linkId)?"confirmed":"draft");
  card.dataset.cardStatus=status;
  const title=card.querySelector('.sales-card-header-v239 b:first-child');
  if(title)title.textContent=status==="confirmed"?"已确认销售卡":status==="modified"?"已确认销售卡（已修改）":"草稿销售卡";
  const del=card.querySelector('.product-link-remove-bottom');
  if(del){del.hidden=status==="confirmed"||status==="modified";del.disabled=del.hidden;}
  let actions=card.querySelector('.sales-card-actions-v285');
  if(!actions){
    actions=document.createElement('div');actions.className='sales-card-actions-v285';
    const confirmBtn=document.createElement('button');confirmBtn.type='button';confirmBtn.className='secondary-btn sales-card-confirm-v285';confirmBtn.textContent='✅ 确认销售';
    confirmBtn.addEventListener('click',()=>confirmSalesCardV285(card.dataset.type,card.dataset.transactionId));
    actions.appendChild(confirmBtn);card.appendChild(actions);
  }
  const cb=actions.querySelector('.sales-card-confirm-v285');if(cb)cb.hidden=status==="confirmed"||status==="modified";
  return card;
}
const _buildSalesCardTransactionV285=buildSalesCardTransactionV239;
buildSalesCardTransactionV239=function(type,dataList=[]){const card=_buildSalesCardTransactionV285(type,dataList);return applySalesCardStateV285(card,dataList)};
const _renderProductLinksEditorV285=renderProductLinksEditorV206;
renderProductLinksEditorV206=function(type,links){_renderProductLinksEditorV285(type,links);salesCardWrappersV239(type).forEach(card=>{const txn=String(card.dataset.transactionId||'');applySalesCardStateV285(card,(links||[]).filter(x=>String(x.transactionId||'')===txn))})};

const _deleteSalesCardTransactionV285=deleteSalesCardTransactionV239;
deleteSalesCardTransactionV239=async function(type,txnId){
  const card=salesCardWrappersV239(type).find(c=>String(c.dataset.transactionId||'')===String(txnId||''));
  if(card&&["confirmed","modified"].includes(salesCardStatusV285(card))){alert('这张销售卡已经确认，不能删除。\n\n如顾客换产品、数量或价格，请直接修改销售卡后点击「保存修改」。系统只会提醒手动处理库存差异。');return;}
  return _deleteSalesCardTransactionV285(type,txnId);
};window.deleteSalesCardTransactionV239=deleteSalesCardTransactionV239;

async function saveSalesCardsByModeV285(type,mode,targetTxnId=''){
  const cards=salesCardWrappersV239(type);
  const target=targetTxnId?cards.filter(c=>String(c.dataset.transactionId||'')===String(targetTxnId)):cards.filter(c=>c.dataset.dirty==='1'||c.dataset.productRemovedV259==='1'||!salesCardIsSavedV241(c));
  if(!target.length){alert(mode==='confirm'?'找不到要确认的销售卡。':'销售卡没有需要保存的修改。');return null;}
  const all=collectProductLinksV206(type),ids=new Set(target.map(c=>String(c.dataset.transactionId||''))),items=all.filter(x=>ids.has(String(x.transactionId||'')));
  if(!items.length){alert('请填写销售卡产品资料。');return null;}
  const official=salesCardsOfficialAmountV241(type),allTotal=all.reduce((s,x)=>s+Number(x.actualPrice||0),0);
  if(official>0&&allTotal>official+0.005){alert(`所有销售卡售价总数 RM${formatAmount(allTotal)} 已超过当天营业额 RM${formatAmount(official)}。`);return null;}
  for(const x of items){if(!x.productName){alert('每一个产品都需要填写产品名称。');return null}if(Number(x.quantity)<=0){alert('数量必须大于 0。');return null}if(Number(x.unitPrice)<=0){alert('售价必须大于 RM0.00。');return null}}
  if(mode==='confirm'&&!confirm('确认这张销售卡正式成交？\n\n确认后：\n• 会通知 Import Cost System 处理库存\n• 销售卡不能删除\n• 以后仍可修改，但只会提醒手动调整库存差异'))return null;
  const hasConfirmed=target.some(c=>["confirmed","modified"].includes(salesCardStatusV285(c)));
  const cardMode=mode==='confirm'?'confirm':(hasConfirmed?'modify':'draft');
  try{
    setSync(cardMode==='draft'?'草稿同步中...':cardMode==='confirm'?'确认销售中...':'销售卡修改同步中...');
    const result=await window.saveSalesProductLinksApiV241(items,{cardMode});
    const saved=Array.isArray(result?.links)?result.links:items;
    target.forEach(card=>{clearSalesCardTransactionDirtyV239(card);delete card.dataset.productRemovedV259;card.querySelectorAll('.product-link-item').forEach(i=>{i.dataset.saved='1';i.dataset.dirty='0'});card.dataset.cardStatus=cardMode==='draft'?'draft':cardMode==='confirm'?'confirmed':'modified';applySalesCardStateV285(card,saved.filter(x=>String(x.transactionId||'')===String(card.dataset.transactionId||'')))});
    salesProductLinksCacheV216.clear();allSalesProductLinksCacheV216={links:null,at:0};clearDailyProfitCacheV237(type,productLinkContextV206(type).date);
    setSync(cardMode==='draft'?'草稿已保存':cardMode==='confirm'?'销售已确认':'修改已保存，请手动处理库存差异',true);
    alert(cardMode==='draft'?'草稿已保存，不会通知 Import System。':cardMode==='confirm'?'销售已确认，Import System 将收到库存待处理提醒。':'修改已保存。此销售已经确认过，不会再次自动扣库存；请按提示到 Import System 手动处理库存差异。');
    return result;
  }catch(e){setSync('销售卡同步失败',false,true);alert('销售卡保存失败：'+(e.message||e));return null}
}
saveProductLinksV206=function(type){return saveSalesCardsByModeV285(type,'draft')};
async function confirmSalesCardV285(type,txnId){return saveSalesCardsByModeV285(type,'confirm',txnId)}
window.confirmSalesCardV285=confirmSalesCardV285;

// V28.6 transport accepts explicit cardMode without changing old callers.
window.saveSalesProductLinksApiV241=async function(items,options={}){
  const json=await jsonp({action:'saveSalesProductLinks',itemsJson:JSON.stringify(items||[]),cardMode:String(options.cardMode||'draft')},{timeoutMs:30000});
  if(!json.ok)throw new Error(json.message||json.error||'盆栽资料保存失败');return json;
};
