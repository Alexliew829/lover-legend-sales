...
function showPage(name,el){
  ...
  // V18.6: every time Live is opened, start from today's date.
  ...
  // V18.6: page switching never waits for or triggers cloud sync.
...
// V18.6: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V18.6: historical record is lazy. Top 5 opens instantly from local rows;
...
function applyCloudCommissionSettings(settings){
  ...
  // V18.6: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V18.6: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V18.6: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V18.6 mobile-safe save: return control immediately after the local durable
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V18.6: local Fair values are direct replacements, never additions. The server
...
// V18.6: paint Home immediately, restore local cache, then perform only a
...
// V18.6: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V18.6: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V18.6: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
function renderAll(){
  // V18.6: one complete render path. This replaces the older partial duplicate
...
// V18.6: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V18.6: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V18.6: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V18.6: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V18.6: deleting the last special commission rule leaves candidate and
