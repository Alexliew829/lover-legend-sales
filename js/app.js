...
function showPage(name,el){
  ...
  // V30.4: page "sales" uses the sales-card type "daily".
  ...
  // V30.4: every time Live is opened, start from today's date.
  ...
  // V30.4: page switching never waits for or triggers cloud sync.
...
// V30.4: Top 5 business performance. Uses rows already loaded in memory only;
...
function readHistoricalHighsCache(){
  ...
  // V30.4: a changed/deleted turnover invalidates the old historical-high cache immediately.
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V30.4: historical record is lazy. Top 5 opens instantly from local rows;
...
function renderTodayCompanyStatus(){
  ...
  // V30.4: status colour represents actual business amount, not merely
...
function applyCloudCommissionSettings(settings){
  ...
  // V30.4: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V30.4: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V30.4: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V30.4: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V30.4: local Fair values are direct replacements, never additions. The server
...
// V30.4: paint Home immediately, restore local cache, then perform only a
...
// V30.4: keep the proven V29.9 sync behavior: start cached Home immediately,
// then warm the current year's historical months in the background so other
// devices converge on cloud-confirmed data without waiting for a manual open.
...
function collectLiveHosts(){
  ...
  // V30.4: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V30.4: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V30.4 Import Cost System product search ================= */
...
function unorderedImportProductMatchV214(sourceValue,queryValue){
  ...
  if(isExactProductCodeQueryV214(query)){
    // V30.4: code search supports case-insensitive prefix/partial entry.
...
function setupImportProductSearchV214(item,nameInput,resultsBox,closeButton){
  // V30.4: iPhone/iOS safe search state.
  ...
  const render=async()=>{
    ...
    try{
      ...
      matches.forEach(record=>{
        ...
        // V30.4: distinguish an intentional tap from list scrolling on iPhone/iOS.
  ...
  if(closeButton){
    ...
    closeButton.addEventListener("click",e=>{
      ...
      // V30.4: X means CLEAR CURRENT PRODUCT SELECTION.
...
/* ================= V30.4 Sales Card unsaved-change protection ================= */
...
function hasUnsavedSalesCardChangesV238(type){
  ...
  // V30.4: use the same transaction-level dirty signal that Live already maintains.
...
/* ================= V30.4 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V30.4: Sales Cards and Daily Profit are mutually exclusive.
    ...
    // V30.4: saved cards first. Do not flash a fake 0.00 card while cloud data loads.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V30.4 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V30.4 button bindings reliable.
...
/* ================= V30.4 multi-product Sales Card ================= */
...
function buildProductSubItemV239(type,card,data={},order=1){
  ...
  [c,p].forEach(el=>el.addEventListener("blur",()=>{
    ...
    // V30.4: blur fires before the Save button click. Re-mark the card dirty here so
...
function bindSingleTapDeleteProductV254(button,handler){
  ...
  // V30.4: for touch/pen, pointerup is the one and only real activation.
...
系统会保留 saleId 的删除记录，让 Import Cost System 知道这张销售已经取消。`))return;
  try{
    ...
    // V30.4: refresh profit view from active backend links after card deletion.
    try{
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  // V30.4: Sales only reminds. Inventory confirmation is handled in Import Cost System.
  ...
  setTimeout(()=>{
    // V30.4: initialization may finish after a very fast first edit.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V30.4 sales card container missing",type);return false}
...
/* ================= V30.4 Sales Card unit-price + stable optimistic UI ================= */
...
async function saveProductLinksV206(type){
  ...
  // V30.4 optimistic local cache: keep the current UI stable; do not redraw from an older async response.
...
/* ================= V30.4 same-day linked bonsai profit summary ================= */
...
async function toggleProductProfitSummaryV216(type,button){
  ...
  try{
    // V30.4: opening Daily Profit closes Sales Cards first, then paints local cache immediately.
    const pre=productLinkPreV208(type),box=document.getElementById(pre+"ProductLinkBox"),body=document.getElementById(pre+"ProductLinkBody");
...
/* ================= V30.4 expandable yearly monthly breakdown + profit ================= */
...
function renderAll(){
  // V30.4: one complete render path. This replaces the older partial duplicate
...
// V30.4: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V30.4: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V30.4: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
/* ================= V30.4 pre-change sales-card context guard ================= */
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V30.4: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V30.4: deleting the last special commission rule leaves candidate and
...
/* ================= V30.4 Sales Card transaction integrity / instant cache ================= */
...
/* ================= V30.4 Live optimistic revision guard ================= */
...
async function refreshOpenSalesChangeLogAfterSaveV243(type,date){
  ...
  }catch(e){console.warn("V30.4 修改/销售记录后台刷新失败",e)}
}
...
saveLiveSales=async function(){
  ...
  // V30.4 preflight: do not even mutate local turnover when saved cards exceed the new amount.
  try{
  ...
  // Critical V30.4 guard: any cloud request started before this local mutation
...
/* ================= V30.4 Profit list copy + compact totals ================= */
...
/* ================= V30.4 profit layout + weighted average margin ================= */
...
/* ================= V30.4 cross-date inventory reminders =================
...
function openImportCostSystemV266(target={}){
  // V30.4: Sales must stay open; Import always opens in a separate tab/window.
...
// V30.4: Sales-side inventory confirmation removed. Import is the only inventory authority.
...
// V30.4: after the user processes inventory in the separate Import tab/app,
...
renderFairMonthlyList=function(){
  ...
  }).catch(e=>console.warn('V30.4 Fair 利润读取失败',e));
};
...
renderLiveMonthlyList=function(){
  ...
  }).catch(e=>console.warn('V30.4 Live 利润读取失败',e));
};
