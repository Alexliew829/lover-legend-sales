...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4360",restoreGeneration:0};
...
// V43.6: an old cached DRAFT can actually be a card already acknowledged by
...
async function loadProductLinksIntoEditorV206(type){
  ...
  // V43.6: a fully confirmed + Import-processed context is final and immutable.
  // Open it entirely from permanent browser cache; no cloud read or repaint.
  if(cachedPainted&&typeof allSalesCardsFinalV432==='function'&&allSalesCardsFinalV432(cached)){
    const pre=productLinkPreV208(type),msg=document.getElementById(pre+"ProductLinkMsg");
    if(msg){msg.classList.add("hidden");msg.textContent="✅ 已确认销售 · Import 已处理库存";}
    return;
  }
...
function sanitizeSalesDraftPendingFinalTxnsV435(value){
  const obj=value&&typeof value==='object'?value:{},finalIds=typeof readFinalSalesCardTxnIdsV435==='function'?readFinalSalesCardTxnIdsV435():new Set();
  Object.keys(obj).forEach(key=>{const entry=obj[key],remaining=(Array.isArray(entry?.items)?entry.items:[]).filter(x=>!finalIds.has(String(x?.transactionId||x?.saleId||'')));if(remaining.length)entry.items=remaining;else delete obj[key]});return obj;
}
function readSalesDraftPendingV314(){try{const x=JSON.parse(localStorage.getItem(SALES_DRAFT_PENDING_KEY_V314)||'{}');return sanitizeSalesDraftPendingFinalTxnsV435(x)}catch(_){return{}}}
function writeSalesDraftPendingV314(obj){try{localStorage.setItem(SALES_DRAFT_PENDING_KEY_V314,JSON.stringify(sanitizeSalesDraftPendingFinalTxnsV435(obj||{})));return true}catch(_){return false}}
...
// V43.6: a final confirmed card must never be resurrected by an older durable
// draft queue. Remove only the completed transaction(s), preserving other cards.
function pruneFinalSalesDraftPendingV433(type,date,location,transactionIds=[]){
  const ids=new Set((Array.isArray(transactionIds)?transactionIds:[]).map(String).filter(Boolean));if(!ids.size)return;
  const all=readSalesDraftPendingV314(),key=salesDraftPendingKeyV314(type,date,location),entry=all[key];if(!entry)return;
  const remaining=(Array.isArray(entry.items)?entry.items:[]).filter(x=>!ids.has(String(x?.transactionId||x?.saleId||'')));
  if(remaining.length){entry.items=remaining;all[key]=entry}else delete all[key];
  writeSalesDraftPendingV314(all);
}
window.pruneFinalSalesDraftPendingV433=pruneFinalSalesDraftPendingV433;
...
function reconcileVisibleSalesCardAckV407(pendingItems=[]){
  ...
  ['daily','fair','live'].forEach(type=>{
    ...
    salesCardWrappersV239(type).forEach(card=>{
      ...
      const cardWasConfirmed=salesCardIsConfirmedV322(card);
      card.querySelectorAll('.product-link-item').forEach(item=>{
        const linkId=String(item.dataset.linkId||'').trim();let status=String(item.dataset.inventoryStatus||'').trim();
        if(!linkId||item.dataset.saved!=='1')return;hasSaved=true;
        if(cardWasConfirmed&&(status==='DRAFT'||status==='DRAFT_INVENTORY_CHANGED')){
          const productId=String(item.querySelector('.product-link-name')?.dataset.productId||'').trim();
          status=productId?(pendingIds.has(linkId)?'PENDING_IMPORT_LINK':'INVENTORY_CONFIRMED'):'NON_INVENTORY';
          item.dataset.inventoryStatus=status;resolvedStatuses.set(linkId,status);changed=true;
        }
        if(status==='PENDING_IMPORT_LINK'&&!pendingIds.has(linkId)){
          status='INVENTORY_CONFIRMED';item.dataset.inventoryStatus=status;resolvedStatuses.set(linkId,status);changed=true;
        }
      ...
      const terminal=[...card.querySelectorAll('.product-link-item[data-saved="1"]')].every(item=>['INVENTORY_CONFIRMED','NON_INVENTORY'].includes(String(item.dataset.inventoryStatus||'')));
      if(cardWasConfirmed&&hasSaved&&!hasPending&&terminal&&typeof markFinalSalesCardTxnIdsV435==='function')markFinalSalesCardTxnIdsV435([String(card.dataset.transactionId||'')]);
      if(card._renderSalesStateV317)card._renderSalesStateV317();
      updateSalesCardDeleteLockV322(card);
    });
    if(!resolvedStatuses.size)return;
    const updateLinks=list=>(Array.isArray(list)?list:[]).map(x=>{const status=resolvedStatuses.get(String(x?.linkId||'').trim());return status?{...x,importSyncStatus:status,confirmedOnce:true}:x});
...
/* ================= V43.6 once-per-day unconfirmed draft reminder =================
