...
function showPage(name,el){
  ...
  // V20.9: every time Live is opened, start from today's date.
  ...
  // V20.9: page switching never waits for or triggers cloud sync.
...
// V20.9: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V20.9: historical record is lazy. Top 5 opens instantly from local rows;
...
function applyCloudCommissionSettings(settings){
  ...
  // V20.9: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V20.9: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V20.9: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V20.9: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V20.9: local Fair values are direct replacements, never additions. The server
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"2009"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=sanitizeClosedMonthsClientV197(state.closedMonths,systemState.currentMonth);systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"2009"}updateReadOnlyMode()}
...
// V20.9: paint Home immediately, restore local cache, then perform only a
...
// V20.9: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V20.9: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V20.9: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V20.9 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V20.9: show the first editable card immediately. This is a pure front-end action
...
function buildProductLinkItemV209(type,id,data={}){
  ...
  const title=document.createElement("b");title.textContent=`盆栽 ${id}`;
  if(saved){const tag=document.createElement("span");tag.className="product-link-saved-tag";tag.textContent=" · 已保存";title.appendChild(tag)}
  ...
  const name=input("product-link-name",String(data.productName||""),"输入盆栽产品名称（下一阶段接 Import Cost System）");item.appendChild(name);

  const grid=document.createElement("div");grid.className="product-link-grid";
  ...
  const p=input("product-link-price money-input",formatAmount(Number(data.actualPrice||0)));p.inputMode="decimal";if(live)p.addEventListener("input",()=>applyLiveDeliveryDefaultItemV206(p));pWrap.appendChild(p);
  grid.append(qWrap,pWrap);item.appendChild(grid);
  ...
  if(live){
    ...
    const d=input("product-link-delivery money-input",formatAmount(Number(data.localDelivery||0)));d.inputMode="decimal";d.dataset.manual=(saved||Number(data.localDelivery||0)>0)?"1":"0";d.addEventListener("input",()=>{d.dataset.manual="1"});dWrap.appendChild(d);
    ...
    grid2.append(dWrap,eWrap);item.appendChild(grid2);
  }
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V20.9 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V20.9 button bindings reliable.
...
function renderAll(){
  // V20.9: one complete render path. This replaces the older partial duplicate
...
// V20.9: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V20.9: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V20.9: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V20.9: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V20.9: deleting the last special commission rule leaves candidate and
