...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4480",restoreGeneration:0};
...
// V44.8: paint Home immediately from Local First cache, then perform one
...
// V44.8: startup performs only Local First + the lightweight global Revision
...
function buildCloudImportProductRecordsV214(data){
  ...
  function addRecord(source){
    ...
    records.push({
      ...
      // V44.8: prefer the current Products ID. Old cached/import PZ IDs still
...
// V44.8: Sales Card freshness is controlled by the global Revision gate.
...
// V44.8: a completed sale is a whole-card, permanent local fact. Keep it in
...
// V44.8: an authoritative cloud context may legitimately become empty after a
...
async function loadProductLinksIntoEditorV206(type){
  ...
  // V44.8 Local First: cached Sales Cards open instantly and are NOT re-read
...
function addProductLinkItemV209(type,data={}){
  ...
  // V44.8: New Sales Card is local-only and instant. If a first-time cloud
...
function renderInventoryPendingGlobalV250(){
  ["daily","live","fair"].forEach(pageType=>{
    ...
    const list=inventoryPendingCacheV250.filter(item=>{
      // V44.8: a draft card must never flash an Import-inventory reminder.
...
/* ================= V44.8 once-per-day unconfirmed draft reminder =================
...
async function checkUnconfirmedDraftReminderV369(){
  ...
  try{
    ...
    // V44.8: only the explicit X button dismisses today's reminder. Leaving,
...
async function addTurnoverEntryV376(type){
  ...
  // V44.8: after an Apps Script timeout the optimistic entry intentionally
...
saveProductLinksV206=async function(type,...args){
  // V44.8 conflict guard: if this device has already detected a newer cloud
  // revision but has not completed the atomic refresh, do not let stale card
  // state overwrite another device's newer Sales Card. Viewing remains allowed.
  if(typeof window.cloudAtomicSyncPendingV448==='function'&&window.cloudAtomicSyncPendingV448()){
    alert('检测到其他设备有较新的云端资料，目前完整同步尚未完成。\n\n请先等待同步成功或重新打开后再修改/保存销售卡，避免覆盖其他设备的新资料。');
    return null;
  }
...
renderAll=function(){
  ...
  try{renderTurnoverComposerV376('daily');renderTurnoverComposerV376('fair');renderTurnoverComposerV376('live')}catch(e){console.warn('V44.8 post-render turnover',e)}
  return result;
};
...
/* ================= V44.8 system status + system information ================= */
...
function renderSystemInformationV442(data){
  ...
  set('systemInfoApiV442',data?.apiVersion?`V${String(data.apiVersion).replace(/^V/i,'').replace(/^4480$/,'44.8')}`:'连接异常');
...
async function runSystemHealthCheckV442(userTriggered=false){
  ...
  try{
    const data=await jsonp({action:'healthV443',clientVersion:'4480'},{timeoutMs:15000});
    if(!data?.ok)throw new Error(data?.message||'系统检查失败');
    const issues=[];
    if(String(data.apiVersion||'')!=='4480')issues.push(`Frontend / API 版本不一致（Frontend 4480 / API ${data.apiVersion||'未知'}）`);
    (Array.isArray(data.issues)?data.issues:[]).forEach(x=>issues.push(String(x)));
    const severe=Boolean(data.severe)||!data.sheetConnected||String(data.apiVersion||'')!=='4480';
