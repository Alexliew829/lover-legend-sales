...
function showPage(name,el){
  ...
  // V10.9: every time Live is opened, start from today's date.
  ...
  // V10.9: page switching never waits for or triggers cloud sync.
...
function getEffectiveCommissionSettings(){
  ...
  // V10.9: historical snapshots created before per-host commission support may
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"10.9"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=Array.isArray(state.closedMonths)?state.closedMonths:[];systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"10.9"}updateReadOnlyMode()}
...
// V10.9: render local Home first. Cloud work starts only after the browser
...
function restoreLastLiveSession(){
  ...
  // V10.9: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V10.9：单击后立即套用，并由所有已开启装置自动读取最新佣金。
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V10.9: apply locally first and immediately refresh Home/Report.
    if(button){button.disabled=true;button.textContent="正在储存...";}
    applyCommissionSettings(settings);
    if((systemState.closedMonths||[]).includes(selectedMonth())){
...
async function resetFairCommissionSettings(){
  ...
  try{
    // V10.9：确认恢复后先立即更新本机与 Home，再在后台同步 Google Sheet。
    applyCommissionSettings(settings);
    if((systemState.closedMonths||[]).includes(selectedMonth())){
      systemState.commissionSnapshots={
        ...(systemState.commissionSnapshots||{}),
...
/* ================= V10.9 Backup / Restore ================= */
function getBackupPayload(){return{system:"Lover Legend Sales System",version:"10.9",createdAt:new Date().toISOString(),rows:dedupeRows(rows),commissionSettings:getCommissionSettings(),
