...
function showPage(name,el){
  ...
  // V10.3: every time Live is opened, start from today's date.
  ...
  // V10.3: page switching never waits for or triggers cloud sync.
...
function getEffectiveCommissionSettings(){
  ...
  // V10.3: historical snapshots created before per-host commission support may
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"10.3"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=Array.isArray(state.closedMonths)?state.closedMonths:[];systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"10.3"}updateReadOnlyMode()}
...
// V10.3: display local data immediately, but never reuse an old “已同步” state after reopening.
...
function startInitialSalesDataLoad() {
  ...
  // V10.3: give the browser time to paint and accept input first.
...
function restoreLastLiveSession(){
  ...
  // V10.3: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V10.3：单击后立即套用，并由所有已开启装置自动读取最新佣金。
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V10.3: apply locally first and immediately refresh Home/Report.
    if(button){button.disabled=true;button.textContent="正在储存...";}
    applyCommissionSettings(settings);
    if((systemState.closedMonths||[]).includes(selectedMonth())){
...
/* ================= V10.3 Backup / Restore ================= */
function getBackupPayload(){return{system:"Lover Legend Sales System",version:"10.3",createdAt:new Date().toISOString(),rows:dedupeRows(rows),commissionSettings:getCommissionSettings(),
...
function backupAllData(){const payload=getBackupPayload();const stamp=new Date().toISOString().replace(/[:T]/g,"-").slice(0,19);downloadFile(`Lover_Legend_Sales_V10_3_Backup_${stamp}.json`,JSON.stringify(payload,null,2),"application/json;charset=utf-8;")}
