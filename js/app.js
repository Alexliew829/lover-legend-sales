...
function renderTodayCompanyStatus(){
  // V37.9: this card follows the explicit "查看日期" selector. Keep the
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"3790",restoreGeneration:0};
...
async function loadProductLinksIntoEditorV206(type){
  ...
  try{
    ...
    // V37.9: a temporary JSONP/network miss must not immediately turn a valid
...
function refreshMonthlyRollupV365(type){
  ...
  // V37.9: profit data is a second cloud source. A transient timeout used to
...
/* ================= V37.9 Import -> associated Sales card deep link =================
...
async function openAssociatedSalesCardFromImportV370(){
  ...
  try{await loadProductLinksIntoEditorV206(type)}catch(e){console.warn('V37.9 associated card load failed',e)}
...
/* ================= V37.9 Fair/Live incremental turnover composer =================
...
function fallbackTurnoverEntriesV376(type,date,location){const total=officialTurnoverV376(type,date,location);return total>0?[{id:'legacy_'+turnoverContextKeyV376(type,date,location),amount:total,legacy:true,createdAt:'',updatedAt:''}]:[]}
...
async function refreshTurnoverEntriesV376(type,{force=false}={}){
  ...
  try{
    ...
    if(rec&&Array.isArray(rec.entries)){
      const official=officialTurnoverV376(type,ctx.date,ctx.location),sum=entriesSumV376(rec.entries);
      if(Math.abs(sum-official)<=0.005)setTurnoverEntryCacheV376(type,ctx.date,ctx.location,rec.entries,'cloud');
    }
  }catch(e){console.warn('V37.9 turnover entry read',e)}
...
async function commitTurnoverEntriesV376(type,entries,actionText){
  ...
  catch(e){rememberTurnoverEntryPendingV376(type,ctx.date,ctx.location,clean,total);console.warn('V37.9 entry detail sync failed',e);setSync(`${type==='live'?'Live':'Fair'} 总营业额已同步；明细稍后自动重试`,true)}
...
async function addTurnoverEntryV376(type){
  ...
  entries.push({id:'e_'+Date.now()+'_'+Math.random().toString(36).slice(2,7),amount:value,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()});
...
// V37.9 save-card handoff: immediately repaint the selected day's profit from the saved result/cache.
...
saveProductLinksV206=async function(type,...args){
  ...
  }catch(e){console.warn('V37.9 immediate profit repaint',e)}return result;
};window.saveProductLinksV206=saveProductLinksV206;

// V37.9 deep-link fast path: handle target as soon as scripts are ready instead of waiting for full load + 500ms.
...
async function retryTurnoverEntryPendingV376(){
  ...
  for(const [key,item] of Object.entries(pending)){
    try{
      ...
      else console.warn('V37.9 turnover detail retry',e);
    }
  }
...
/* ================= V37.9 turnover UI + legacy-date compatibility =================
...
function refreshTurnoverContextV377(type,{cloud=true}={}){
  try{renderTurnoverComposerV376(type)}catch(e){console.warn('V37.9 turnover render',e)}
  if(cloud){
    setTimeout(()=>{try{refreshTurnoverEntriesV376(type,{force:true})}catch(e){console.warn('V37.9 turnover cloud refresh',e)}},0);
  }
}
...
// Earlier date controls captured the old update function before V37.9 replaced it.
...
renderAll=function(){
  ...
  try{renderTurnoverComposerV376('fair');renderTurnoverComposerV376('live')}catch(e){console.warn('V37.9 post-render turnover',e)}
  return result;
};
