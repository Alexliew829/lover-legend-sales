...
// V39.3: turnover writes are real transactions. Keep navigation guarded until cloud confirmation.
...
function renderTodayCompanyStatus(){
  // V39.3: this card follows the explicit "查看日期" selector. Keep the
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"3930",restoreGeneration:0};
...
async function loadProductLinksIntoEditorV206(type){
  ...
  try{
    ...
    // V39.3: a temporary JSONP/network miss must not immediately turn a valid
...
function refreshMonthlyRollupV365(type){
  ...
  // V39.3: profit data is a second cloud source. A transient timeout used to
...
/* ================= V39.3 Import -> associated Sales card deep link =================
...
async function openAssociatedSalesCardFromImportV370(){
  ...
  try{await loadProductLinksIntoEditorV206(type)}catch(e){console.warn('V39.3 associated card load failed',e)}
...
/* ================= V39.3 Fair/Live incremental turnover composer =================
...
async function refreshTurnoverAuditDerivedV380(type,ctx){
  ...
  try{
    ...
    return derived;
  }catch(e){console.warn('V39.3 turnover audit reconstruction',e);return[]}
}
...
async function refreshTurnoverEntriesV376(type,{force=false}={}){
  ...
  }catch(e){console.warn('V39.3 turnover entry read',e);try{const derived=await refreshTurnoverAuditDerivedV380(type,ctx);if(derived.length)setTurnoverEntryCacheV376(type,ctx.date,ctx.location,derived,'audit-v380')}catch(_){}}
...
async function commitTurnoverEntriesV376(type,entries,actionText){
  ...
  catch(e){rememberTurnoverEntryPendingV376(type,ctx.date,ctx.location,clean,total);console.warn('V39.3 entry detail sync failed',e);setSync(`${type==='live'?'Live':'Fair'} 总营业额已同步；明细稍后自动重试`,true)}
...
async function addTurnoverEntryV376(type){
  ...
  // V39.3: show the new component/total immediately while the authoritative cloud write runs.
...
// V39.3 save-card handoff: immediately repaint the selected day's profit from the saved result/cache.
...
saveProductLinksV206=async function(type,...args){
  ...
  }catch(e){console.warn('V39.3 immediate profit repaint',e)}return result;
};window.saveProductLinksV206=saveProductLinksV206;

// V39.3 deep-link fast path: handle target as soon as scripts are ready instead of waiting for full load + 500ms.
...
async function retryTurnoverEntryPendingV376(){
  ...
  for(const [key,item] of Object.entries(pending)){
    try{
      ...
      else console.warn('V39.3 turnover detail retry',e);
    }
  }
...
/* ================= V39.3 turnover UI + legacy-date compatibility =================
...
function refreshTurnoverContextV377(type,{cloud=true}={}){
  try{renderTurnoverComposerV376(type)}catch(e){console.warn('V39.3 turnover render',e)}
  if(cloud){
    setTimeout(()=>{try{refreshTurnoverEntriesV376(type,{force:true})}catch(e){console.warn('V39.3 turnover cloud refresh',e)}},0);
  }
}
...
// Earlier date controls captured the old update function before V39.3 replaced it.
...
renderAll=function(){
  ...
  try{renderTurnoverComposerV376('fair');renderTurnoverComposerV376('live')}catch(e){console.warn('V39.3 post-render turnover',e)}
  return result;
};
