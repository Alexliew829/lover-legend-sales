...
function showPage(name,el){
  ...
  // V24.6: every time Live is opened, start from today's date.
  ...
  // V24.6: page switching never waits for or triggers cloud sync.
...
// V24.6: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V24.6: historical record is lazy. Top 5 opens instantly from local rows;
...
function applyCloudCommissionSettings(settings){
  ...
  // V24.6: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V24.6: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V24.6: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V24.6: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V24.6: local Fair values are direct replacements, never additions. The server
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"2460"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=sanitizeClosedMonthsClientV197(state.closedMonths,systemState.currentMonth);systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"2460"}updateReadOnlyMode()}
...
// V24.6: paint Home immediately, restore local cache, then perform only a
...
// V24.6: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V24.6: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V24.6: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V24.6 Import Cost System product search ================= */
...
function unorderedImportProductMatchV214(sourceValue,queryValue){
  ...
  if(isExactProductCodeQueryV214(query)){
    // V24.6: code search supports case-insensitive prefix/partial entry.
...
function setupImportProductSearchV214(item,nameInput,resultsBox,closeButton){
  // V24.6: iPhone/iOS safe search state.
  ...
  const render=async()=>{
    ...
    try{
      ...
      matches.forEach(record=>{
        ...
        // V24.6: distinguish an intentional tap from list scrolling on iPhone/iOS.
...
/* ================= V24.6 Sales Card unsaved-change protection ================= */
...
/* ================= V24.6 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V24.6: Sales Cards and Daily Profit are mutually exclusive.
    ...
    // V24.6: saved cards first. Do not flash a fake 0.00 card while cloud data loads.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V24.6 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V24.6 button bindings reliable.
...
/* ================= V24.6 multi-product Sales Card ================= */
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V24.6 sales card container missing",type);return false}
...
/* ================= V24.6 Sales Card unit-price + stable optimistic UI ================= */
...
async function saveProductLinksV206(type){
  ...
  // V24.6 optimistic local cache: keep the current UI stable; do not redraw from an older async response.
...
/* ================= V24.6 same-day linked bonsai profit summary ================= */
...
async function toggleProductProfitSummaryV216(type,button){
  ...
  try{
    // V24.6: opening Daily Profit closes Sales Cards first, then paints local cache immediately.
    const pre=productLinkPreV208(type),box=document.getElementById(pre+"ProductLinkBox"),body=document.getElementById(pre+"ProductLinkBody");
...
/* ================= V24.6 expandable yearly monthly breakdown ================= */
...
function yearBreakdownRowsV224(kind){
  // V24.6: Balakong / Belimbing / Fair / Live show months inside the selected
...
function renderAll(){
  // V24.6: one complete render path. This replaces the older partial duplicate
...
// V24.6: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V24.6: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V24.6: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V24.6: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V24.6: deleting the last special commission rule leaves candidate and
...
/* ================= V24.6 Sales Card transaction integrity / instant cache ================= */
...
/* ================= V24.6 Live optimistic revision guard ================= */
...
async function refreshOpenSalesChangeLogAfterSaveV243(type,date){
  ...
  }catch(e){console.warn("V24.6 修改/销售记录后台刷新失败",e)}
}
...
saveLiveSales=async function(){
  ...
  // Critical V24.6 guard: any cloud request started before this local mutation
...
/* ================= V24.6 Profit list copy + compact totals ================= */
...
/* ================= V24.6 profit layout + weighted average margin ================= */
...
renderProductProfitSummaryV216=function(type,allLinks){
  ...
  const sections=ordered.map(group=>{const s=productProfitGroupStatsV216(group,salesMap,type),t=productProfitTotalsV246(group.links),avg=productProfitWeightedAverageRateV246(group.links);dayProfit+=s.totalProfit;daySales+=s.sales;
    const totals=group.links.length?`<div class="product-profit-rollup-v246"><span>总数：</span><b>成本 RM${formatAmount(t.cost)}</b><b>售价 RM${formatAmount(t.sale)}</b><b>利润 RM${formatAmount(t.profit)}</b><b>平均利润率 ${avg.toFixed(2)}%</b></div>`:'';
