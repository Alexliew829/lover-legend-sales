...
/* ================= V29.4 expandable yearly monthly breakdown + profit ================= */
...
function liveProfitRowV294(r,profit){
  const sales=Number(r.amount||0);
  return `<div class="month-profit-row-v294 live-profit-row-v294"><span><b>${r.date}</b><small>佣金 ${r.commissionRate}% · ${money(r.commissionAmount)}</small></span><strong>${money(sales)}</strong><strong class="profit-value-v294">${money(profit)}</strong><strong class="profit-rate-v294">${marginTextV294(profit,sales)}</strong></div>`;
}
...
renderFairMonthlyList=function(){
  ...
  if(!list.length){container.innerHTML='<div class="sub">这个月份还没有 Fair 记录</div>';renderFairPageTop3();return;}
  ...
  Promise.resolve(loadAllSalesProductLinksV203({force:false,maxAgeMs:120000})).then(links=>{
    ...
    container.innerHTML=locations.map(group=>{
      ...
      return `<div class="month-record-group month-profit-group-v294"><div class="month-record-group-title">${escapeChangeLogHtmlV200(group.name)}</div>${profitHeadV294()}${rowsHtml}<div class="month-profit-row-v294 month-profit-total-v294"><span>这场 Fair 总数</span><strong>${money(groupSales)}</strong><strong class="profit-value-v294">${money(groupProfit)}</strong><strong class="profit-rate-v294">${marginTextV294(groupProfit,groupSales)}</strong></div></div>`;
    }).join('');
  ...
  }).catch(e=>console.warn('V29.4 Fair 利润读取失败',e));
};
...
renderLiveMonthlyList=function(){
  ...
  container.innerHTML=groups.map(group=>{const sales=group.rows.reduce((s,r)=>s+Number(r.amount||0),0),comm=group.rows.reduce((s,r)=>s+Number(r.commissionAmount||0),0);return `<div class="live-sales-group month-profit-group-v294"><div class="live-sales-group-title"><span>${escapeChangeLogHtmlV200(group.name)}</span><b>佣金 ${money(comm)}</b></div>${profitHeadV294()}${group.rows.map(r=>liveProfitRowV294(r,0)).join('')}<div class="month-profit-row-v294 month-profit-total-v294"><span>${escapeChangeLogHtmlV200(group.name)} 总数</span><strong>${money(sales)}</strong><strong class="profit-value-v294">0.00</strong><strong class="profit-rate-v294">0.00%</strong></div></div>`}).join('')+`<div class="profit-grand-v294"><strong>Live 全部主播总计</strong><div><span>营业额</span><b>${money(total)}</b></div><div><span>利润</span><b>0.00</b></div><div><span>整体利润率</span><b>0.00%</b></div></div>`;
  renderLivePageTop3();
  Promise.resolve(loadAllSalesProductLinksV203({force:false,maxAgeMs:120000})).then(links=>{
    if(token!==monthlyProfitRenderTokenV294.live)return;
    container.innerHTML=groups.map(group=>{let gp=0,gs=0;const comm=group.rows.reduce((s,r)=>s+Number(r.commissionAmount||0),0);const html=group.rows.map(r=>{const p=profitByDayV294(links,'live',group.name,r.date);gp+=p;gs+=Number(r.amount||0);return liveProfitRowV294(r,p)}).join('');return `<div class="live-sales-group month-profit-group-v294"><div class="live-sales-group-title"><span>${escapeChangeLogHtmlV200(group.name)}</span><b>佣金 ${money(comm)}</b></div>${profitHeadV294()}${html}<div class="month-profit-row-v294 month-profit-total-v294"><span>${escapeChangeLogHtmlV200(group.name)} 总数</span><strong>${money(gs)}</strong><strong class="profit-value-v294">${money(gp)}</strong><strong class="profit-rate-v294">${marginTextV294(gp,gs)}</strong></div></div>`}).join('');
  ...
  }).catch(e=>console.warn('V29.4 Live 利润读取失败',e));
};
