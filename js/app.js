...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4730",restoreGeneration:0};
...
function refreshProductLinkContextV210(type){
  ...
  // V47.3: context switches paint Local First immediately, but the dedicated
...
async function loadProductLinksIntoEditorV206(type,options={}){
  ...
  // No cache on this device. During a committed context switch V47.3 lets the
...
function bindTurnoverContextRefreshV377(){
  ...
  // V47.3: context controls repaint local data only. One dedicated current-context
...
// V47.3: the read-only health endpoint is advisory only. A slow health scan must
...
function renderSystemInformationV442(data){
  ...
  set('systemInfoApiV442',data?.apiVersion?`V${String(data.apiVersion).replace(/^V/i,'').replace(/^4730$/,'47.3')}`:'连接异常');
...
async function runSystemHealthCheckV442(userTriggered=false){
  ...
  try{
    const data=await jsonp({action:'healthV443',clientVersion:'4730'},{timeoutMs:15000});
    if(!data?.ok)throw new Error(data?.message||'系统检查失败');
    const issues=[];
    if(String(data.apiVersion||'')!=='4730')issues.push(`Frontend / API 版本不一致（Frontend 4730 / API ${data.apiVersion||'未知'}）`);
    (Array.isArray(data.issues)?data.issues:[]).forEach(x=>issues.push(String(x)));
    const severe=Boolean(data.severe)||!data.sheetConnected||String(data.apiVersion||'')!=='4730';
...
// V47.3: expose only the currently visible Sales/Fair/Live business context to
...
// V47.3: a committed Sales/Fair/Live context switch must immediately verify the
