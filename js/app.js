...
function showPage(name,el){
  ...
  if(currentName&&currentName!==name&&typeof hasUnsavedSalesCardChangesV238==="function"&&hasUnsavedSalesCardChangesV238(currentName)){
    if(!confirm("销售卡尚未保存，确定离开？\n\n点击“确定”将放弃未保存的销售卡修改。"))return false;
    discardUnsavedSalesCardChangesV238(currentName);
  }
  ...
  // V26.2: every time Live is opened, start from today's date.
  ...
  // V26.2: page switching never waits for or triggers cloud sync.
...
// V26.2: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V26.2: historical record is lazy. Top 5 opens instantly from local rows;
...
function renderTodayCompanyStatus(){
  ...
  // V26.2: status colour represents actual business amount, not merely
...
function applyCloudCommissionSettings(settings){
  ...
  // V26.2: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V26.2: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V26.2: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V26.2: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V26.2: local Fair values are direct replacements, never additions. The server
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=sanitizeClosedMonthsClientV197(state.closedMonths,systemState.currentMonth);systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"2480"}updateReadOnlyMode()}
...
// V26.2: paint Home immediately, restore local cache, then perform only a
...
// V26.2: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V26.2: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V26.2: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V26.2 Import Cost System product search ================= */
...
function unorderedImportProductMatchV214(sourceValue,queryValue){
  ...
  if(isExactProductCodeQueryV214(query)){
    // V26.2: code search supports case-insensitive prefix/partial entry.
...
function setupImportProductSearchV214(item,nameInput,resultsBox,closeButton){
  // V26.2: iPhone/iOS safe search state.
  ...
  const render=async()=>{
    ...
    try{
      ...
      matches.forEach(record=>{
        ...
        // V26.2: distinguish an intentional tap from list scrolling on iPhone/iOS.
  ...
  if(closeButton){
    ...
    closeButton.addEventListener("click",e=>{
      ...
      // V26.2: X means CLEAR CURRENT PRODUCT SELECTION.
...
/* ================= V26.2 Sales Card unsaved-change protection ================= */
...
function hasUnsavedSalesCardChangesV238(type){
  if(!["live","fair"].includes(String(type||"")))return false;
...
function discardUnsavedSalesCardChangesV238(type){
  if(!["live","fair"].includes(String(type||"")))return;
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(wrap)[...wrap.querySelectorAll(".product-link-item")].forEach(clearSalesCardDirtyV238);
...
/* ================= V26.2 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V26.2: Sales Cards and Daily Profit are mutually exclusive.
    ...
    // V26.2: saved cards first. Do not flash a fake 0.00 card while cloud data loads.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V26.2 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V26.2 button bindings reliable.
...
async function saveProductLinksV206(type){
  ...
  try{
    ...
    setSync("销售卡已保存",true);
    alert("销售卡保存成功。\n\n如有从 Import Cost System 选择的产品，请记得到 Import Cost System 手动修改库存；处理后在销售卡按「我已处理库存」。");
...
/* ================= V26.2 multi-product Sales Card ================= */
...
function buildProductSubItemV239(type,card,data={},order=1){
  ...
  [c,p].forEach(el=>el.addEventListener("blur",()=>{
    ...
    // V26.2: blur fires before the Save button click. Re-mark the card dirty here so
...
function bindSingleTapDeleteProductV254(button,handler){
  ...
  // V26.2: for touch/pen, pointerup is the one and only real activation.
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  const inventoryBox=document.createElement("div");inventoryBox.className="sales-card-inventory-v249";
  const renderInventoryStatus=()=>{
    const mapped=[...card.querySelectorAll(".product-link-item")].filter(x=>String(x.querySelector(".product-link-name")?.dataset.productId||""));
    if(!mapped.length){inventoryBox.hidden=true;return}
    inventoryBox.hidden=false;
    const done=card.dataset.inventoryStatus==="INVENTORY_CONFIRMED";
    inventoryBox.innerHTML=done?'<span class="inventory-done-v249">✓ 库存已手动处理</span>':'<span class="inventory-pending-v249">⚠️ 库存待处理：请到 Import Cost System 手动修改库存</span><button type="button">✓ 我已处理库存</button>';
    const btn=inventoryBox.querySelector("button");
    if(btn)btn.onclick=async()=>{
      if(!confirm("请确认：你已经在 Import Cost System 手动修改这张销售卡对应的库存。\n\n确认后，这张销售卡将不再提示。"))return;
      const ctx=productLinkContextV206(type);
      try{
        btn.disabled=true;btn.textContent="确认中…";
        await confirmSalesCardInventoryV249({type,date:ctx.date,location:ctx.location,transactionId:String(card.dataset.transactionId||"")});
        card.dataset.inventoryStatus="INVENTORY_CONFIRMED";
        card.querySelectorAll(".product-link-item").forEach(x=>{if(String(x.querySelector(".product-link-name")?.dataset.productId||""))x.dataset.inventoryStatus="INVENTORY_CONFIRMED"});
        renderInventoryStatus();setSync("库存处理状态已确认",true);
        if(typeof refreshInventoryPendingV250==="function")refreshInventoryPendingV250(true);
      }catch(e){btn.disabled=false;btn.textContent="✓ 我已处理库存";alert("库存状态确认失败："+(e.message||e))}
    };
  };
  ...
  setTimeout(()=>{
    // V26.2: initialization may finish after a very fast first edit.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V26.2 sales card container missing",type);return false}
...
/* ================= V26.2 Sales Card unit-price + stable optimistic UI ================= */
...
function productAutoDeliveryV240(item){
  return liveDeliveryDefaultV203(salesCardProductTotalV240(item));
}
function salesCardSharedValuesV239(card){
  return{
    commissionRate:Math.max(0,Number(String(card.querySelector(".sales-card-commission-rate-v239")?.value||"0").replace(/[^0-9.\-]/g,""))||0),
...
function allocateSalesCardSharedCostsV239(card){
  ...
  return products.map((item,i)=>{
    ...
    const extraWeight=totalPrice>0?totals[i]/totalPrice:(products.length?1/products.length:0);
    return{
      delivery,
      extra:shared.extraTotal*extraWeight,
...
function buildProductSubItemV239(type,card,data={},order=1){
  ...
  const result=document.createElement("div");result.className="product-subitem-result-v240";
  ...
  const shipInput=input("product-link-delivery-v240",formatAmount(Number(data.localDelivery||liveDeliveryDefaultV203(qty*unitPrice))));
  shipInput.inputMode="decimal";shipInput.dataset.manual=(saved&&Number(data.localDelivery||0)!==liveDeliveryDefaultV203(qty*unitPrice))?"1":"0";ship.appendChild(shipInput);
  ...
  shipInput.addEventListener("blur",()=>{shipInput.value=formatAmount(toAmount(shipInput.value||0));recalcSalesCardTransactionV239(card)});
  bindSingleTapDeleteProductV254(remove,()=>removeProductFromTransactionV239(type,card,item));
  setTimeout(()=>{updateProductLinkMinimumWarningV214(item);recalcSalesCardTransactionV239(card)},0);
  return item;
}
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  extra.inp.addEventListener("blur",()=>{extra.inp.value=formatAmount(toAmount(extra.inp.value||0));recalcSalesCardTransactionV239(card)});
  commission.inp.addEventListener("blur",()=>{commission.inp.value=(Math.max(0,Number(commission.inp.value)||0)).toFixed(2);recalcSalesCardTransactionV239(card)});
...
async function saveProductLinksV206(type){
  ...
  // V26.2 optimistic local cache: keep the current UI stable; do not redraw from an older async response.
...
/* ================= V26.2 same-day linked bonsai profit summary ================= */
...
function productProfitSelectedDateV216(type){
  return type==="live"?isoToDisplay(document.getElementById("liveDate")?.value||""):String(document.getElementById("fairProductDate")?.value||"");
}
function productProfitGroupKeyV216(type,value){return type==="live"?normalizeLiveHostKey(value):normalizeFairLocationKey(value)}
function productProfitSalesByGroupV216(type,date){
  const map=new Map();
  dedupeRows(rows).forEach(r=>{
    if(r.type!==type||r.date!==date)return;
    const name=String(r.location||"").trim();if(!name)return;
...
function productProfitGroupLinksV216(type,date,links){
  ...
  (links||[]).filter(x=>String(x.type||"")===type&&String(x.date||"")===date&&String(x.productName||"").trim()).forEach(x=>{
    const name=String(x.location||"").trim()||"未指定",key=productProfitGroupKeyV216(type,name)||name;
...
function renderProductProfitSummaryV216(type,allLinks){
  ...
  const sections=ordered.map(group=>{
    ...
    return `<section class="product-profit-group"><div class="product-profit-group-title">${type==="live"?"主播":"地点"}：${escapeChangeLogHtmlV200(group.name)}</div><div class="product-profit-table-wrap"><table class="product-profit-table"><thead><tr><th>产品名</th><th>成本总数</th><th>售价总数</th><th>利润总数</th><th>利润率</th></tr></thead><tbody>${group.links.length?productProfitDesktopRowsV216(group.links):`<tr><td colspan="5" class="product-profit-empty">没有已保存的关联盆栽资料</td></tr>`}</tbody></table></div><div class="product-profit-mobile-list">${group.links.length?productProfitMobileCardsV216(group.links):`<div class="product-profit-empty">没有已保存的关联盆栽资料</div>`}</div><div class="product-profit-total profit-highlight"><span>总利润</span><b>RM${formatAmount(s.totalProfit)}</b></div><div class="product-profit-total profit-highlight"><span>整体利润率</span><b>${s.overallRate.toFixed(2)}%</b></div><div class="product-profit-formula">全部利润总和 ÷ ${type==="live"?"该主播":"该地点"}销售额 RM${formatAmount(s.sales)}</div></section>`;
  }).join("");
  const dayRate=daySales>0?dayProfit/daySales*100:0;
  panel.innerHTML=`<div class="product-profit-summary-head"><div><b>${date} · ${type==="live"?"全部主播":"全部地点"}</b></div><button type="button" class="secondary-btn product-profit-export-btn" onclick="exportProductProfitExcelV216('${type}')">📈 导出 Excel</button></div>${sections||`<div class="product-profit-empty">当天还没有关联盆栽资料</div>`}<div class="product-profit-day-summary"><div class="product-profit-total profit-highlight"><span>当天总利润</span><b>RM${formatAmount(dayProfit)}</b></div><div class="product-profit-total profit-highlight"><span>当天整体利润率</span><b>${dayRate.toFixed(2)}%</b></div></div>`;
...
async function toggleProductProfitSummaryV216(type,button){
  ...
  try{
    // V26.2: opening Daily Profit closes Sales Cards first, then paints local cache immediately.
    const pre=productLinkPreV208(type),box=document.getElementById(pre+"ProductLinkBox"),body=document.getElementById(pre+"ProductLinkBody");
...
function renderMonthGrandHistoryV223(){
  ...
  const grand=list.reduce((sum,item)=>sum+Number(item.amount||0),0);
  panel.classList.remove("hidden");
  if(arrow)arrow.textContent="▲";
  ...
  panel.innerHTML=list.length
    ?`<div class="month-grand-history-list">${list.map(x=>`<div><span>${x.label}</span><b>RM${money(x.amount)}</b></div>`).join("")}<div class="month-grand-history-total"><span>总数</span><b>RM${money(grand)}</b></div></div>`
...
async function toggleMonthGrandHistoryV223(){
  ...
  if(typeof loadYearInBackground==="function"&&/^\d{4}$/.test(year)){
    ...
    try{
      ...
      if(result&&result.ok)renderMonthGrandHistoryV223();
    }catch(e){
      console.warn("Month turnover history load skipped:",e);
    }finally{
...
/* ================= V26.2 expandable yearly monthly breakdown ================= */
...
function yearBreakdownRowsV224(kind){
  // V26.2: Balakong / Belimbing / Fair / Live show months inside the selected
...
function renderYearBreakdownV224(kind){
  ...
  panel.innerHTML=list.length
    ?`<div class="month-grand-history-list">${list.map(x=>`<div><span>${kind==="total"?String(x.year||x.month):String(x.month).slice(5,7)+"-"+String(x.month).slice(0,4)}</span><b>RM${money(x.amount)}</b></div>`).join("")}<div class="month-grand-history-total"><span>总数</span><b>RM${money(grand)}</b></div></div>`
    :'<div class="sub">还没有月份营业额记录</div>';
}
...
async function toggleYearBreakdownV224(kind){
  ...
  if(typeof loadYearInBackground==="function"&&/^\d{4}$/.test(year)){
    ...
    try{ await loadYearInBackground(year); }
...
function renderAll(){
  // V26.2: one complete render path. This replaces the older partial duplicate
...
// V26.2: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V26.2: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V26.2: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
window.addEventListener("beforeunload",event=>{
  const unsavedCards=hasUnsavedSalesCardChangesV238("live")||hasUnsavedSalesCardChangesV238("fair");
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V26.2: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V26.2: deleting the last special commission rule leaves candidate and
...
async function backupAllData(){
  ...
  try{
    ...
    payload.backupIncludes={sales:true,fair:true,live:true,commission:true,closedMonths:true,commissionSnapshots:true,productLinks:true,salesChangeLogs:true,profitData:true,remarks:true,averageCost:true,minimumPrice:true,deliveryAndExtraFees:true};
    setBackupRestoreStateV234({type:"backup",status:"running",message:"正在生成 Backup 文件..."});
    const stamp=new Date().toISOString().replace(/[:T]/g,"-").slice(0,19);
    downloadFile(`Lover_Legend_Sales_V23_7_Backup_${stamp}.json`,JSON.stringify(payload,null,2),"application/json;charset=utf-8");
...
/* ================= V26.2 Sales Card transaction integrity / instant cache ================= */
...
function salesCardsOfficialAmountV241(type){
  const ctx=productLinkContextV206(type);
  return dedupeRows(rows).filter(r=>r.type===type&&r.date===ctx.date&&(type==='live'?normalizeLiveHostKey(r.location)===normalizeLiveHostKey(ctx.location):normalizeFairLocationKey(r.location)===normalizeFairLocationKey(ctx.location))).reduce((m,r)=>Math.max(m,Number(r.amount||0)),0);
}
...
saveProductLinksV206=async function(type){
  const cards=salesCardWrappersV239(type),dirty=cards.filter(c=>c.dataset.dirty==='1'||c.dataset.productRemovedV259==='1'||!salesCardIsSavedV241(c));
  if(!dirty.length){alert('销售卡没有需要保存的修改。');return null;}
  /* Keep only changed transactions in the request; server treats existing other cards as context. */
  ...
  if(!items.length){alert('请填写销售卡产品资料。');return null;}
  const first=items[0],official=salesCardsOfficialAmountV241(type),allTotal=all.reduce((s,x)=>s+Number(x.actualPrice||0),0);
  if(official>0&&allTotal>official+0.005){alert(`所有销售卡售价总数 RM${formatAmount(allTotal)} 已超过当天营业额 RM${formatAmount(official)}。`);return null;}
  for(const x of items){
    if(!x.productName){alert('每一个产品都需要填写产品名称。');return null}
    if(!Number.isFinite(Number(x.quantity))||Number(x.quantity)<=0){alert(`产品${Number(x.productOrder||0)||""} 数量必须大于 0。`);return null}
    if(!Number.isFinite(Number(x.unitPrice))||Number(x.unitPrice)<=0){
      alert(`产品${Number(x.productOrder||0)||""} 售价必须大于 RM0.00，无法保存销售卡。`);
      return null;
    }
  }
  /* Optimistic merge before network: prevents old cached cards/records flashing back. */
  const ctx=productLinkContextV206(type),old=(typeof getSalesCardPersistentCacheV232==='function'?getSalesCardPersistentCacheV232(type,ctx.date,ctx.location):[])||[];
  const merged=[...old.filter(x=>!dirtyIds.has(String(x.transactionId||''))),...items];
  ...
  try{
    setSync('销售卡同步中...');const result=await window.saveSalesProductLinksApiV241(items);const saved=Array.isArray(result?.links)?result.links:items;
    ...
    dirty.forEach(card=>{clearSalesCardTransactionDirtyV239(card);delete card.dataset.productRemovedV259;card.querySelectorAll('.product-link-item').forEach(i=>{i.dataset.saved='1';i.dataset.dirty='0'})});
    renumberSavedSalesCardsV241(type);setSync('销售卡已保存',true);alert('销售卡保存成功。');if(result?.warning)alert(result.warning);return result;
  }catch(e){setSync('销售卡同步失败',false,true);alert('销售卡保存失败：'+(e.message||e));return null}
};
...
/* ================= V26.2 Live optimistic revision guard ================= */
...
async function refreshOpenSalesChangeLogAfterSaveV243(type,date){
  ...
  }catch(e){console.warn("V26.2 修改/销售记录后台刷新失败",e)}
}
...
saveLiveSales=async function(){
  ...
  // Critical V26.2 guard: any cloud request started before this local mutation
...
/* ================= V26.2 Profit list copy + compact totals ================= */
...
/* ================= V26.2 profit layout + weighted average margin ================= */
...
/* ================= V26.2 cross-date inventory reminders ================= */
...
function inventoryPendingProductTextV250(item){
  if(item&&item.productName)return `${String(item.productName||"未命名产品")} ${Number(item.quantity||1)}棵`;
  return (item.products||[]).map(p=>`${String(p.productName||"未命名产品")} ${Number(p.quantity||1)}棵`).join("、");
}
function copyInventoryProductNameV262(name,el){
  const text=String(name||"").trim();if(!text)return;
  const done=()=>{const old=el?.textContent;if(el)el.textContent="已复制";setTimeout(()=>{if(el)el.textContent=old||text},800);};
...
function renderInventoryPendingGlobalV250(){
  ["live","fair"].forEach(pageType=>{
    const box=document.getElementById(pageType+"InventoryReminderV250");if(!box)return;
    const list=inventoryPendingCacheV250;
    if(!list.length){box.classList.add("hidden");box.innerHTML="";return}
    box.classList.remove("hidden");
    box.innerHTML=`<div class="inventory-global-head-v250"><b>⚠️ 有 ${list.length} 项库存待处理</b><span>请到 Import Cost System 手动修改库存</span></div>`+
      list.map(item=>{
        const old=inventoryPendingDaysOldV250(item.date),stale=old>=1,name=String(item.productName||"未命名产品"),qty=Number(item.quantity||1);
        return `<div class="inventory-global-item-v250${stale?" stale-v250":""}">
          <div class="inventory-global-main-v250">
            <b>${stale?"⚠️ ":""}${escapeChangeLogHtmlV200(item.date)} · ${item.type==="live"?"Live":"Fair"} · ${escapeChangeLogHtmlV200(item.location)}</b>
            <button type="button" class="inventory-product-copy-v262" title="点击复制产品名称" onclick='copyInventoryProductNameV262(${JSON.stringify(name)},this)'>${escapeChangeLogHtmlV200(name)}</button>
            <small>${qty}棵${stale?` · 已待处理 ${old} 天`:""}</small>
          </div>
          <div class="inventory-global-actions-v250">
            <button type="button" class="confirm-v250" onclick='confirmPendingInventoryV250(${JSON.stringify(JSON.stringify(item))},this)'>✓ 我已处理库存</button>
          </div>
        </div>`;
      }).join("");
  });
...
async function refreshInventoryPendingV250(force=false){
  ...
  inventoryPendingLoadingV250=(async()=>{
    try{
      ...
      inventoryPendingCacheV250=Array.isArray(list)?list:[];
...
async function confirmPendingInventoryV250(raw,btn){
  const item=typeof raw==="string"?JSON.parse(raw):raw;if(!item)return;
  if(!confirm(`请确认：你已经在 Import Cost System 手动修改以下库存。\\n\\n${item.date} · ${item.type==="live"?"Live":"Fair"} · ${item.location}\\n${inventoryPendingProductTextV250(item)}\\n\\n确认后，这笔不再提示。`))return;
  try{
    if(btn){btn.disabled=true;btn.textContent="确认中…"}
    await confirmSalesCardInventoryV249({type:item.type,date:item.date,location:item.location,transactionId:item.transactionId,linkId:item.linkId||""});
    inventoryPendingCacheV250=inventoryPendingCacheV250.filter(x=>String(x.linkId||"")!==String(item.linkId||""));
    inventoryPendingLastAtV250=Date.now();renderInventoryPendingGlobalV250();
    // Force exact card cache refresh next time so card-level status also becomes confirmed.
    if(typeof clearSalesCardPersistentCacheV232==="function")clearSalesCardPersistentCacheV232(item.type,item.date,item.location);
    if(typeof salesProductLinksCacheV216!=="undefined")salesProductLinksCacheV216.delete(salesProductLinksCacheKeyV216(item.type,item.date,item.location));
    setSync("库存处理状态已确认",true);
  }catch(e){if(btn){btn.disabled=false;btn.textContent="✓ 我已处理库存"}alert("库存状态确认失败："+(e.message||e))}
}
window.confirmPendingInventoryV250=confirmPendingInventoryV250;

async function openPendingInventorySalesCardV250(raw){
  const item=typeof raw==="string"?JSON.parse(raw):raw;if(!item)return;
  const type=item.type;
  const nav=document.querySelector(`.nav-item[data-page="${type}"]`);
  if(nav)showPage(type,nav);
...
showPage=function(name,el){
  ...
  if(name==="live"||name==="fair")setTimeout(()=>refreshInventoryPendingV250(true),50);
  return result;
};
