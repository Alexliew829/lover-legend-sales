...
let importCanonicalProductsV281=[];
let importCanonicalProductsPromiseV281=null;

function canonicalImportProductKeyV281(value){
  return normalizeImportProductSearchTextV214(String(value||""));
}

function mergeCanonicalAverageCostV281(records,canonicalProducts){
  const list=Array.isArray(records)?records:[];
  const products=Array.isArray(canonicalProducts)?canonicalProducts:[];
  const byId=new Map();
  const byName=new Map();

  products.forEach(product=>{
    const id=String(product?.productId||product?.id||"").trim();
    const name=canonicalImportProductKeyV281(product?.productName||product?.name||"");
  ...
  return list.map(record=>{
    ...
    const name=canonicalImportProductKeyV281(record?.productName||"");
    const canonical=(id&&byId.get(id))||byName.get(name)||null;
    if(!canonical)return record;

    const averageCost=parseCloudNumberV214(canonical?.averageCost);
    return{
      ...record,
      // V28.1: one source of truth only. Never recompute Sales average cost.
      averageCost:Number.isFinite(averageCost)&&averageCost>=0?averageCost:0,
      stock:canonical?.stock===undefined||canonical?.stock===null
        ? record?.stock
        : parseCloudNumberV214(canonical.stock)
    };
  });
}

async function loadCanonicalImportProductsV281(force=false){
  if(importCanonicalProductsV281.length&&!force)return importCanonicalProductsV281;
  if(importCanonicalProductsPromiseV281)return importCanonicalProductsPromiseV281;

  importCanonicalProductsPromiseV281=(async()=>{
    const result=await jsonp({action:"getImportProducts"},{timeoutMs:15000});
    if(!result?.ok||!Array.isArray(result.products)){
      throw new Error(result?.error||"无法读取 Import Products.averageCost");
    }
    importCanonicalProductsV281=result.products;
    return importCanonicalProductsV281;
  })().finally(()=>{importCanonicalProductsPromiseV281=null;});

  return importCanonicalProductsPromiseV281;
}

async function refreshLinkedAverageCostsV281(list){
  const links=Array.isArray(list)?list:[];
  if(!links.length)return links;
  try{
    const canonical=await loadCanonicalImportProductsV281(false);
    return mergeCanonicalAverageCostV281(links,canonical);
  }catch(e){
    console.warn("V28.1 Products.averageCost refresh skipped",e);
    return links;
  }
}
...
async function loadImportProductsV214(force=false){
  ...
  // V28.1: cached search records may contain an old/recomputed average cost.
  // Before returning any search result, overwrite averageCost from current Import Products.
  if(importProductsV214.length&&!force){
    try{
      const canonical=await loadCanonicalImportProductsV281(false);
      importProductsV214=mergeCanonicalAverageCostV281(importProductsV214,canonical);
      saveImportProductsCacheV224(importProductsV214);
    }catch(e){
      console.warn("V28.1 canonical averageCost read failed; using last available cache",e);
    }
    return importProductsV214;
  }
  ...
  importProductsPromiseV214=(async()=>{
    ...
    // Keep original price / inland misc / shipping / Batch mapping from Import cloud,
    // but averageCost is replaced by Products.averageCost from the canonical Products sheet.
    let records=buildCloudImportProductRecordsV214(data);
    const canonical=await loadCanonicalImportProductsV281(true);
    records=mergeCanonicalAverageCostV281(records,canonical);

    importProductsV214=records;
    saveImportProductsCacheV224(importProductsV214);
    return importProductsV214;
...
async function loadProductLinksIntoEditorV206(type){
  ...
  if(Array.isArray(cached)){
    const refreshedCached=await refreshLinkedAverageCostsV281(cached);
    if(!productImportSearchIsActiveV226(type))renderProductLinksEditorV206(type,refreshedCached);
  }else{
  ...
  try{
    ...
    if(
      ...
      const refreshedLinks=await refreshLinkedAverageCostsV281(links);
      const before=JSON.stringify(Array.isArray(cached)?await refreshLinkedAverageCostsV281(cached):[]);
      const after=JSON.stringify(Array.isArray(refreshedLinks)?refreshedLinks:[]);
      if(!Array.isArray(cached)||before!==after){
        renderProductLinksEditorV206(type,refreshedLinks);
      }
    }
  }catch(e){
    const current=productLinkContextV206(type);
