...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4580",restoreGeneration:0};
...
// V45.8: paint Home immediately from Local First cache, then perform one
...
// V45.8: startup performs only Local First + the lightweight global Revision
...
function buildCloudImportProductRecordsV214(data){
  ...
  function addRecord(source){
    ...
    records.push({
      ...
      // V45.8: prefer the current Products ID. Old cached/import PZ IDs still
...
// V45.8: Sales Card freshness is controlled by the global Revision gate.
...
// V45.8: a completed sale is a whole-card, permanent local fact. Keep it in
...
// V45.8: an authoritative cloud context may legitimately become empty after a
...
async function loadProductLinksIntoEditorV206(type){
  ...
  // V45.8 Local First + exact-context verification. Paint the last complete
  ...
  if(Array.isArray(cached)){
    ...
    // V45.8: when cloud revision advanced, an old durable pending draft must not
    ...
    try{
      ...
      // V45.8: exact-context refresh commits card + profit together from the SAME
...
function salesCardWriteReadyV449(){
  // V45.8: do not freeze unrelated work while cloud sync is slow. Existing
...
系统会保留 saleId 的删除记录，让 Import Cost System 知道这张销售已经取消。`))return;
  ...
  try{
    if(saved.length){
      setSync('删除中 · 正在写入云端…');
      await deleteSalesTransactionV256(txnId);
      // V45.8: verify only the affected context. Card + profit share this exact
      // authority; unrelated Sales/Fair/Live contexts are never blocked.
      const ctx=productLinkContextV206(type);
      const fresh=await loadSalesProductLinksV206(type,ctx.date,ctx.location,{force:true,maxAgeMs:0});
      if((fresh||[]).some(x=>String(x.transactionId||'')===String(txnId||'')))throw new Error('删除后云端仍返回这张销售卡，请重试');
      if(typeof replaceProfitAggregateContextV457==='function')replaceProfitAggregateContextV457(type,ctx.date,ctx.location,fresh||[]);
      else if(typeof mergeDailyProfitContextCacheV237==='function')mergeDailyProfitContextCacheV237(type,ctx.date,ctx.location,fresh||[]);
      const profitPanelV458=typeof productProfitSummaryPanelV216==='function'?productProfitSummaryPanelV216(type):null;
      if((productProfitSummaryOpenV216?.[type]||(profitPanelV458&&!profitPanelV458.classList.contains('hidden')))&&typeof renderProductProfitSummaryV216==='function'){
        const dayV458=typeof getDailyProfitCacheV237==='function'?(getDailyProfitCacheV237(type,ctx.date)||[]):(fresh||[]);
        renderProductProfitSummaryV216(type,dayV458);
      }
    }else{
      card.remove();
    ...
    setSync("已同步",true);
    alert('删除成功');
  }catch(e){setSync('删除失败 · 请重试',false,true);alert("删除失败："+(e.message||e))}
...
function addProductLinkItemV209(type,data={}){
  ...
  // V45.8: New Sales Card is local-only and instant. If a first-time cloud
...
saveProductLinksV206=async function(type,saveMode='confirm',button=null){
  ...
  if(saveMode==='draft'){
    try{
      // V45.8: local safety copy is still written first, but “保存成功” is not
      ...
      // V45.8: action result and sync state are separate. The alert reports the
...
function renderInventoryPendingGlobalV250(){
  ["daily","live","fair"].forEach(pageType=>{
    ...
    const list=inventoryPendingCacheV250.filter(item=>{
      // V45.8: a draft card must never flash an Import-inventory reminder.
...
/* ================= V45.8 field drag/drop protection =================
...
/* ================= V45.8 once-per-day unconfirmed draft reminder =================
...
async function checkUnconfirmedDraftReminderV369(){
  ...
  try{
    ...
    // V45.8: only the explicit X button dismisses today's reminder. Leaving,
...
async function addTurnoverEntryV376(type){
  ...
  // V45.8: after an Apps Script timeout the optimistic entry intentionally
...
saveProductLinksV206=async function(type,...args){
  // V45.8: do not freeze unrelated contexts while the global background probe is slow.
...
renderAll=function(){
  ...
  try{renderTurnoverComposerV376('daily');renderTurnoverComposerV376('fair');renderTurnoverComposerV376('live')}catch(e){console.warn('V45.8 post-render turnover',e)}
  return result;
};
...
/* ================= V45.8 transaction UX + global drag/drop safety ================= */
...
// V45.8: mobile OS/app switching cannot be cancelled by a web page. If the app is
...
/* ================= V45.8 system status + system information ================= */
...
function renderSystemInformationV442(data){
  ...
  set('systemInfoApiV442',data?.apiVersion?`V${String(data.apiVersion).replace(/^V/i,'').replace(/^4580$/,'45.8')}`:'连接异常');
...
async function runSystemHealthCheckV442(userTriggered=false){
  ...
  try{
    const data=await jsonp({action:'healthV443',clientVersion:'4580'},{timeoutMs:15000});
    if(!data?.ok)throw new Error(data?.message||'系统检查失败');
    const issues=[];
    if(String(data.apiVersion||'')!=='4580')issues.push(`Frontend / API 版本不一致（Frontend 4580 / API ${data.apiVersion||'未知'}）`);
    (Array.isArray(data.issues)?data.issues:[]).forEach(x=>issues.push(String(x)));
    const severe=Boolean(data.severe)||!data.sheetConnected||String(data.apiVersion||'')!=='4580';
