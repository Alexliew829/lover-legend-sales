...
function showPage(name,el){
  ...
  // V30.3: page "sales" uses the sales-card type "daily".
  ...
  // V30.3: every time Live is opened, start from today's date.
  ...
  // V30.3: page switching never waits for or triggers cloud sync.
...
// V30.3: Top 5 business performance. Uses rows already loaded in memory only;
...
function readHistoricalHighsCache(){
  ...
  // V30.3: a changed/deleted turnover invalidates the old historical-high cache immediately.
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V30.3: historical record is lazy. Top 5 opens instantly from local rows;
...
function renderTodayCompanyStatus(){
  ...
  // V30.3: status colour represents actual business amount, not merely
...
function applyCloudCommissionSettings(settings){
  ...
  // V30.3: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V30.3: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V30.3: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V30.3: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V30.3: local Fair values are direct replacements, never additions. The server
...
// V30.3: paint Home immediately, restore local cache, then perform only a
...
// V30.3 cache-first startup: show local data immediately and perform only the
...
function collectLiveHosts(){
  ...
  // V30.3: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V30.3: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V30.3 Import Cost System product search ================= */
...
function unorderedImportProductMatchV214(sourceValue,queryValue){
  ...
  if(isExactProductCodeQueryV214(query)){
    // V30.3: code search supports case-insensitive prefix/partial entry.
...
function setupImportProductSearchV214(item,nameInput,resultsBox,closeButton){
  // V30.3: iPhone/iOS safe search state.
  ...
  const render=async()=>{
    ...
    try{
      ...
      matches.forEach(record=>{
        ...
        // V30.3: distinguish an intentional tap from list scrolling on iPhone/iOS.
  ...
  if(closeButton){
    ...
    closeButton.addEventListener("click",e=>{
      ...
      // V30.3: X means CLEAR CURRENT PRODUCT SELECTION.
...
/* ================= V30.3 Sales Card unsaved-change protection ================= */
...
function hasUnsavedSalesCardChangesV238(type){
  ...
  // V30.3: use the same transaction-level dirty signal that Live already maintains.
...
/* ================= V30.3 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V30.3: Sales Cards and Daily Profit are mutually exclusive.
    ...
    // V30.3: saved cards first. Do not flash a fake 0.00 card while cloud data loads.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V30.3 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V30.3 button bindings reliable.
...
/* ================= V30.3 multi-product Sales Card ================= */
...
function buildProductSubItemV239(type,card,data={},order=1){
  ...
  [c,p].forEach(el=>el.addEventListener("blur",()=>{
    ...
    // V30.3: blur fires before the Save button click. Re-mark the card dirty here so
...
function bindSingleTapDeleteProductV254(button,handler){
  ...
  // V30.3: for touch/pen, pointerup is the one and only real activation.
...
系统会保留 saleId 的删除记录，让 Import Cost System 知道这张销售已经取消。`))return;
  try{
    ...
    // V30.3: refresh profit view from active backend links after card deletion.
    try{
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  // V30.3: Sales only reminds. Inventory confirmation is handled in Import Cost System.
  ...
  setTimeout(()=>{
    // V30.3: initialization may finish after a very fast first edit.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V30.3 sales card container missing",type);return false}
...
/* ================= V30.3 Sales Card unit-price + stable optimistic UI ================= */
...
async function saveProductLinksV206(type){
  ...
  // V30.3 optimistic local cache: keep the current UI stable; do not redraw from an older async response.
...
/* ================= V30.3 same-day linked bonsai profit summary ================= */
...
async function toggleProductProfitSummaryV216(type,button){
  ...
  try{
    // V30.3: opening Daily Profit closes Sales Cards first, then paints local cache immediately.
    const pre=productLinkPreV208(type),box=document.getElementById(pre+"ProductLinkBox"),body=document.getElementById(pre+"ProductLinkBody");
...
/* ================= V30.3 expandable yearly monthly breakdown + profit ================= */
...
function renderAll(){
  // V30.3: one complete render path. This replaces the older partial duplicate
...
// V30.3: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V30.3: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V30.3: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
/* ================= V30.3 pre-change sales-card context guard ================= */
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V30.3: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V30.3: deleting the last special commission rule leaves candidate and
...
/* ================= V30.3 Sales Card transaction integrity / instant cache ================= */
...
/* ================= V30.3 Live optimistic revision guard ================= */
...
async function refreshOpenSalesChangeLogAfterSaveV243(type,date){
  ...
  }catch(e){console.warn("V30.3 修改/销售记录后台刷新失败",e)}
}
...
saveLiveSales=async function(){
  ...
  // V30.3 preflight: do not even mutate local turnover when saved cards exceed the new amount.
  try{
  ...
  // Critical V30.3 guard: any cloud request started before this local mutation
...
/* ================= V30.3 Profit list copy + compact totals ================= */
...
/* ================= V30.3 profit layout + weighted average margin ================= */
...
/* ================= V30.3 cross-date inventory reminders =================
...
function openImportCostSystemV266(target={}){
  // V30.3: Sales must stay open; Import always opens in a separate tab/window.
...
// V30.3: Sales-side inventory confirmation removed. Import is the only inventory authority.
...
// V30.3: after the user processes inventory in the separate Import tab/app,
...
renderFairMonthlyList=function(){
  ...
  }).catch(e=>console.warn('V30.3 Fair 利润读取失败',e));
};
...
renderLiveMonthlyList=function(){
  ...
  }).catch(e=>console.warn('V30.3 Live 利润读取失败',e));
};
