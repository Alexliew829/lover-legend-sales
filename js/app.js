...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4090",restoreGeneration:0};
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  // V40.9: confirmation belongs to the sales-card transaction, not only to the
  ...
  // V40.9: this is the active Sales / Fair / Live card builder. Keep each
...
// V40.9: the persisted Restore job remains authoritative through the final
...
function markDraftSavedLocallyV314(type,ctx,dirty,items,dirtyIds){
  ...
  const local=items.map(x=>{
    ...
    let status='DRAFT';
    if(wasConfirmed){
      // V40.9: preserve each already-confirmed row, but a NEW product added to an
...
async function validateSalesInventoryAvailabilityV325(type,items,saveMode='draft'){
  ...
  // V40.9: keep the same authoritative Import oversell guard, but avoid the old
...
saveProductLinksV206=async function(type,saveMode='confirm',button=null){
  ...
  // V40.9 protects navigation from the instant Confirm is pressed.
  ...
  // V40.9: a draft is local/cloud sales-card data only. Import availability is
...
// V40.9: the pending endpoint is the smallest authoritative ACK check. When a
...
// V40.9: do not wait for the full Sales cloud-resume cycle. Import ACK is a
...
// V40.9 final navigation gate, installed after every legacy showPage wrapper.
