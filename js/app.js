...
function showPage(name,el){
  ...
  // V19.4: every time Live is opened, start from today's date.
  ...
  // V19.4: page switching never waits for or triggers cloud sync.
...
// V19.4: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V19.4: historical record is lazy. Top 5 opens instantly from local rows;
...
function applyCloudCommissionSettings(settings){
  ...
  // V19.4: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V19.4: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V19.4: timeout must not undo the user's local action.
...
const companyMonthDetailOpenV186=JSON.parse(localStorage.getItem("companyMonthDetailOpenV191")||"{\"balakong\":false,\"belimbing\":false}");

function companyMonthDetailRowsV186(company){
  const month=selectedMonth();
  return dedupeRows(rows)
    .filter(r=>r.type==="daily"&&r.company===company)
    .filter(r=>displayToISO(r.date).slice(0,7)===month)
    .filter(r=>Number(r.amount||0)>0)
    .sort((a,b)=>displayToISO(b.date).localeCompare(displayToISO(a.date)));
}

function renderCompanyMonthDetailsV186(company){
  const panel=document.getElementById(company+"MonthDetails");
  const list=document.getElementById(company+"MonthDetailsList");
  const totalEl=document.getElementById(company+"MonthDetailsTotal");
  const monthEl=document.getElementById(company+"MonthDetailsMonth");
  ...
  const open=Boolean(companyMonthDetailOpenV186[company]);
  ...
  const month=selectedMonth();
  const companyRows=companyMonthDetailRowsV186(company);
  const total=companyRows.reduce((sum,r)=>sum+Number(r.amount||0),0);
  totalEl.textContent="RM"+money(total);

  if(monthEl){
    const parts=String(month||"").split("-");
    monthEl.textContent=parts.length===2?`${parts[1]}-${parts[0]}`:month;
  }

  list.innerHTML=companyRows.length
    ?companyRows.map(r=>`
      <div class="company-month-detail-row">
        <span>${r.date}</span>
        <strong>RM${money(r.amount)}</strong>
      </div>
    `).join("")
...
function renderAllCompanyMonthDetailsV186(){
  renderCompanyMonthDetailsV186("balakong");
  renderCompanyMonthDetailsV186("belimbing");
}

function toggleCompanyMonthDetails(company){
  if(company!=="balakong"&&company!=="belimbing")return;
  companyMonthDetailOpenV186[company]=!companyMonthDetailOpenV186[company];localStorage.setItem("companyMonthDetailOpenV191",JSON.stringify(companyMonthDetailOpenV186));
  renderCompanyMonthDetailsV186(company);
}
...
function renderDashboard(){const bt=totalBy("daily","balakong","today"),blt=totalBy("daily","belimbing","today"),ft=totalBy("fair","","today"),bm=totalBy("daily","balakong","month"),blm=totalBy("daily","belimbing","month"),fm=totalBy("fair","","month"),by=totalBy("daily","balakong","year"),bly=totalBy("daily","belimbing","year"),fy=totalBy("fair","","year");document.getElementById("balakongMonth").textContent=money(bm);document.getElementById("belimbingMonth").textContent=money(blm);renderFairLocationList();document.getElementById("fairMonthTotal").textContent=money(fm);renderFairCommission(fm);document.getElementById("monthGrandTotal").textContent=money(bm+blm+fm);document.getElementById("balakongYearTotal").textContent=money(by);document.getElementById("belimbingYearTotal").textContent=money(bly);document.getElementById("fairYearTotal").textContent=money(fy);document.getElementById("yearGrandTotal").textContent=money(by+bly+fy);renderTodayCompanyStatus();renderAllCompanyMonthDetailsV186()}
...
async function saveDailySales(){
  ...
  // V19.4 mobile-safe save: return control immediately after the local durable
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V19.4: local Fair values are direct replacements, never additions. The server
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"1801"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=Array.isArray(state.closedMonths)?state.closedMonths:[];systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"1801"}updateReadOnlyMode()}
...
// V19.4: paint Home immediately, restore local cache, then perform only a
...
// V19.4: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V19.4: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V19.4: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
function renderAll(){
  // V19.4: one complete render path. This replaces the older partial duplicate
...
// V19.4: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V19.4: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V19.4: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V19.4: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V19.4: deleting the last special commission rule leaves candidate and
