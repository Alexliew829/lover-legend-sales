...
function showPage(name,el){
  ...
  // V15.6: every time Live is opened, start from today's date.
  ...
  // V15.6: page switching never waits for or triggers cloud sync.
...
function applyCloudCommissionSettings(settings){
  ...
  // V15.6: an older cloud response may arrive after a newer local delete/edit.
...
function getCommissionSettingsForMonth(month){
  ...
  // V15.6: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V15.6: timeout must not undo the user's local action.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V15.6: local Fair values are direct replacements, never additions. The server
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"15.6"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=Array.isArray(state.closedMonths)?state.closedMonths:[];systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"15.6"}updateReadOnlyMode()}
...
// V15.6: paint Home immediately, restore local cache, then perform only a
...
// V15.6: password/access gate removed; start cached Home + revision check immediately.
...
function collectLiveHosts(){
  ...
  // V15.6: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V15.6: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
function renderAll(){
  // V15.6: one complete render path. This replaces the older partial duplicate
...
async function toggleMonthlySummary(force){
  ...
  // V15.6: show cached figures immediately, then fetch the full year only on demand.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V15.6：单击后立即套用，并由所有已开启装置自动读取最新佣金。
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V15.6: deleting the last special commission rule leaves candidate and
...
async function resetFairCommissionSettings(){
  ...
  try{
    // V15.6：确认恢复后先立即更新本机与 Home，再在后台同步 Google Sheet。
    applyCommissionSettings(settings);
    if((systemState.closedMonths||[]).includes(selectedMonth())){
