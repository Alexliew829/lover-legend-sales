...
function showPage(name,el){
  ...
  // V16.4: every time Live is opened, start from today's date.
  ...
  // V16.4: page switching never waits for or triggers cloud sync.
...
function normalizeCommissionSettings(settings){
  ...
  if(![rate1,rate2,rate3].every(Number.isFinite)||rate1<0||rate2<0||rate3<0){
    return{...DEFAULT_COMMISSION_SETTINGS,liveHostRates:{},liveHosts:{},inactiveLiveHosts:{},liveRateSchedules:[],liveRevision};
  }
  return{rate1,rate2,rate3,liveHostRates,liveHosts,inactiveLiveHosts,liveRateSchedules,liveRevision};
}
...
function applyCloudCommissionSettings(settings){
  ...
  // V16.4: an older cloud response may arrive after a newer local delete/edit.
  ...
  if(liveCommissionDraftDirty||incomingRevision<localRevision){
    applyCommissionSettings({
      ...incoming,
      liveHostRates:local.liveHostRates,
      liveHosts:local.liveHosts,
      inactiveLiveHosts:local.inactiveLiveHosts,
      liveRateSchedules:local.liveRateSchedules,
      liveRevision:localRevision
    });
    if(!fairCommissionDraftDirty)savedFairCommissionSnapshot=fairCommissionComparable(incoming);
    return;
  }
  applyCommissionSettings(incoming);
  setSavedCommissionSnapshots(incoming);
...
function getCommissionSettingsForMonth(month){
  ...
  // V16.4: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V16.4: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V16.4 mobile-safe save: return control immediately after the local durable
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V16.4: local Fair values are direct replacements, never additions. The server
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"16.4"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=Array.isArray(state.closedMonths)?state.closedMonths:[];systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"16.4"}updateReadOnlyMode()}
...
// V16.4: paint Home immediately, restore local cache, then perform only a
...
// V16.4: password/access gate removed; start cached Home + revision check immediately.
...
function collectLiveHosts(){
  ...
  // V16.4: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V16.4: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
function renderAll(){
  // V16.4: one complete render path. This replaces the older partial duplicate
...
// V16.4: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  renderDailyTotals();
  if(typeof loadMonthCloudShared!=="function"||typeof mergeCloudMonthRows!=="function")return;
  ...
  try{
    ...
    if(status)status.textContent="云端暂时无法读取，先显示本机已有资料";
  }
}
...
function renderMonthlySummary(){
  ...
  let chosen=[];
  const currentItem=sorted.find(x=>x.month===current);
  if(currentItem)chosen.push(currentItem);
  for(const item of sorted){if(chosen.length>=2)break;if(!chosen.some(x=>x.month===item.month))chosen.push(item);}
  const monthLabel=m=>m.slice(5,7)+"-"+m.slice(0,4);
  const value=(item,key)=>item?`RM${money(item[key]||0)}`:"-";
  const head=chosen.map(item=>`<button type="button" class="monthly-month-head${item.month===current?" active":""}" onclick="selectSummaryMonth('${item.month}')">${monthLabel(item.month)}</button>`).join("");
  const row=(label,key)=>`<div class="monthly-grid-label">${label}</div>${chosen.map(item=>`<div class="monthly-grid-value">${value(item,key)}</div>`).join("")}`;
  body.innerHTML=`<div class="monthly-grid-head-label">月份</div>${head}${row("Balakong","balakong")}${row("Belimbing","belimbing")}${row("Fair","fair")}`;
}
async function toggleMonthlySummary(force){
  ...
  // V16.4: show cached figures immediately, then fetch the full year only on demand.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    const unchanged=Number(previous.rate1)===Number(settings.rate1)&&Number(previous.rate2)===Number(settings.rate2)&&Number(previous.rate3)===Number(settings.rate3);
    if(unchanged){
    ...
    // V16.4：单击后立即套用，并由所有已开启装置自动读取最新佣金。
    ...
    renderDashboard();
    renderTable();
    setSavedCommissionSnapshots(saved||settings,{fair:true,live:false});
    updateFairCommissionDraftState();
    setSync("已同步",true);
  }catch(e){
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V16.4: deleting the last special commission rule leaves candidate and
...
async function resetFairCommissionSettings(){
  ...
  try{
    // V16.4：确认恢复后先立即更新本机与 Home，再在后台同步 Google Sheet。
    applyCommissionSettings(settings);
    if((systemState.closedMonths||[]).includes(selectedMonth())){
...
function backupAllData(){const payload=getBackupPayload();const stamp=new Date().toISOString().replace(/[:T]/g,"-").slice(0,19);downloadFile(`Lover_Legend_Sales_V16_0_Backup_${stamp}.json`,JSON.stringify(payload,null,2),"application/json;charset=utf-8;")}
