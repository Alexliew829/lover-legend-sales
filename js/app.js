...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4990",restoreGeneration:0};
...
function getVndPotMossFeeV267(record){
  ...
  // V49.9: match Import V21.5 VND pot/misc tiers exactly.
...
function syncSalesCardAutoExtraV267(card){
  ...
  [...card.querySelectorAll(".product-link-item")].forEach(item=>{
    ...
    // V49.9: Import V21.5 VND pot/misc fee is a per-unit selling cost.
...
function liveDeliveryDefaultV203(price){
  ...
  if(n<=0)return 0;
  // V49.9: match Import V21.5 木架＋本地运费 A–F exactly.
  if(n<300)return 20;
  if(n<=500)return 50;
  if(n<=1000)return 80;
...
async function loadProductLinksIntoEditorV206(type){
  ...
  // V49.9: keep the V49.9 Local First sync architecture.
  ...
  // are already handled by the V49.9 shared priorityRevisionV456 change journal,
  ...
  try{
    ...
    if(SALES_CARD_LOAD_SEQ_V245[type]===seq&&currentKey===contextKey&&!productImportSearchIsActiveV226(type)){
      ...
      if(SALES_CARD_VERIFY_PENDING_V445[type]===contextKey)SALES_CARD_VERIFY_PENDING_V445[type]='';
      // V49.9: an explicit Sales Card view does not declare the whole context synced.
    }
    return links;
  }catch(e){
    const current=productLinkContextV206(type);
    ...
    if(SALES_CARD_LOAD_SEQ_V245[type]===seq&&currentKey===contextKey&&!productImportSearchIsActiveV226(type)){
      ...
      // V49.9: card-view read failure stays inside the card panel; global sync is unchanged.
    }
    return null;
  }
}
...
function salesCardAutoDeliveryForItemV239(item){
  const card=item?.closest?.(".sales-card-transaction-v239");
  // V49.9: only Live auto-generates 木架＋本地运费. Sales / Fair stay manual.
...
function productAutoDeliveryV240(item){
  const card=item?.closest?.(".sales-card-transaction-v239");
  // V49.9: Sales / Fair never auto-generate delivery; manual input is preserved.
  if(String(card?.dataset?.type||"").toLowerCase()!=="live")return 0;
...
function buildProductSubItemV239(type,card,data={},order=1){
  ...
  if(type==="live"){
    ...
    // V49.9: only Live auto-selects the Import V21.5 crate/local-delivery tier
...
function filterTurnoverAuditLogsV380(type,location,logs){
  ...
  // V49.9: Sales (Balakong / Belimbing) uses the same auxiliary audit fallback
...
// V49.9: the authoritative turnover total may arrive a few seconds before the
...
// V49.9: keep V48.3/V46.6 main sync untouched. After an authoritative turnover
...
renderAll=function(){
  ...
  }catch(e){console.warn('V49.9 post-render turnover',e)}
  return result;
};
...
// V49.9: the read-only health endpoint is advisory only. A slow health scan must
...
function renderSystemInformationV442(data){
  ...
  set('systemInfoApiV442',data?.apiVersion?`V${String(data.apiVersion).replace(/^V/i,'').replace(/^4990$/,'48.8')}`:'连接异常');
...
async function runSystemHealthCheckV442(userTriggered=false){
  ...
  try{
    const data=await jsonp({action:'healthV443',clientVersion:'4990'},{timeoutMs:15000});
    if(!data?.ok)throw new Error(data?.message||'系统检查失败');
    const issues=[];
    if(String(data.apiVersion||'')!=='4990')issues.push(`Frontend / API 版本不一致（Frontend 4990 / API ${data.apiVersion||'未知'}）`);
    (Array.isArray(data.issues)?data.issues:[]).forEach(x=>issues.push(String(x)));
    const severe=Boolean(data.severe)||!data.sheetConnected||String(data.apiVersion||'')!=='4990';
...
/* ================= V49.9 minimal Sales Card UX fixes (V49.9 baseline) =================
   Built directly on V49.9. Do not change the V49.9 revision/sync cadence.
...
/* ================= V49.9 persistent profit view cache =================
   Profit is secondary display data. It never blocks or mutates the V48.8/V49.9
...
async function runSilentProfitRefreshV496(reason='background'){
  ...
  silentProfitRefreshPendingV496=(async()=>{
    try{
      ...
      return clean;
    }catch(e){console.warn('V49.9 静默利润读取失败',reason,e);return null}
