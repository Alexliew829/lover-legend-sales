...
// V39.7: turnover writes are real transactions. Keep navigation guarded until cloud confirmation.
...
function renderTodayCompanyStatus(){
  // V39.7: this card follows the explicit "查看日期" selector. Keep the
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"3970",restoreGeneration:0};
...
async function loadProductLinksIntoEditorV206(type){
  ...
  try{
    ...
    // V39.7: a temporary JSONP/network miss must not immediately turn a valid
...
async function validateSalesInventoryAvailabilityV325(type,items,saveMode='draft'){
  ...
  // V39.7: race two independent read-only Import checks instead of waiting for
...
function refreshMonthlyRollupV365(type){
  ...
  // V39.7: profit data is a second cloud source. A transient timeout used to
...
/* ================= V39.7 Import -> associated Sales card deep link =================
...
async function openAssociatedSalesCardFromImportV370(){
  ...
  try{await loadProductLinksIntoEditorV206(type)}catch(e){console.warn('V39.7 associated card load failed',e)}
...
/* ================= V39.7 Fair/Live incremental turnover composer =================
...
async function refreshTurnoverAuditDerivedV380(type,ctx){
  ...
  try{
    ...
    return derived;
  }catch(e){console.warn('V39.7 turnover audit reconstruction',e);return[]}
}
...
async function refreshTurnoverEntriesV376(type,{force=false}={}){
  ...
  }catch(e){console.warn('V39.7 turnover entry read',e);try{const derived=await refreshTurnoverAuditDerivedV380(type,ctx);if(derived.length)setTurnoverEntryCacheV376(type,ctx.date,ctx.location,derived,'audit-v380')}catch(_){}}
...
async function saveTurnoverTotalV376(type,total,notificationMeta={}){
  ...
  try{
    ...
    // V39.7: a browser timeout is not proof that the write failed. Read the
...
async function commitTurnoverEntriesV376(type,entries,actionText,notificationMeta={}){
  ...
  // V39.7: the authoritative total write is the user-facing completion point.
...
async function addTurnoverEntryV376(type){
  ...
  // V39.7: show the new component/total immediately while the authoritative cloud write runs.
...
// V39.7 save-card handoff: immediately repaint the selected day's profit from the saved result/cache.
...
saveProductLinksV206=async function(type,...args){
  ...
  }catch(e){console.warn('V39.7 immediate profit repaint',e)}return result;
};window.saveProductLinksV206=saveProductLinksV206;

// V39.7 deep-link fast path: handle target as soon as scripts are ready instead of waiting for full load + 500ms.
...
async function retryTurnoverEntryPendingV376(){
  ...
  for(const [key,item] of Object.entries(pending)){
    try{
      ...
      else console.warn('V39.7 turnover detail retry',e);
    }
  }
...
/* ================= V39.7 turnover UI + legacy-date compatibility =================
...
function refreshTurnoverContextV377(type,{cloud=true}={}){
  try{renderTurnoverComposerV376(type)}catch(e){console.warn('V39.7 turnover render',e)}
  if(cloud){
    setTimeout(()=>{try{refreshTurnoverEntriesV376(type,{force:true})}catch(e){console.warn('V39.7 turnover cloud refresh',e)}},0);
  }
}
...
// Earlier date controls captured the old update function before V39.7 replaced it.
...
renderAll=function(){
  ...
  try{renderTurnoverComposerV376('fair');renderTurnoverComposerV376('live')}catch(e){console.warn('V39.7 post-render turnover',e)}
  return result;
};
