...
function showPage(name,el){
  ...
  // V26.7: every time Live is opened, start from today's date.
  ...
  // V26.7: page switching never waits for or triggers cloud sync.
...
// V26.7: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V26.7: historical record is lazy. Top 5 opens instantly from local rows;
...
function renderTodayCompanyStatus(){
  ...
  // V26.7: status colour represents actual business amount, not merely
...
function applyCloudCommissionSettings(settings){
  ...
  // V26.7: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V26.7: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V26.7: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V26.7: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V26.7: local Fair values are direct replacements, never additions. The server
...
// V26.7: paint Home immediately, restore local cache, then perform only a
...
// V26.7: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V26.7: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V26.7: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V26.7 Import Cost System product search ================= */
...
function unorderedImportProductMatchV214(sourceValue,queryValue){
  ...
  if(isExactProductCodeQueryV214(query)){
    // V26.7: code search supports case-insensitive prefix/partial entry.
...
function setupImportProductSearchV214(item,nameInput,resultsBox,closeButton){
  // V26.7: iPhone/iOS safe search state.
  ...
  const render=async()=>{
    ...
    try{
      ...
      matches.forEach(record=>{
        ...
        // V26.7: distinguish an intentional tap from list scrolling on iPhone/iOS.
  ...
  if(closeButton){
    ...
    closeButton.addEventListener("click",e=>{
      ...
      // V26.7: X means CLEAR CURRENT PRODUCT SELECTION.
...
/* ================= V26.7 Sales Card unsaved-change protection ================= */
...
/* ================= V26.7 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V26.7: Sales Cards and Daily Profit are mutually exclusive.
    ...
    // V26.7: saved cards first. Do not flash a fake 0.00 card while cloud data loads.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V26.7 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V26.7 button bindings reliable.
...
/* ================= V26.7 multi-product Sales Card ================= */
...
function buildProductSubItemV239(type,card,data={},order=1){
  ...
  [c,p].forEach(el=>el.addEventListener("blur",()=>{
    ...
    // V26.7: blur fires before the Save button click. Re-mark the card dirty here so
...
function bindSingleTapDeleteProductV254(button,handler){
  ...
  // V26.7: for touch/pen, pointerup is the one and only real activation.
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  // V26.7: Sales only reminds. Inventory confirmation is handled in Import Cost System.
  ...
  setTimeout(()=>{
    // V26.7: initialization may finish after a very fast first edit.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V26.7 sales card container missing",type);return false}
...
/* ================= V26.7 Sales Card unit-price + stable optimistic UI ================= */
...
async function saveProductLinksV206(type){
  ...
  // V26.7 optimistic local cache: keep the current UI stable; do not redraw from an older async response.
...
/* ================= V26.7 same-day linked bonsai profit summary ================= */
...
async function toggleProductProfitSummaryV216(type,button){
  ...
  try{
    // V26.7: opening Daily Profit closes Sales Cards first, then paints local cache immediately.
    const pre=productLinkPreV208(type),box=document.getElementById(pre+"ProductLinkBox"),body=document.getElementById(pre+"ProductLinkBody");
...
/* ================= V26.7 expandable yearly monthly breakdown ================= */
...
function yearBreakdownRowsV224(kind){
  // V26.7: Balakong / Belimbing / Fair / Live show months inside the selected
...
function renderAll(){
  // V26.7: one complete render path. This replaces the older partial duplicate
...
// V26.7: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V26.7: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V26.7: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V26.7: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V26.7: deleting the last special commission rule leaves candidate and
...
/* ================= V26.7 Sales Card transaction integrity / instant cache ================= */
...
/* ================= V26.7 Live optimistic revision guard ================= */
...
async function refreshOpenSalesChangeLogAfterSaveV243(type,date){
  ...
  }catch(e){console.warn("V26.7 修改/销售记录后台刷新失败",e)}
}
...
saveLiveSales=async function(){
  ...
  // Critical V26.7 guard: any cloud request started before this local mutation
...
/* ================= V26.7 Profit list copy + compact totals ================= */
...
/* ================= V26.7 profit layout + weighted average margin ================= */
...
/* ================= V26.7 cross-date inventory reminders =================
...
function openImportCostSystemV266(target={}){
  // V26.7: Sales must stay open; Import always opens in a separate tab/window.
...
// V26.7: Sales-side inventory confirmation removed. Import is the only inventory authority.
...
// V26.7: after the user processes inventory in the separate Import tab/app,
