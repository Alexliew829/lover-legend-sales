...
function showPage(name,el){
  ...
  // V27.1: every time Live is opened, start from today's date.
  ...
  // V27.1: page switching never waits for or triggers cloud sync.
...
// V27.1: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V27.1: historical record is lazy. Top 5 opens instantly from local rows;
...
function renderTodayCompanyStatus(){
  ...
  // V27.1: status colour represents actual business amount, not merely
...
function applyCloudCommissionSettings(settings){
  ...
  // V27.1: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V27.1: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V27.1: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V27.1: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V27.1: local Fair values are direct replacements, never additions. The server
...
// V27.1: paint Home immediately, restore local cache, then perform only a
...
// V27.1: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V27.1: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V27.1: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V27.1 Import Cost System product search ================= */
...
function unorderedImportProductMatchV214(sourceValue,queryValue){
  ...
  if(isExactProductCodeQueryV214(query)){
    // V27.1: code search supports case-insensitive prefix/partial entry.
...
function autoPotMossFeeV271(record){
...
function syncCardAutoPotMossV271(card){
  if(!card||card.dataset.extraManualV271==="1")return;
  const extra=card.querySelector(".sales-card-extra-total-v239");
  if(!extra)return;
  const total=[...card.querySelectorAll(".product-link-item")]
    .reduce((sum,item)=>sum+Math.max(0,Number(item.dataset.autoPotMossFeeV271||0)),0);
  extra.value=formatAmount(total);
...
function applyImportRecordToProductCardV214(item,nameInput,record){
  ...
  item.dataset.autoPotMossFeeV271=String(autoPotMossFeeV271(record));
  item.dispatchEvent(new CustomEvent("import-product-selected-v271",{bubbles:true}));
...
function clearImportMappingForManualProductV214(item){
  ...
  item.dataset.autoPotMossFeeV271="0";
  item.dispatchEvent(new CustomEvent("import-product-selected-v271",{bubbles:true}));
  updateProductLinkMinimumWarningV214(item);
}
...
function setupImportProductSearchV214(item,nameInput,resultsBox,closeButton){
  // V27.1: iPhone/iOS safe search state.
  ...
  const render=async()=>{
    ...
    try{
      ...
      matches.forEach(record=>{
        ...
        // V27.1: distinguish an intentional tap from list scrolling on iPhone/iOS.
  ...
  if(closeButton){
    ...
    closeButton.addEventListener("click",e=>{
      ...
      // V27.1: X means CLEAR CURRENT PRODUCT SELECTION.
...
/* ================= V27.1 Sales Card unsaved-change protection ================= */
...
/* ================= V27.1 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V27.1: Sales Cards and Daily Profit are mutually exclusive.
    ...
    // V27.1: saved cards first. Do not flash a fake 0.00 card while cloud data loads.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V27.1 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V27.1 button bindings reliable.
...
/* ================= V27.1 multi-product Sales Card ================= */
...
function buildProductSubItemV239(type,card,data={},order=1){
  ...
  [c,p].forEach(el=>el.addEventListener("blur",()=>{
    ...
    // V27.1: blur fires before the Save button click. Re-mark the card dirty here so
...
function bindSingleTapDeleteProductV254(button,handler){
  ...
  // V27.1: for touch/pen, pointerup is the one and only real activation.
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  // V27.1: Sales only reminds. Inventory confirmation is handled in Import Cost System.
  ...
  setTimeout(()=>{
    // V27.1: initialization may finish after a very fast first edit.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V27.1 sales card container missing",type);return false}
...
/* ================= V27.1 Sales Card unit-price + stable optimistic UI ================= */
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  const extra=make("附加费用 花盆/苔藓","sales-card-extra-total-v239",formatAmount(list.reduce((s,x)=>s+Number(x.extraFee||0),0)));
  shared.append(commission.w,delivery.w,extra.w);card.appendChild(shared);
  card.addEventListener("import-product-selected-v271",()=>syncCardAutoPotMossV271(card));
  ...
  extra.inp.addEventListener("input",()=>{
    card.dataset.extraManualV271="1";
    sharedEdit();
  });
  remark.addEventListener("input",()=>markSalesCardTransactionDirtyV239(card));
...
async function saveProductLinksV206(type){
  ...
  // V27.1 optimistic local cache: keep the current UI stable; do not redraw from an older async response.
...
/* ================= V27.1 same-day linked bonsai profit summary ================= */
...
async function toggleProductProfitSummaryV216(type,button){
  ...
  try{
    // V27.1: opening Daily Profit closes Sales Cards first, then paints local cache immediately.
    const pre=productLinkPreV208(type),box=document.getElementById(pre+"ProductLinkBox"),body=document.getElementById(pre+"ProductLinkBody");
...
/* ================= V27.1 expandable yearly monthly breakdown ================= */
...
function yearBreakdownRowsV224(kind){
  // V27.1: Balakong / Belimbing / Fair / Live show months inside the selected
...
function renderAll(){
  // V27.1: one complete render path. This replaces the older partial duplicate
...
// V27.1: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V27.1: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V27.1: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V27.1: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V27.1: deleting the last special commission rule leaves candidate and
...
/* ================= V27.1 Sales Card transaction integrity / instant cache ================= */
...
/* ================= V27.1 Live optimistic revision guard ================= */
...
async function refreshOpenSalesChangeLogAfterSaveV243(type,date){
  ...
  }catch(e){console.warn("V27.1 修改/销售记录后台刷新失败",e)}
}
...
saveLiveSales=async function(){
  ...
  // Critical V27.1 guard: any cloud request started before this local mutation
...
/* ================= V27.1 Profit list copy + compact totals ================= */
...
/* ================= V27.1 profit layout + weighted average margin ================= */
...
/* ================= V27.1 cross-date inventory reminders =================
...
function openImportCostSystemV266(target={}){
  // V27.1: Sales must stay open; Import always opens in a separate tab/window.
...
// V27.1: Sales-side inventory confirmation removed. Import is the only inventory authority.
...
// V27.1: after the user processes inventory in the separate Import tab/app,
