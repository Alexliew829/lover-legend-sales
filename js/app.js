...
// V41.9: click a Home Fair location to expand its newest month, plus the
...
const fairHomeHistoryOpenV419=new Set();
const FAIR_HOME_PROFIT_CACHE_KEY_V419='loverLegendSales.fairHomeProfitLinks.v1';
function readFairHomeProfitCacheV419(){
  try{
    const raw=localStorage.getItem(FAIR_HOME_PROFIT_CACHE_KEY_V419)||localStorage.getItem('loverLegendSales.fairHomeProfitLinks.v417')||'null';
    const saved=JSON.parse(raw);
    return saved&&Array.isArray(saved.links)?saved:null;
  }catch(_e){return null}
}
function fairHomeProfitSignatureV419(links){
...
function saveFairHomeProfitCacheV419(links,checkedAt=Date.now()){
  try{
    localStorage.setItem(FAIR_HOME_PROFIT_CACHE_KEY_V419,JSON.stringify({
      links:Array.isArray(links)?links:[],signature:fairHomeProfitSignatureV419(links),checkedAt
    }));
  }catch(e){console.warn('V41.9 Fair 利润缓存保存失败',e)}
}
const fairHomeProfitSavedV419=readFairHomeProfitCacheV419();
let fairHomeProfitLinksV419=fairHomeProfitSavedV419?fairHomeProfitSavedV419.links:null;
let fairHomeProfitCheckedAtV419=Number(fairHomeProfitSavedV419?.checkedAt||0);
let fairHomeProfitPromiseV419=null;
let fairHomeProfitVerifiedThisSessionV419=false;
let fairHomeProfitIndexSourceV419=null;
let fairHomeProfitIndexV419=new Map();
let fairHomeProfitVerifyTimerV419=null;
let fairHomeHistoryRowsCacheV419=new Map();
let fairHomeCombinedRateCacheV419=null;
function fairCommissionRateForMonthV419(month){
...
function fairLocationHistoryRowsV419(locationKey){
  if(fairHomeHistoryRowsCacheV419.has(locationKey))return fairHomeHistoryRowsCacheV419.get(locationKey);
  const map=new Map();
  ...
  if(!latest){fairHomeHistoryRowsCacheV419.set(locationKey,list);return list}
  ...
  fairHomeHistoryRowsCacheV419.set(locationKey,result);
  return result;
}
function fairHomeAllLocationKeysV419(){
  return Object.keys(fairByLocation()).map(name=>normalizeFairLocationKey(name)).filter(Boolean);
}
function fairHomeCombinedCommissionRateV419(locationKeys=fairHomeAllLocationKeysV419()){
  if(fairHomeCombinedRateCacheV419!==null)return fairHomeCombinedRateCacheV419;
  const keys=[...new Set(locationKeys)].filter(Boolean);
  const combinedSales=keys.reduce((sum,key)=>sum+fairLocationHistoryRowsV419(key).reduce((subtotal,r)=>subtotal+Number(r.amount||0),0),0);
  fairHomeCombinedRateCacheV419=getFairCommissionRate(combinedSales);
  return fairHomeCombinedRateCacheV419;
}
function fairHistoryProfitV419(links,locationKey,date){
  if(fairHomeProfitIndexSourceV419!==links){
    fairHomeProfitIndexSourceV419=links;
    fairHomeProfitIndexV419=new Map();
    ...
    clean.forEach(x=>{
      ...
      fairHomeProfitIndexV419.set(key,(fairHomeProfitIndexV419.get(key)||0)+Number(x.profit||0));
    });
  }
  return fairHomeProfitIndexV419.get(locationKey+'|'+date)||0;
}
function fairHistoryTableV419(locationKey,links){
  const list=fairLocationHistoryRowsV419(locationKey),ready=Array.isArray(links),rate=fairHomeCombinedCommissionRateV419();
  if(!list.length)return '<div class="fair-home-history-empty-v419">还没有历史记录</div>';
  let totalSales=0,totalProfit=0,totalCommission=0;
  const body=list.map(r=>{
    const sales=Number(r.amount||0),commission=sales*rate,profit=ready?fairHistoryProfitV419(links,locationKey,r.date):null,margin=profit===null?null:(sales>0?profit/sales*100:0);
    totalSales+=sales;totalCommission+=commission;if(profit!==null)totalProfit+=profit;
    return `<div class="fair-home-history-row-v419"><span>${shortDayMonthV297(r.date)}</span><b>${money(sales)}</b><b>${profit===null?'--':money(profit)}</b><b>${margin===null?'--':margin.toFixed(2)+'%'}</b><b>${money(commission)}</b></div>`;
  }).join('');
  const totalMargin=ready&&totalSales>0?totalProfit/totalSales*100:0;
  return `<div class="fair-home-history-table-v419"><div class="fair-home-history-head-v419"><span>日期</span><span>营业额</span><span>利润</span><span>利润率</span><span>佣金</span></div>${body}<div class="fair-home-history-row-v419 fair-home-history-total-v419"><span>总数</span><b>${money(totalSales)}</b><b>${ready?money(totalProfit):'--'}</b><b>${ready?totalMargin.toFixed(2)+'%':'--'}</b><b>${money(totalCommission)}</b></div></div>`;
}
function fairOpenLocationsGrandV419(locationKeys,links){
  const keys=[...new Set(locationKeys)].filter(Boolean),ready=Array.isArray(links);
  if(keys.length<2)return '';
  let totalSales=0,totalProfit=0;
  keys.forEach(key=>fairLocationHistoryRowsV419(key).forEach(r=>{
    const sales=Number(r.amount||0);
    totalSales+=sales;
    if(ready)totalProfit+=fairHistoryProfitV419(links,key,r.date);
  }));
  ...
  return `<div class="fair-home-grand-v419"><strong>${keys.length}个地点总数</strong><div class="fair-home-grand-grid-v419 fair-home-grand-head-v419"><span>营业额</span><span>利润</span><span>利润率</span><span>佣金</span></div><div class="fair-home-grand-grid-v419 fair-home-grand-values-v419"><b>${money(totalSales)}</b><b>${ready?money(totalProfit):'--'}</b><b>${ready?margin.toFixed(2)+'%':'--'}</b><b>${money(totalCommission)}</b></div></div>`;
}
function paintFairHomeHistoryV419(){
  document.querySelectorAll('.fair-home-history-panel-v419[data-location-key]').forEach(panel=>{const key=decodeURIComponent(panel.dataset.locationKey||'');panel.innerHTML=fairHistoryTableV419(key,fairHomeProfitLinksV419)});
  const grand=document.getElementById('fairHomeGrandV419');if(grand){const keys=String(grand.dataset.locationKeys||'').split(',').filter(Boolean).map(decodeURIComponent);grand.outerHTML=fairOpenLocationsGrandV419(keys,fairHomeProfitLinksV419).replace('<div class="fair-home-grand-v419"',`<div id="fairHomeGrandV419" data-location-keys="${keys.map(encodeURIComponent).join(',')}" class="fair-home-grand-v419"`)}
}
function ensureFairHomeProfitLinksV419(){
  // V41.9: this runs after the tap has already painted. Cloud verification is
  // silent and only repaints when the authoritative rows actually changed.
  if(fairHomeProfitPromiseV419)return fairHomeProfitPromiseV419;
  if(fairHomeProfitVerifiedThisSessionV419)return Promise.resolve(Array.isArray(fairHomeProfitLinksV419)?fairHomeProfitLinksV419:[]);
  const oldSignature=fairHomeProfitSignatureV419(fairHomeProfitLinksV419);
  fairHomeProfitPromiseV419=Promise.resolve(loadAllSalesProductLinksV203({force:true,maxAgeMs:0})).then(links=>{
    const fresh=Array.isArray(links)?links:[],freshSignature=fairHomeProfitSignatureV419(fresh);
    fairHomeProfitCheckedAtV419=Date.now();
    fairHomeProfitVerifiedThisSessionV419=true;
    if(!Array.isArray(fairHomeProfitLinksV419)||freshSignature!==oldSignature){
      fairHomeProfitLinksV419=fresh;
      paintFairHomeHistoryV419();
    }
    saveFairHomeProfitCacheV419(fresh,fairHomeProfitCheckedAtV419);
    return fresh;
  }).catch(e=>{console.warn('V41.9 Fair 全部历史利润静默检查失败，继续显示上次资料',e);return Array.isArray(fairHomeProfitLinksV419)?fairHomeProfitLinksV419:[]}).finally(()=>{fairHomeProfitPromiseV419=null});
  return fairHomeProfitPromiseV419;
}
function scheduleFairHomeProfitVerifyV419(){
  if(fairHomeProfitVerifiedThisSessionV419||fairHomeProfitPromiseV419||fairHomeProfitVerifyTimerV419!==null)return;
  const run=()=>{fairHomeProfitVerifyTimerV419=null;ensureFairHomeProfitLinksV419()};
  if(typeof requestIdleCallback==='function'){
    fairHomeProfitVerifyTimerV419=requestIdleCallback(run,{timeout:1200});
  }else{
    fairHomeProfitVerifyTimerV419=setTimeout(run,350);
  }
}
function refreshFairHomeGrandV419(){
  const grid=document.querySelector('#fairLocationList .fair-location-grid');
  if(!grid)return;
  const old=document.getElementById('fairHomeGrandV419');
  if(!fairHomeHistoryOpenV419.size){if(old)old.remove();return}
  const keys=fairHomeAllLocationKeysV419();
  const html=fairOpenLocationsGrandV419(keys,fairHomeProfitLinksV419);
  if(!html){if(old)old.remove();return}
  const wrapped=html.replace('<div class="fair-home-grand-v419"',`<div id="fairHomeGrandV419" data-location-keys="${keys.map(encodeURIComponent).join(',')}" class="fair-home-grand-v419"`);
  if(old)old.outerHTML=wrapped;else grid.insertAdjacentHTML('beforeend',wrapped);
}
function toggleFairHomeHistoryV419(encodedKey,button){
  const key=decodeURIComponent(String(encodedKey||''));if(!key)return;
  const card=button?.closest('.fair-location-card-v419');
  const opening=!fairHomeHistoryOpenV419.has(key);
  if(opening)fairHomeHistoryOpenV419.add(key);else fairHomeHistoryOpenV419.delete(key);
  if(button){
    button.setAttribute('aria-expanded',String(opening));
    const arrow=button.querySelector('b');if(arrow)arrow.textContent=opening?'▲':'▼';
  }
  if(card)card.classList.toggle('fair-location-card-open-v419',opening);
  let panel=card?.querySelector('.fair-home-history-panel-v419');
  if(!opening){
    if(panel)panel.remove();
    if(!fairHomeHistoryOpenV419.size){
      const grand=document.getElementById('fairHomeGrandV419');if(grand)grand.remove();
    }
    return;
  }
  if(card&&!panel){
    panel=document.createElement('div');
    panel.className='fair-home-history-panel-v419';
    panel.dataset.locationKey=encodeURIComponent(key);
    panel.innerHTML='<div class="fair-home-history-empty-v419">正在显示…</div>';
  ...
  requestAnimationFrame(()=>setTimeout(()=>{
    if(!fairHomeHistoryOpenV419.has(key))return;
    const current=card?.querySelector('.fair-home-history-panel-v419');
    if(current)current.innerHTML=fairHistoryTableV419(key,fairHomeProfitLinksV419);
    refreshFairHomeGrandV419();
    scheduleFairHomeProfitVerifyV419();
  },0));
}
window.toggleFairHomeHistoryV419=toggleFairHomeHistoryV419;
function renderFairLocationList(){
  fairHomeHistoryRowsCacheV419=new Map();
  fairHomeCombinedRateCacheV419=null;
  ...
  const hasOpenLocation=allLocationKeys.some(key=>fairHomeHistoryOpenV419.has(key));
  const grand=hasOpenLocation?fairOpenLocationsGrandV419(allLocationKeys,fairHomeProfitLinksV419):'';
  const grandHtml=grand?grand.replace('<div class="fair-home-grand-v419"',`<div id="fairHomeGrandV419" data-location-keys="${allLocationKeys.map(encodeURIComponent).join(',')}" class="fair-home-grand-v419"`):'';
  c.innerHTML='<div class="fair-location-grid">'+locs.map(l=>{const key=normalizeFairLocationKey(l),open=fairHomeHistoryOpenV419.has(key),encoded=encodeURIComponent(key);return `<div class="fair-location-card fair-location-card-v419 ${open?'fair-location-card-open-v419':''}"><button type="button" class="fair-location-title fair-location-toggle-v419" aria-expanded="${open}" onclick="toggleFairHomeHistoryV419('${encoded}',this)"><span>${escapeChangeLogHtmlV200(l)}</span><b>${open?'▲':'▼'}</b></button><div class="fair-location-row"><span>营业额</span><b>${money(g[l])}</b></div><div class="fair-location-row"><span>佣金 ${Number(rate.toFixed(2))}%</span><b>${money(g[l]*rate/100)}</b></div>${open?`<div class="fair-home-history-panel-v419" data-location-key="${encoded}">${fairHistoryTableV419(key,fairHomeProfitLinksV419)}</div>`:''}</div>`}).join("")+grandHtml+'</div>';
}
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4190",restoreGeneration:0};
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  // V41.9: confirmation belongs to the sales-card transaction, not only to the
  ...
  // V41.9: this is the active Sales / Fair / Live card builder. Keep each
...
function renderRemoteRestoreMaintenanceV345(active,message=""){
  // V41.9: the device that started/resumed this Restore stays on its More
...
// V41.9: the persisted Restore job remains authoritative through the final
...
function markDraftSavedLocallyV314(type,ctx,dirty,items,dirtyIds){
  ...
  const local=items.map(x=>{
    ...
    let status='DRAFT';
    if(wasConfirmed){
      // V41.9: preserve each already-confirmed row, but a NEW product added to an
...
async function validateSalesInventoryAvailabilityV325(type,items,saveMode='draft'){
  ...
  // V41.9: keep the same authoritative Import oversell guard, but avoid the old
...
saveProductLinksV206=async function(type,saveMode='confirm',button=null){
  ...
  // V41.9 protects navigation from the instant Confirm is pressed.
  ...
  // V41.9: a draft is local/cloud sales-card data only. Import availability is
...
// V41.9: the pending endpoint is the smallest authoritative ACK check. When a
...
// V41.9: do not wait for the full Sales cloud-resume cycle. Import ACK is a
...
// V41.9 final navigation gate, installed after every legacy showPage wrapper.
