...
function showPage(name,el){
  ...
  // V21.1: every time Live is opened, start from today's date.
  ...
  // V21.1: page switching never waits for or triggers cloud sync.
...
// V21.1: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V21.1: historical record is lazy. Top 5 opens instantly from local rows;
...
function applyCloudCommissionSettings(settings){
  ...
  // V21.1: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V21.1: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V21.1: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V21.1: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V21.1: local Fair values are direct replacements, never additions. The server
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"2110"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=sanitizeClosedMonthsClientV197(state.closedMonths,systemState.currentMonth);systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"2110"}updateReadOnlyMode()}
...
// V21.1: paint Home immediately, restore local cache, then perform only a
...
// V21.1: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V21.1: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V21.1: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V21.1 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V21.1: show the first editable card immediately. This is a pure front-end action
...
function productLinkDefaultCommissionRateV211(type){
  if(type==="live"){
    const ctx=productLinkContextV206("live");
    return Math.max(0,Number(getLiveHostRate(ctx.location,ctx.date)||10));
  }
  const fairMonthTotal=totalBy("fair","","month");
  return Math.max(0,Number(getFairCommissionRate(fairMonthTotal)*100||0));
}
...
function buildProductLinkItemV209(type,id,data={}){
  ...
  const rate=input("product-link-commission-rate",Number(defaultRate||0).toFixed(2));rate.type="number";rate.min="0";rate.step="0.01";rate.inputMode="decimal";rateWrap.appendChild(rate);
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V21.1 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V21.1 button bindings reliable.
...
function collectProductLinksV206(type){
  ...
  return items.map(item=>{
    ...
    return{linkId,type,date,location,productId:"",productName,quantity,averageCost,actualPrice,commissionRate,commissionAmount,localDelivery,extraFee,profit,profitRate,remark};
...
function renderAll(){
  // V21.1: one complete render path. This replaces the older partial duplicate
...
// V21.1: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V21.1: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V21.1: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V21.1: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V21.1: deleting the last special commission rule leaves candidate and
