...
// V41.5: click a Home Fair location to expand its newest month, plus the
...
const fairHomeHistoryOpenV415=new Set();
let fairHomeProfitLinksV415=null;
let fairHomeProfitPromiseV415=null;
function fairCommissionRateForMonthV415(month){
...
function fairLocationHistoryRowsV415(locationKey){
  const map=new Map();
...
function fairHistoryProfitV415(links,locationKey,date){
...
function fairHistoryTableV415(locationKey,links){
  const list=fairLocationHistoryRowsV415(locationKey),ready=Array.isArray(links);
  if(!list.length)return '<div class="fair-home-history-empty-v415">还没有历史记录</div>';
  let totalSales=0,totalProfit=0,totalCommission=0;
  const body=list.map(r=>{
    const sales=Number(r.amount||0),month=displayToISO(r.date).slice(0,7),rate=fairCommissionRateForMonthV415(month),commission=sales*rate,profit=ready?fairHistoryProfitV415(links,locationKey,r.date):null,margin=profit===null?null:(sales>0?profit/sales*100:0);
    totalSales+=sales;totalCommission+=commission;if(profit!==null)totalProfit+=profit;
    return `<div class="fair-home-history-row-v415"><span>${shortDayMonthV297(r.date)}</span><b>${money(sales)}</b><b>${profit===null?'--':money(profit)}</b><b>${margin===null?'--':margin.toFixed(2)+'%'}</b><b>${money(commission)}</b></div>`;
  }).join('');
  const totalMargin=ready&&totalSales>0?totalProfit/totalSales*100:0;
  return `<div class="fair-home-history-table-v415"><div class="fair-home-history-head-v415"><span>日期</span><span>营业额</span><span>利润</span><span>利润率</span><span>佣金</span></div>${body}<div class="fair-home-history-row-v415 fair-home-history-total-v415"><span>总数</span><b>${money(totalSales)}</b><b>${ready?money(totalProfit):'--'}</b><b>${ready?totalMargin.toFixed(2)+'%':'--'}</b><b>${money(totalCommission)}</b></div></div>`;
}
function fairOpenLocationsGrandV415(locationKeys,links){
  const keys=[...new Set(locationKeys)].filter(Boolean),ready=Array.isArray(links);
  if(keys.length<2)return '';
  let totalSales=0,totalProfit=0,totalCommission=0;
  keys.forEach(key=>fairLocationHistoryRowsV415(key).forEach(r=>{
    const sales=Number(r.amount||0),month=displayToISO(r.date).slice(0,7),rate=fairCommissionRateForMonthV415(month);
    totalSales+=sales;totalCommission+=sales*rate;
    if(ready)totalProfit+=fairHistoryProfitV415(links,key,r.date);
  }));
  const margin=ready&&totalSales>0?totalProfit/totalSales*100:0;
  return `<div class="fair-home-grand-v415"><strong>${keys.length}个地点总数</strong><div class="fair-home-grand-grid-v415 fair-home-grand-head-v415"><span>营业额</span><span>利润</span><span>利润率</span><span>佣金</span></div><div class="fair-home-grand-grid-v415 fair-home-grand-values-v415"><b>${money(totalSales)}</b><b>${ready?money(totalProfit):'--'}</b><b>${ready?margin.toFixed(2)+'%':'--'}</b><b>${money(totalCommission)}</b></div></div>`;
}
function paintFairHomeHistoryV415(){
  document.querySelectorAll('.fair-home-history-panel-v415[data-location-key]').forEach(panel=>{const key=decodeURIComponent(panel.dataset.locationKey||'');panel.innerHTML=fairHistoryTableV415(key,fairHomeProfitLinksV415)});
  const grand=document.getElementById('fairHomeGrandV415');if(grand){const keys=String(grand.dataset.locationKeys||'').split(',').filter(Boolean).map(decodeURIComponent);grand.outerHTML=fairOpenLocationsGrandV415(keys,fairHomeProfitLinksV415).replace('<div class="fair-home-grand-v415"',`<div id="fairHomeGrandV415" data-location-keys="${keys.map(encodeURIComponent).join(',')}" class="fair-home-grand-v415"`)}
}
function ensureFairHomeProfitLinksV415(){
  if(Array.isArray(fairHomeProfitLinksV415)){paintFairHomeHistoryV415();return Promise.resolve(fairHomeProfitLinksV415)}
  if(fairHomeProfitPromiseV415)return fairHomeProfitPromiseV415;
  fairHomeProfitPromiseV415=Promise.resolve(loadAllSalesProductLinksV203({force:false,maxAgeMs:180000})).then(links=>{fairHomeProfitLinksV415=Array.isArray(links)?links:[];paintFairHomeHistoryV415();return fairHomeProfitLinksV415}).catch(e=>{console.warn('V41.5 Fair 全部历史利润读取失败',e);return[]}).finally(()=>{fairHomeProfitPromiseV415=null});
  return fairHomeProfitPromiseV415;
}
function toggleFairHomeHistoryV415(encodedKey){
  const key=decodeURIComponent(String(encodedKey||''));if(!key)return;
  if(fairHomeHistoryOpenV415.has(key))fairHomeHistoryOpenV415.delete(key);else fairHomeHistoryOpenV415.add(key);
  renderFairLocationList();
  if(fairHomeHistoryOpenV415.has(key))ensureFairHomeProfitLinksV415();
}
window.toggleFairHomeHistoryV415=toggleFairHomeHistoryV415;
function renderFairLocationList(){
  ...
  const hasOpenLocation=allLocationKeys.some(key=>fairHomeHistoryOpenV415.has(key));
  const grand=hasOpenLocation?fairOpenLocationsGrandV415(allLocationKeys,fairHomeProfitLinksV415):'';
  const grandHtml=grand?grand.replace('<div class="fair-home-grand-v415"',`<div id="fairHomeGrandV415" data-location-keys="${allLocationKeys.map(encodeURIComponent).join(',')}" class="fair-home-grand-v415"`):'';
  c.innerHTML='<div class="fair-location-grid">'+locs.map(l=>{const key=normalizeFairLocationKey(l),open=fairHomeHistoryOpenV415.has(key),encoded=encodeURIComponent(key);return `<div class="fair-location-card fair-location-card-v415 ${open?'fair-location-card-open-v415':''}"><button type="button" class="fair-location-title fair-location-toggle-v415" aria-expanded="${open}" onclick="toggleFairHomeHistoryV415('${encoded}')"><span>${escapeChangeLogHtmlV200(l)}</span><b>${open?'▲':'▼'}</b></button><div class="fair-location-row"><span>营业额</span><b>${money(g[l])}</b></div><div class="fair-location-row"><span>佣金 ${Number(rate.toFixed(2))}%</span><b>${money(g[l]*rate/100)}</b></div>${open?`<div class="fair-home-history-panel-v415" data-location-key="${encoded}">${fairHistoryTableV415(key,fairHomeProfitLinksV415)}</div>`:''}</div>`}).join("")+grandHtml+'</div>';
}
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4150",restoreGeneration:0};
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  // V41.5: confirmation belongs to the sales-card transaction, not only to the
  ...
  // V41.5: this is the active Sales / Fair / Live card builder. Keep each
...
function renderRemoteRestoreMaintenanceV345(active,message=""){
  // V41.5: the device that started/resumed this Restore stays on its More
...
// V41.5: the persisted Restore job remains authoritative through the final
...
function markDraftSavedLocallyV314(type,ctx,dirty,items,dirtyIds){
  ...
  const local=items.map(x=>{
    ...
    let status='DRAFT';
    if(wasConfirmed){
      // V41.5: preserve each already-confirmed row, but a NEW product added to an
...
async function validateSalesInventoryAvailabilityV325(type,items,saveMode='draft'){
  ...
  // V41.5: keep the same authoritative Import oversell guard, but avoid the old
...
saveProductLinksV206=async function(type,saveMode='confirm',button=null){
  ...
  // V41.5 protects navigation from the instant Confirm is pressed.
  ...
  // V41.5: a draft is local/cloud sales-card data only. Import availability is
...
// V41.5: the pending endpoint is the smallest authoritative ACK check. When a
...
// V41.5: do not wait for the full Sales cloud-resume cycle. Import ACK is a
...
// V41.5 final navigation gate, installed after every legacy showPage wrapper.
