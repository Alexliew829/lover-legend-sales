...
function showPage(name,el){
  ...
  // V22.8: every time Live is opened, start from today's date.
  ...
  // V22.8: page switching never waits for or triggers cloud sync.
...
// V22.8: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V22.8: historical record is lazy. Top 5 opens instantly from local rows;
...
function applyCloudCommissionSettings(settings){
  ...
  // V22.8: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V22.8: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V22.8: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V22.8: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V22.8: local Fair values are direct replacements, never additions. The server
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"2280"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=sanitizeClosedMonthsClientV197(state.closedMonths,systemState.currentMonth);systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"2280"}updateReadOnlyMode()}
...
// V22.8: paint Home immediately, restore local cache, then perform only a
...
// V22.8: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V22.8: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V22.8: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V22.8 Import Cost System product search ================= */
...
function setupImportProductSearchV214(item,nameInput,resultsBox,closeButton){
  // V22.8: iPhone/iOS safe search state.
  ...
  const render=async()=>{
    ...
    try{
      ...
      matches.forEach(record=>{
        ...
        // V22.8: keep iPhone dropdown stable, but allow option selection.
        // Do not prevent touchstart/pointerdown; that blocked taps on iOS.
        ...
        row.addEventListener("click",chooseRecord);
        row.addEventListener("touchend",e=>{
          // iOS may not always dispatch click reliably after keyboard interaction.
          chooseRecord(e);
        },{passive:false});
...
/* ================= V22.8 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V22.8: Sales Cards and Daily Profit are mutually exclusive.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V22.8 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V22.8 button bindings reliable.
...
async function loadProductLinksIntoEditorV206(type){
  ...
  // Never replace the DOM while the user is already typing/searching.
...
/* ================= V22.8 same-day linked bonsai profit summary ================= */
...
async function toggleProductProfitSummaryV216(type,button){
  ...
  try{
    // V22.8: opening Daily Profit closes Sales Cards first.
    const pre=productLinkPreV208(type),box=document.getElementById(pre+"ProductLinkBox"),body=document.getElementById(pre+"ProductLinkBody");
...
/* ================= V22.8 expandable yearly monthly breakdown ================= */
...
function yearBreakdownRowsV224(kind){
  // V22.8: Balakong / Belimbing / Fair / Live show months inside the selected
...
function renderAll(){
  // V22.8: one complete render path. This replaces the older partial duplicate
...
// V22.8: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V22.8: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V22.8: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V22.8: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V22.8: deleting the last special commission rule leaves candidate and
