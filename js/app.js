...
function showPage(name,el){
  ...
  // V18.5: every time Live is opened, start from today's date.
  ...
  // V18.5: page switching never waits for or triggers cloud sync.
...
// V18.5: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V18.5: historical record is lazy. Top 5 opens instantly from local rows;
...
function applyCloudCommissionSettings(settings){
  ...
  // V18.5: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V18.5: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V18.5: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V18.5 mobile-safe save: return control immediately after the local durable
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V18.5: local Fair values are direct replacements, never additions. The server
...
// V18.5: paint Home immediately, restore local cache, then perform only a
...
// V18.5: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V18.5: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V18.5: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
function renderAll(){
  // V18.5: one complete render path. This replaces the older partial duplicate
...
// V18.5: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V18.5: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V18.5: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V18.5: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V18.5: deleting the last special commission rule leaves candidate and
