...
function showPage(name,el){
  ...
  // V9.9: every time Live is opened, start from today's date.
  ...
  // V9.9: page switching never waits for or triggers cloud sync.
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"9.9"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=Array.isArray(state.closedMonths)?state.closedMonths:[];systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"9.9"}updateReadOnlyMode()}
...
// V9.9: display the last saved local data immediately while the password screen is open.
...
function startInitialSalesDataLoad() {
  ...
  // V9.9: give the browser time to paint and accept input first.
...
function restoreLastLiveSession(){
  ...
  // V9.9: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V9.9：单击后立即套用，并由所有已开启装置自动读取最新佣金。
...
/* ================= V9.9 Backup / Restore ================= */
function getBackupPayload(){return{system:"Lover Legend Sales System",version:"9.9",createdAt:new Date().toISOString(),rows:dedupeRows(rows),commissionSettings:getCommissionSettings(),
...
function backupAllData(){const payload=getBackupPayload();const stamp=new Date().toISOString().replace(/[:T]/g,"-").slice(0,19);downloadFile(`Lover_Legend_Sales_V9_9_Backup_${stamp}.json`,JSON.stringify(payload,null,2),"application/json;charset=utf-8;")}
