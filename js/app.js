...
function showPage(name,el){
  ...
  // V20.8: every time Live is opened, start from today's date.
  ...
  // V20.8: page switching never waits for or triggers cloud sync.
...
// V20.8: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V20.8: historical record is lazy. Top 5 opens instantly from local rows;
...
function applyCloudCommissionSettings(settings){
  ...
  // V20.8: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V20.8: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V20.8: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V20.8: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V20.8: local Fair values are direct replacements, never additions. The server
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"2008"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=sanitizeClosedMonthsClientV197(state.closedMonths,systemState.currentMonth);systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"2008"}updateReadOnlyMode()}
...
// V20.8: paint Home immediately, restore local cache, then perform only a
...
// V20.8: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V20.8: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V20.8: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V20.8 optional bonsai product association ================= */
...
function ensureProductLinkFirstCardV208(type){
  ...
  if(!wrap.querySelector('.product-link-item'))addProductLinkItemV206(type);
  return true;
}
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V20.8: show the first editable card immediately. This is a pure front-end action
...
function addProductLinkItemV206(type,data={}){
  // V20.8: adding a blank card must not depend on selected date/host/location or cloud state.
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap)return false;
  const id=++productLinkItemSeqV206;
  wrap.insertAdjacentHTML("beforeend",productLinkItemHtmlV206(type,id,data));
  return true;
}
// Explicit globals keep inline buttons reliable even after cached/ordered script loading.
...
function renderAll(){
  // V20.8: one complete render path. This replaces the older partial duplicate
...
// V20.8: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V20.8: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V20.8: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V20.8: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V20.8: deleting the last special commission rule leaves candidate and
