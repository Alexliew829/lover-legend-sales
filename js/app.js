...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4630",restoreGeneration:0};
...
function renderSystemInformationV442(data){
  ...
  set('systemInfoApiV442',data?.apiVersion?`V${String(data.apiVersion).replace(/^V/i,'').replace(/^4630$/,'46.3')}`:'连接异常');
...
async function runSystemHealthCheckV442(userTriggered=false){
  ...
  try{
    const data=await jsonp({action:'healthV443',clientVersion:'4630'},{timeoutMs:15000});
    if(!data?.ok)throw new Error(data?.message||'系统检查失败');
    const issues=[];
    if(String(data.apiVersion||'')!=='4630')issues.push(`Frontend / API 版本不一致（Frontend 4630 / API ${data.apiVersion||'未知'}）`);
    (Array.isArray(data.issues)?data.issues:[]).forEach(x=>issues.push(String(x)));
    const severe=Boolean(data.severe)||!data.sheetConnected||String(data.apiVersion||'')!=='4630';
