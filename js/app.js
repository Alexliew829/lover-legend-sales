...
function showPage(name,el){
  ...
  // V29.0: page "sales" uses the sales-card type "daily".
  ...
  // V29.0: every time Live is opened, start from today's date.
  ...
  // V29.0: page switching never waits for or triggers cloud sync.
...
// V29.0: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V29.0: historical record is lazy. Top 5 opens instantly from local rows;
...
function renderTodayCompanyStatus(){
  ...
  // V29.0: status colour represents actual business amount, not merely
...
function applyCloudCommissionSettings(settings){
  ...
  // V29.0: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V29.0: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V29.0: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V29.0: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V29.0: local Fair values are direct replacements, never additions. The server
...
// V29.0: paint Home immediately, restore local cache, then perform only a
...
// V29.0: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V29.0: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V29.0: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V29.0 Import Cost System product search ================= */
...
function unorderedImportProductMatchV214(sourceValue,queryValue){
  ...
  if(isExactProductCodeQueryV214(query)){
    // V29.0: code search supports case-insensitive prefix/partial entry.
...
function setupImportProductSearchV214(item,nameInput,resultsBox,closeButton){
  // V29.0: iPhone/iOS safe search state.
  ...
  const render=async()=>{
    ...
    try{
      ...
      matches.forEach(record=>{
        ...
        // V29.0: distinguish an intentional tap from list scrolling on iPhone/iOS.
  ...
  if(closeButton){
    ...
    closeButton.addEventListener("click",e=>{
      ...
      // V29.0: X means CLEAR CURRENT PRODUCT SELECTION.
...
/* ================= V29.0 Sales Card unsaved-change protection ================= */
...
function hasUnsavedSalesCardChangesV238(type){
  ...
  // V29.0: use the same transaction-level dirty signal that Live already maintains.
...
/* ================= V29.0 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V29.0: Sales Cards and Daily Profit are mutually exclusive.
    ...
    // V29.0: saved cards first. Do not flash a fake 0.00 card while cloud data loads.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V29.0 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V29.0 button bindings reliable.
...
/* ================= V29.0 multi-product Sales Card ================= */
...
function buildProductSubItemV239(type,card,data={},order=1){
  ...
  [c,p].forEach(el=>el.addEventListener("blur",()=>{
    ...
    // V29.0: blur fires before the Save button click. Re-mark the card dirty here so
...
function bindSingleTapDeleteProductV254(button,handler){
  ...
  // V29.0: for touch/pen, pointerup is the one and only real activation.
...
系统会保留 saleId 的删除记录，让 Import Cost System 知道这张销售已经取消。`))return;
  try{
    ...
    // V29.0: refresh profit view from active backend links after card deletion.
    try{
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  // V29.0: Sales only reminds. Inventory confirmation is handled in Import Cost System.
  ...
  setTimeout(()=>{
    // V29.0: initialization may finish after a very fast first edit.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V29.0 sales card container missing",type);return false}
...
/* ================= V29.0 Sales Card unit-price + stable optimistic UI ================= */
...
async function saveProductLinksV206(type){
  ...
  // V29.0 optimistic local cache: keep the current UI stable; do not redraw from an older async response.
...
/* ================= V29.0 same-day linked bonsai profit summary ================= */
...
async function toggleProductProfitSummaryV216(type,button){
  ...
  try{
    // V29.0: opening Daily Profit closes Sales Cards first, then paints local cache immediately.
    const pre=productLinkPreV208(type),box=document.getElementById(pre+"ProductLinkBox"),body=document.getElementById(pre+"ProductLinkBody");
...
/* ================= V29.0 expandable yearly monthly turnover + profit breakdown ================= */
...
const yearBreakdownProfitCacheV286={links:null,loadedAt:0};

function yearBreakdownProfitByMonthV286(kind,links){
  const map=new Map();
  (links||[]).forEach(x=>{
    if(String(x.saleStatus||'active').toLowerCase()==='deleted')return;
    const d=String(x.date||'');
    const m=d.match(/^(\d{2})-(\d{2})-(\d{4})$/);if(!m)return;
    const month=`${m[3]}-${m[2]}`;
    const type=String(x.type||'');
    // V29.0: existing Daily Sales Cards belong to Belimbing. Balakong has no
    // product sales-card profit source in the V28.4/V29.0 data model.
    let bucket='';
    if(type==='daily')bucket='belimbing';
    else if(type==='fair')bucket='fair';
    else if(type==='live')bucket='live';
    if(!bucket)return;
    const rec=map.get(month)||{balakong:0,belimbing:0,fair:0,live:0,total:0};
    const p=Number(x.profit||0);rec[bucket]+=p;rec.total+=p;map.set(month,rec);
  });
  return map;
}
function yearBreakdownRowsV224(kind){
  const profitMap=yearBreakdownProfitByMonthV286(kind,yearBreakdownProfitCacheV286.links||[]);
  if(kind==='total'){
    const byYear=new Map();
    buildMonthlySummary().forEach(item=>{
      const year=String(item.month||'').slice(0,4);if(!/^\d{4}$/.test(year))return;
      const rec=byYear.get(year)||{year,amount:0,profit:0};
      rec.amount+=Number(item.total||0);
      rec.profit+=Number(profitMap.get(item.month)?.total||0);
      byYear.set(year,rec);
    });
    return [...byYear.values()].filter(x=>Math.abs(x.amount)>0.000001||Math.abs(x.profit)>0.000001).sort((a,b)=>a.year.localeCompare(b.year));
  }
  const year=String(document.getElementById('yearPicker')?.value||selectedYear()||'');
  return buildMonthlySummary().filter(item=>!year||String(item.month||'').slice(0,4)===year)
    .map(item=>({month:item.month,amount:Number(item[kind]||0),profit:Number(profitMap.get(item.month)?.[kind]||0)}))
    .filter(item=>Math.abs(item.amount)>0.000001||Math.abs(item.profit)>0.000001)
    .sort((a,b)=>String(a.month).localeCompare(String(b.month)));
}
function yearBreakdownLineV286(kind,x){
  const label=kind==='total'?String(x.year||x.month):String(x.month).slice(5,7)+'-'+String(x.month).slice(0,4);
  const rate=Number(x.amount||0)>0?Number(x.profit||0)/Number(x.amount||0)*100:0;
  return `<div class="year-profit-row-v286"><span>${label}</span><div><b>${money(x.amount)}</b><b class="year-profit-v286">${money(x.profit)}</b><em>${rate.toFixed(2)}%</em></div></div>`;
}
function renderYearBreakdownV224(kind){
  ...
  const list=yearBreakdownRowsV224(kind),grand=list.reduce((s,x)=>({amount:s.amount+Number(x.amount||0),profit:s.profit+Number(x.profit||0)}),{amount:0,profit:0});
  const rate=grand.amount>0?grand.profit/grand.amount*100:0;panel.classList.remove('hidden');if(arrow)arrow.textContent='▲';
  if(yearBreakdownLoadingV224[kind]&&!yearBreakdownProfitCacheV286.links){panel.innerHTML='<div class="sub">正在读取营业 / 利润...</div>';return;}
  panel.innerHTML=list.length?`<div class="month-grand-history-list"><div class="year-profit-header-v286"><span>日期</span><b>营业额</b><b>利润</b><em>利润率</em></div>${list.map(x=>yearBreakdownLineV286(kind,x)).join('')}<div class="month-grand-history-total year-profit-total-v286"><span>总数</span><div><b>${money(grand.amount)}</b><b>${money(grand.profit)}</b><em>${rate.toFixed(2)}%</em></div></div></div>`:'<div class="sub">还没有营业 / 利润记录</div>';
}
...
async function toggleYearBreakdownV224(kind){
  ...
  try{
    await Promise.all([
      typeof loadYearInBackground==='function'&&/^\d{4}$/.test(year)?loadYearInBackground(year):Promise.resolve(),
      loadAllSalesProductLinksV203({force:true,maxAgeMs:0}).then(v=>{yearBreakdownProfitCacheV286.links=Array.isArray(v)?v:[];yearBreakdownProfitCacheV286.loadedAt=Date.now()})
    ]);
  }catch(e){console.warn('V29.0 Year turnover/profit load skipped:',kind,e)}
...
function renderAll(){
  // V29.0: one complete render path. This replaces the older partial duplicate
...
// V29.0: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V29.0: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V29.0: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
/* ================= V29.0 pre-change sales-card context guard ================= */
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V29.0: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V29.0: deleting the last special commission rule leaves candidate and
...
/* ================= V29.0 Sales Card transaction integrity / instant cache ================= */
...
saveProductLinksV206=async function(type,saveMode='confirm',button=null){
  ...
  if(!dirty.length){alert('销售卡没有需要保存的修改。');if(button){button.disabled=false;button.dataset.busyV285='0';button.textContent=originalButtonText;}if(button){button.disabled=false;button.dataset.busyV285='0';button.textContent=originalButtonText;}return null;}
  ...
  if(!items.length){alert('请填写销售卡产品资料。');if(button){button.disabled=false;button.dataset.busyV285='0';button.textContent=originalButtonText;}return null;}
  const first=items[0],official=salesCardsOfficialAmountV241(type),allTotal=all.reduce((s,x)=>s+Number(x.actualPrice||0),0);
  if(official>0&&allTotal>official+0.005){alert(`所有销售卡售价总数 RM${formatAmount(allTotal)} 已超过当天营业额 RM${formatAmount(official)}。`);if(button){button.disabled=false;button.dataset.busyV285='0';button.textContent=originalButtonText;}return null;}
  for(const x of items){
    if(!x.productName){alert('每一个产品都需要填写产品名称。');if(button){button.disabled=false;button.dataset.busyV285='0';button.textContent=originalButtonText;}return null}
    if(!Number.isFinite(Number(x.quantity))||Number(x.quantity)<=0){alert(`产品${Number(x.productOrder||0)||""} 数量必须大于 0。`);if(button){button.disabled=false;button.dataset.busyV285='0';button.textContent=originalButtonText;}return null}
    if(!Number.isFinite(Number(x.unitPrice))||Number(x.unitPrice)<=0){
      alert(`产品${Number(x.productOrder||0)||""} 售价必须大于 RM0.00，无法保存销售卡。`);
      if(button){button.disabled=false;button.dataset.busyV285='0';button.textContent=originalButtonText;}return null;
    }
  }
...
/* ================= V29.0 Live optimistic revision guard ================= */
...
async function refreshOpenSalesChangeLogAfterSaveV243(type,date){
  ...
  }catch(e){console.warn("V29.0 修改/销售记录后台刷新失败",e)}
}
...
saveLiveSales=async function(){
  ...
  // V29.0 preflight: do not even mutate local turnover when saved cards exceed the new amount.
  try{
  ...
  // Critical V29.0 guard: any cloud request started before this local mutation
...
/* ================= V29.0 Profit list copy + compact totals ================= */
...
/* ================= V29.0 profit layout + weighted average margin ================= */
...
/* ================= V29.0 cross-date inventory reminders =================
...
function openImportCostSystemV266(target={}){
  // V29.0: Sales must stay open; Import always opens in a separate tab/window.
...
// V29.0: Sales-side inventory confirmation removed. Import is the only inventory authority.
...
// V29.0: after the user processes inventory in the separate Import tab/app,
