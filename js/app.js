...
function showPage(name,el){
  ...
  // V21.2: every time Live is opened, start from today's date.
  ...
  // V21.2: page switching never waits for or triggers cloud sync.
...
// V21.2: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V21.2: historical record is lazy. Top 5 opens instantly from local rows;
...
function applyCloudCommissionSettings(settings){
  ...
  // V21.2: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V21.2: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V21.2: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V21.2: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V21.2: local Fair values are direct replacements, never additions. The server
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"2120"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=sanitizeClosedMonthsClientV197(state.closedMonths,systemState.currentMonth);systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"2120"}updateReadOnlyMode()}
...
// V21.2: paint Home immediately, restore local cache, then perform only a
...
// V21.2: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V21.2: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V21.2: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V21.2 Import Cost System product picker ================= */
let importProductsV212=[];
let importProductsPromiseV212=null;
function ensureImportProductDatalistV212(){
  let dl=document.getElementById("importProductOptionsV212");
  if(!dl){dl=document.createElement("datalist");dl.id="importProductOptionsV212";document.body.appendChild(dl);}
  return dl;
}
async function loadImportProductsV212(){
  if(importProductsV212.length)return importProductsV212;
  if(importProductsPromiseV212)return importProductsPromiseV212;
  importProductsPromiseV212=(async()=>{
    const res=await jsonp({action:"getImportProducts"},{timeoutMs:12000});
    if(!res||res.ok===false)throw new Error(res?.message||"读取 Import Cost System 产品失败");
    importProductsV212=Array.isArray(res.products)?res.products:[];
    const dl=ensureImportProductDatalistV212();
    dl.innerHTML=importProductsV212.map(x=>`<option value="${escapeChangeLogHtmlV200(String(x.productName||""))}"></option>`).join("");
    return importProductsV212;
  })().finally(()=>{importProductsPromiseV212=null;});
  return importProductsPromiseV212;
}
function applyImportProductSelectionV212(item,nameInput){
  const typed=String(nameInput?.value||"").trim();
  const match=importProductsV212.find(x=>String(x.productName||"").trim()===typed);
  const cost=item?.querySelector(".product-link-avg-cost");
  if(match){
    nameInput.dataset.productId=String(match.productId||"");
    if(cost)cost.value=formatAmount(Number(match.averageCost||0));
  }else{
    nameInput.dataset.productId="";
    if(cost)cost.value="0.00";
  }
  recalcProductLinkProfitV211(item);
}

/* ================= V21.2 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V21.2: show the first editable card immediately. This is a pure front-end action
...
function productLinkDefaultCommissionRateV211(type){
  // V21.2: linked-product commission starts at 0 for both Live and Fair.
  // Each sales card remains independently editable.
  return 0;
}
...
function buildProductLinkItemV209(type,id,data={}){
  ...
  const name=input("product-link-name",String(data.productName||""),"可从 Import Cost System 选择，也可自己输入");
  name.setAttribute("list","importProductOptionsV212");
  name.dataset.productId=String(data.productId||"");
  item.appendChild(name);
  ensureImportProductDatalistV212();
  name.addEventListener("focus",()=>loadImportProductsV212().catch(()=>{}));
  name.addEventListener("change",()=>applyImportProductSelectionV212(item,name));
  ...
  p.addEventListener("input",()=>{if(live)applyLiveDeliveryDefaultItemV206(p);recalc()});
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V21.2 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V21.2 button bindings reliable.
...
function collectProductLinksV206(type){
  ...
  return items.map(item=>{
    ...
    const productId=String(item.querySelector(".product-link-name")?.dataset.productId||"");return{linkId,type,date,location,productId,productName,quantity,averageCost,actualPrice,commissionRate,commissionAmount,localDelivery,extraFee,profit,profitRate,remark};
...
function renderAll(){
  // V21.2: one complete render path. This replaces the older partial duplicate
...
// V21.2: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V21.2: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V21.2: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V21.2: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V21.2: deleting the last special commission rule leaves candidate and
