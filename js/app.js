...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4080",restoreGeneration:0};
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  // V40.8: confirmation belongs to the sales-card transaction, not only to the
  ...
  // V40.8: this is the active Sales / Fair / Live card builder. Keep each
...
window.addEventListener("beforeunload",event=>{
  ...
  if(!(fairCommissionDraftDirty||liveCommissionDraftDirty||backupRestoreOperationRunningV234||unsavedCards||anyTurnoverLeaveRiskV382()||SALES_CONFIRMATION_IN_FLIGHT_V407))return;
...
function markDraftSavedLocallyV314(type,ctx,dirty,items,dirtyIds){
  ...
  const local=items.map(x=>{
    ...
    let status='DRAFT';
    if(wasConfirmed){
      // V40.8: preserve each already-confirmed row, but a NEW product added to an
...
async function validateSalesInventoryAvailabilityV325(type,items,saveMode='draft'){
  ...
  // V40.8: keep the same authoritative Import oversell guard, but avoid the old
...
saveProductLinksV206=async function(type,saveMode='confirm',button=null){
  ...
  // V40.8 protects navigation from the instant Confirm is pressed.
  ...
  // V40.8: a draft is local/cloud sales-card data only. Import availability is
...
// V40.8: the pending endpoint is the smallest authoritative ACK check. When a
...
// V40.8: do not wait for the full Sales cloud-resume cycle. Import ACK is a
