...
function showPage(name,el){
  ...
  // V27.9: page "sales" uses the sales-card type "daily".
  ...
  // V27.9: every time Live is opened, start from today's date.
  ...
  // V27.9: page switching never waits for or triggers cloud sync.
...
// V27.9: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V27.9: historical record is lazy. Top 5 opens instantly from local rows;
...
function renderTodayCompanyStatus(){
  ...
  // V27.9: status colour represents actual business amount, not merely
...
function applyCloudCommissionSettings(settings){
  ...
  // V27.9: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V27.9: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V27.9: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V27.9: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V27.9: local Fair values are direct replacements, never additions. The server
...
// V27.9: paint Home immediately, restore local cache, then perform only a
...
// V27.9: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V27.9: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V27.9: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V27.9 Import Cost System product search ================= */
...
function unorderedImportProductMatchV214(sourceValue,queryValue){
  ...
  if(isExactProductCodeQueryV214(query)){
    // V27.9: code search supports case-insensitive prefix/partial entry.
...
function setupImportProductSearchV214(item,nameInput,resultsBox,closeButton){
  // V27.9: iPhone/iOS safe search state.
  ...
  const render=async()=>{
    ...
    try{
      ...
      matches.forEach(record=>{
        ...
        // V27.9: distinguish an intentional tap from list scrolling on iPhone/iOS.
  ...
  if(closeButton){
    ...
    closeButton.addEventListener("click",e=>{
      ...
      // V27.9: X means CLEAR CURRENT PRODUCT SELECTION.
...
/* ================= V27.9 Sales Card unsaved-change protection ================= */
...
function hasUnsavedSalesCardChangesV238(type){
  ...
  // V27.9: use the same transaction-level dirty signal that Live already maintains.
...
/* ================= V27.9 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V27.9: Sales Cards and Daily Profit are mutually exclusive.
    ...
    // V27.9: saved cards first. Do not flash a fake 0.00 card while cloud data loads.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V27.9 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V27.9 button bindings reliable.
...
/* ================= V27.9 multi-product Sales Card ================= */
...
function buildProductSubItemV239(type,card,data={},order=1){
  ...
  [c,p].forEach(el=>el.addEventListener("blur",()=>{
    ...
    // V27.9: blur fires before the Save button click. Re-mark the card dirty here so
...
function bindSingleTapDeleteProductV254(button,handler){
  ...
  // V27.9: for touch/pen, pointerup is the one and only real activation.
...
系统会保留 saleId 的删除记录，让 Import Cost System 知道这张销售已经取消。`))return;
  try{
    ...
    // V27.9: refresh profit view from active backend links after card deletion.
    try{
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  // V27.9: Sales only reminds. Inventory confirmation is handled in Import Cost System.
  ...
  setTimeout(()=>{
    // V27.9: initialization may finish after a very fast first edit.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V27.9 sales card container missing",type);return false}
...
/* ================= V27.9 Sales Card unit-price + stable optimistic UI ================= */
...
async function saveProductLinksV206(type){
  ...
  // V27.9 optimistic local cache: keep the current UI stable; do not redraw from an older async response.
...
/* ================= V27.9 same-day linked bonsai profit summary ================= */
...
async function toggleProductProfitSummaryV216(type,button){
  ...
  try{
    // V27.9: opening Daily Profit closes Sales Cards first, then paints local cache immediately.
    const pre=productLinkPreV208(type),box=document.getElementById(pre+"ProductLinkBox"),body=document.getElementById(pre+"ProductLinkBody");
...
/* ================= V27.9 expandable yearly monthly breakdown ================= */
...
function yearBreakdownRowsV224(kind){
  // V27.9: Balakong / Belimbing / Fair / Live show months inside the selected
...
function renderAll(){
  // V27.9: one complete render path. This replaces the older partial duplicate
...
// V27.9: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V27.9: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V27.9: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
/* ================= V27.9 pre-change sales-card context guard ================= */
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V27.9: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V27.9: deleting the last special commission rule leaves candidate and
...
/* ================= V27.9 Sales Card transaction integrity / instant cache ================= */
...
/* ================= V27.9 Live optimistic revision guard ================= */
...
async function refreshOpenSalesChangeLogAfterSaveV243(type,date){
  ...
  }catch(e){console.warn("V27.9 修改/销售记录后台刷新失败",e)}
}
...
saveLiveSales=async function(){
  ...
  // V27.9 preflight: do not even mutate local turnover when saved cards exceed the new amount.
  try{
  ...
  // Critical V27.9 guard: any cloud request started before this local mutation
...
/* ================= V27.9 Profit list copy + compact totals ================= */
...
/* ================= V27.9 profit layout + weighted average margin ================= */
...
/* ================= V27.9 cross-date inventory reminders =================
...
function openImportCostSystemV266(target={}){
  // V27.9: Sales must stay open; Import always opens in a separate tab/window.
...
// V27.9: Sales-side inventory confirmation removed. Import is the only inventory authority.
...
// V27.9: after the user processes inventory in the separate Import tab/app,
