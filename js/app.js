...
function syncFairProductDatesV203(){const sel=document.getElementById("fairProductDate");if(!sel)return;const start=document.getElementById("fairStart")?.value,end=document.getElementById("fairEnd")?.value||start;if(!start||!end){sel.innerHTML="";if(typeof refreshSalesActionLocksV270==="function")refreshSalesActionLocksV270();return}const current=sel.value;const dates=dateRange(start,end);sel.innerHTML=dates.map(d=>`<option value="${d}">${d}</option>`).join("");if(dates.includes(current))sel.value=current;if(typeof refreshSalesActionLocksV270==="function")refreshSalesActionLocksV270();}
...
系统会保留 saleId 的删除记录，让 Import Cost System 知道这张销售已经取消。`))return;
  try{
    ...
    // V27.1: refresh profit view from active backend links after card deletion.
    try{
...
saveLiveSales=async function(){
  ...
  // V27.1 preflight: do not even mutate local turnover when saved cards exceed the new amount.
  try{
