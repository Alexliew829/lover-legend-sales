...
function turnoverLeaveGuardV382(type){
  if(turnoverWriteStateV382[type])return confirm('营业额正在储存中，尚未完成云端确认。\n\n确定仍要离开？');
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4540",restoreGeneration:0};
...
// V45.4: paint Home immediately from Local First cache, then perform one
...
// V45.4: startup performs only Local First + the lightweight global Revision
...
function buildCloudImportProductRecordsV214(data){
  ...
  function addRecord(source){
    ...
    records.push({
      ...
      // V45.4: prefer the current Products ID. Old cached/import PZ IDs still
...
// V45.4: Sales Card freshness is controlled by the global Revision gate.
...
// V45.4: a completed sale is a whole-card, permanent local fact. Keep it in
...
// V45.4: an authoritative cloud context may legitimately become empty after a
...
async function loadProductLinksIntoEditorV206(type){
  ...
  // V45.4 Local First + exact-context verification. Paint the last complete
  ...
  if(Array.isArray(cached)){
    ...
    // V45.4: when cloud revision advanced, an old durable pending draft must not
    ...
    try{
      ...
      // V45.4: exact-context refresh commits card + profit together from the SAME
...
function salesCardWriteReadyV449(){
  // V45.4: do not freeze unrelated work while cloud sync is slow. Existing
...
async function deleteSalesCardTransactionV239(type,txnId){
  if(!requireSalesCardWriteReadyV449())return;
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  const card=[...(wrap?.querySelectorAll(".sales-card-transaction-v239")||[])].find(x=>x.dataset.transactionId===txnId);
  if(!card)return;
...
系统会保留 saleId 的删除记录，让 Import Cost System 知道这张销售已经取消。`))return;
  try{
    if(saved.length){
      ...
      // V45.4: deletion is complete only after an authoritative one-request
      // bundle confirms BOTH the card snapshot and the month/profit state.
      const month=(typeof selectedMonth==='function'&&selectedMonth())||new Date().toISOString().slice(0,7);
      const bundle=await (typeof fetchSyncBundleV450==='function'?fetchSyncBundleV450(month,22000):window.fetchSyncBundleV450(month,22000));
      if(!bundle||!bundle.ok||!Array.isArray(bundle.salesProductLinks))throw new Error('删除后云端完整核对失败');
      if(typeof commitAllSalesCardsAtomicV449==='function')commitAllSalesCardsAtomicV449(bundle.salesProductLinks);
      if(typeof applyLocalDataRevision==='function')applyLocalDataRevision(bundle.dataRevision);
      if(bundle.salesCardRevision!==undefined&&typeof getPrioritySyncLocalV315==='function'&&typeof setPrioritySyncLocalV315==='function'){
        const p=getPrioritySyncLocalV315();setPrioritySyncLocalV315({...p,salesCardRevision:Number(bundle.salesCardRevision||0),turnoverRevision:Number(bundle.turnoverRevision||p.turnoverRevision||0),at:Date.now()});
      }
      try{clearDailyProfitCacheV237(type,productProfitSelectedDateV216(type));}catch(_){}
      if(productProfitSummaryOpenV216?.[type])renderProductProfitSummaryV216(type,bundle.salesProductLinks);
    }else{
      card.remove();
...
async function saveSingleSalesCardV405(type,txnId,mode,button){
  if(!requireSalesCardWriteReadyV449())return null;
...
function addProductLinkItemV209(type,data={}){
  ...
  // V45.4: New Sales Card is local-only and instant. If a first-time cloud
...
saveProductLinksV206=async function(type,saveMode='confirm',button=null){
  ...
  if(saveMode==='draft'){
    try{
      // V45.4: local safety copy is still written first, but “保存成功” is not
...
function renderInventoryPendingGlobalV250(){
  ["daily","live","fair"].forEach(pageType=>{
    ...
    const list=inventoryPendingCacheV250.filter(item=>{
      // V45.4: a draft card must never flash an Import-inventory reminder.
...
/* ================= V45.4 field drag/drop protection =================
...
/* ================= V45.4 once-per-day unconfirmed draft reminder =================
...
async function checkUnconfirmedDraftReminderV369(){
  ...
  try{
    ...
    // V45.4: only the explicit X button dismisses today's reminder. Leaving,
...
async function addTurnoverEntryV376(type){
  ...
  // V45.4: after an Apps Script timeout the optimistic entry intentionally
...
saveProductLinksV206=async function(type,...args){
  // V45.4 write barrier: viewing/editing locally is allowed, but a Sales Card
  // cannot be committed while this device has not confirmed the latest cloud
  // revision. This prevents stale Local First state overwriting another device.
  if(!requireSalesCardWriteReadyV449())return null;
...
renderAll=function(){
  ...
  try{renderTurnoverComposerV376('daily');renderTurnoverComposerV376('fair');renderTurnoverComposerV376('live')}catch(e){console.warn('V45.4 post-render turnover',e)}
  return result;
};
...
/* ================= V45.4 transaction UX + global drag/drop safety ================= */
...
/* ================= V45.4 system status + system information ================= */
...
function renderSystemInformationV442(data){
  ...
  set('systemInfoApiV442',data?.apiVersion?`V${String(data.apiVersion).replace(/^V/i,'').replace(/^4540$/,'45.4')}`:'连接异常');
...
async function runSystemHealthCheckV442(userTriggered=false){
  ...
  try{
    const data=await jsonp({action:'healthV443',clientVersion:'4540'},{timeoutMs:15000});
    if(!data?.ok)throw new Error(data?.message||'系统检查失败');
    const issues=[];
    if(String(data.apiVersion||'')!=='4540')issues.push(`Frontend / API 版本不一致（Frontend 4540 / API ${data.apiVersion||'未知'}）`);
    (Array.isArray(data.issues)?data.issues:[]).forEach(x=>issues.push(String(x)));
    const severe=Boolean(data.severe)||!data.sheetConnected||String(data.apiVersion||'')!=='4540';
