...
// V41.4: click a Home Fair location to expand its newest month, plus the
...
const fairHomeHistoryOpenV414=new Set();
let fairHomeProfitLinksV414=null;
let fairHomeProfitPromiseV414=null;
function fairCommissionRateForMonthV414(month){
...
function fairLocationHistoryRowsV414(locationKey){
  const map=new Map();
...
function fairHistoryProfitV414(links,locationKey,date){
...
function fairHistoryTableV414(locationKey,links){
  const list=fairLocationHistoryRowsV414(locationKey),ready=Array.isArray(links);
  if(!list.length)return '<div class="fair-home-history-empty-v414">还没有历史记录</div>';
  let totalSales=0,totalProfit=0,totalCommission=0;
  const body=list.map(r=>{
    const sales=Number(r.amount||0),month=displayToISO(r.date).slice(0,7),rate=fairCommissionRateForMonthV414(month),commission=sales*rate,profit=ready?fairHistoryProfitV414(links,locationKey,r.date):null,margin=profit===null?null:(sales>0?profit/sales*100:0);
    totalSales+=sales;totalCommission+=commission;if(profit!==null)totalProfit+=profit;
    return `<div class="fair-home-history-row-v414"><span>${shortDayMonthV297(r.date)}</span><b>${money(sales)}</b><b>${profit===null?'--':money(profit)}</b><b>${margin===null?'--':margin.toFixed(2)+'%'}</b><b>${money(commission)}</b></div>`;
  }).join('');
  const totalMargin=ready&&totalSales>0?totalProfit/totalSales*100:0;
  return `<div class="fair-home-history-table-v414"><div class="fair-home-history-head-v414"><span>日期</span><span>营业额</span><span>利润</span><span>利润率</span><span>佣金</span></div>${body}<div class="fair-home-history-row-v414 fair-home-history-total-v414"><span>总数</span><b>${money(totalSales)}</b><b>${ready?money(totalProfit):'--'}</b><b>${ready?totalMargin.toFixed(2)+'%':'--'}</b><b>${money(totalCommission)}</b></div></div>`;
}
function fairOpenLocationsGrandV414(locationKeys,links){
  const keys=[...new Set(locationKeys)].filter(Boolean),ready=Array.isArray(links);
  if(keys.length<2)return '';
  let totalSales=0,totalProfit=0,totalCommission=0;
  keys.forEach(key=>fairLocationHistoryRowsV414(key).forEach(r=>{
    const sales=Number(r.amount||0),month=displayToISO(r.date).slice(0,7),rate=fairCommissionRateForMonthV414(month);
    totalSales+=sales;totalCommission+=sales*rate;
    if(ready)totalProfit+=fairHistoryProfitV414(links,key,r.date);
  }));
  const margin=ready&&totalSales>0?totalProfit/totalSales*100:0;
  return `<div class="fair-home-grand-v414"><strong>${keys.length}个地点总数</strong><div class="fair-home-history-head-v414"><span></span><span>营业额</span><span>利润</span><span>利润率</span><span>佣金</span></div><div class="fair-home-history-row-v414 fair-home-history-total-v414"><span>总数</span><b>${money(totalSales)}</b><b>${ready?money(totalProfit):'--'}</b><b>${ready?margin.toFixed(2)+'%':'--'}</b><b>${money(totalCommission)}</b></div></div>`;
}
function paintFairHomeHistoryV414(){
  document.querySelectorAll('.fair-home-history-panel-v414[data-location-key]').forEach(panel=>{const key=decodeURIComponent(panel.dataset.locationKey||'');panel.innerHTML=fairHistoryTableV414(key,fairHomeProfitLinksV414)});
  const grand=document.getElementById('fairHomeGrandV414');if(grand){const keys=String(grand.dataset.locationKeys||'').split(',').filter(Boolean).map(decodeURIComponent);grand.outerHTML=fairOpenLocationsGrandV414(keys,fairHomeProfitLinksV414).replace('<div class="fair-home-grand-v414"',`<div id="fairHomeGrandV414" data-location-keys="${keys.map(encodeURIComponent).join(',')}" class="fair-home-grand-v414"`)}
}
function ensureFairHomeProfitLinksV414(){
  if(Array.isArray(fairHomeProfitLinksV414)){paintFairHomeHistoryV414();return Promise.resolve(fairHomeProfitLinksV414)}
  if(fairHomeProfitPromiseV414)return fairHomeProfitPromiseV414;
  fairHomeProfitPromiseV414=Promise.resolve(loadAllSalesProductLinksV203({force:false,maxAgeMs:180000})).then(links=>{fairHomeProfitLinksV414=Array.isArray(links)?links:[];paintFairHomeHistoryV414();return fairHomeProfitLinksV414}).catch(e=>{console.warn('V41.4 Fair 全部历史利润读取失败',e);return[]}).finally(()=>{fairHomeProfitPromiseV414=null});
  return fairHomeProfitPromiseV414;
}
function toggleFairHomeHistoryV414(encodedKey){
  const key=decodeURIComponent(String(encodedKey||''));if(!key)return;
  if(fairHomeHistoryOpenV414.has(key))fairHomeHistoryOpenV414.delete(key);else fairHomeHistoryOpenV414.add(key);
  renderFairLocationList();
  if(fairHomeHistoryOpenV414.has(key))ensureFairHomeProfitLinksV414();
}
window.toggleFairHomeHistoryV414=toggleFairHomeHistoryV414;
function renderFairLocationList(){
  ...
  const hasOpenLocation=allLocationKeys.some(key=>fairHomeHistoryOpenV414.has(key));
  const grand=hasOpenLocation?fairOpenLocationsGrandV414(allLocationKeys,fairHomeProfitLinksV414):'';
  const grandHtml=grand?grand.replace('<div class="fair-home-grand-v414"',`<div id="fairHomeGrandV414" data-location-keys="${allLocationKeys.map(encodeURIComponent).join(',')}" class="fair-home-grand-v414"`):'';
  c.innerHTML='<div class="fair-location-grid">'+locs.map(l=>{const key=normalizeFairLocationKey(l),open=fairHomeHistoryOpenV414.has(key),encoded=encodeURIComponent(key);return `<div class="fair-location-card fair-location-card-v414 ${open?'fair-location-card-open-v414':''}"><button type="button" class="fair-location-title fair-location-toggle-v414" aria-expanded="${open}" onclick="toggleFairHomeHistoryV414('${encoded}')"><span>${escapeChangeLogHtmlV200(l)}</span><b>${open?'▲':'▼'}</b></button><div class="fair-location-row"><span>营业额</span><b>${money(g[l])}</b></div><div class="fair-location-row"><span>佣金 ${Number(rate.toFixed(2))}%</span><b>${money(g[l]*rate/100)}</b></div>${open?`<div class="fair-home-history-panel-v414" data-location-key="${encoded}">${fairHistoryTableV414(key,fairHomeProfitLinksV414)}</div>`:''}</div>`}).join("")+grandHtml+'</div>';
}
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4140",restoreGeneration:0};
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  // V41.4: confirmation belongs to the sales-card transaction, not only to the
  ...
  // V41.4: this is the active Sales / Fair / Live card builder. Keep each
...
function renderRemoteRestoreMaintenanceV345(active,message=""){
  // V41.4: the device that started/resumed this Restore stays on its More
...
// V41.4: the persisted Restore job remains authoritative through the final
...
function markDraftSavedLocallyV314(type,ctx,dirty,items,dirtyIds){
  ...
  const local=items.map(x=>{
    ...
    let status='DRAFT';
    if(wasConfirmed){
      // V41.4: preserve each already-confirmed row, but a NEW product added to an
...
async function validateSalesInventoryAvailabilityV325(type,items,saveMode='draft'){
  ...
  // V41.4: keep the same authoritative Import oversell guard, but avoid the old
...
saveProductLinksV206=async function(type,saveMode='confirm',button=null){
  ...
  // V41.4 protects navigation from the instant Confirm is pressed.
  ...
  // V41.4: a draft is local/cloud sales-card data only. Import availability is
...
// V41.4: the pending endpoint is the smallest authoritative ACK check. When a
...
// V41.4: do not wait for the full Sales cloud-resume cycle. Import ACK is a
...
// V41.4 final navigation gate, installed after every legacy showPage wrapper.
