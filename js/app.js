...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4780",restoreGeneration:0};
...
async function startInitialSalesDataLoad() {
  ...
  startupSalesSyncPromise = (async () => {
    ...
    try{
      ...
      if(result?.ok&&!pending&&!(fg&&fg.active)&&typeof setSync==='function'){
        if(result?.revisionUnconfirmed)setSync('云端连接较慢 · 本机资料已保留');
        else setSync('已同步',true);
      }
    }catch(_){}
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    ...
    // V47.8: explicit Sales Card opening may run one silent safety verification
...
async function loadProductLinksIntoEditorV206(type,options={}){
  ...
  // V47.8: automatic date/host/page refreshes must never read Sales Card cloud data
  ...
  if(Array.isArray(cached)){
    ...
    const pendingKey=salesDraftPendingKeyV314(type,date,location);
    const pending=readSalesDraftPendingV314()[pendingKey];
    // V47.8: a cached Sales Card is Local First and is NOT re-read merely because
  ...
  try{
    ...
    if(SALES_CARD_LOAD_SEQ_V245[type]===seq&&currentKey===contextKey&&!productImportSearchIsActiveV226(type)){
      ...
      if(SALES_CARD_VERIFY_PENDING_V445[type]===contextKey)SALES_CARD_VERIFY_PENDING_V445[type]='';
      // V47.8: card view success does not declare turnover/context fully synced.
    }
    return links;
  }catch(e){
    const current=productLinkContextV206(type);
    ...
    if(SALES_CARD_LOAD_SEQ_V245[type]===seq&&currentKey===contextKey&&!productImportSearchIsActiveV226(type)){
      ...
      // V47.8: card-view read failure stays inside the card panel only.
    }
    return null;
  }
}
...
async function renderSelectedDayGrandV362(type){
  ...
  try{
    ...
    // V47.8: Profit display never owns the sync status. A cached/cloud Profit result
...
// V47.8: do not let the later V36.8 legacy all-Sales-Card renderer overwrite
...
function renderSystemInformationV442(data){
  ...
  set('systemInfoApiV442',data?.apiVersion?`V${String(data.apiVersion).replace(/^V/i,'').replace(/^4780$/,'47.8')}`:'连接异常');
...
async function runSystemHealthCheckV442(userTriggered=false){
  ...
  try{
    const data=await jsonp({action:'healthV443',clientVersion:'4780'},{timeoutMs:15000});
    if(!data?.ok)throw new Error(data?.message||'系统检查失败');
    const issues=[];
    if(String(data.apiVersion||'')!=='4780')issues.push(`Frontend / API 版本不一致（Frontend 4780 / API ${data.apiVersion||'未知'}）`);
    (Array.isArray(data.issues)?data.issues:[]).forEach(x=>issues.push(String(x)));
    const severe=Boolean(data.severe)||!data.sheetConnected||String(data.apiVersion||'')!=='4780';
