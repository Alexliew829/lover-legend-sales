...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"4460",restoreGeneration:0};
...
// V44.6: paint Home immediately from Local First cache, then perform one
...
// V44.6: startup performs only Local First + the lightweight global Revision
...
function buildCloudImportProductRecordsV214(data){
  ...
  function addRecord(source){
    ...
    records.push({
      ...
      // V44.6: prefer the current Products ID. Old cached/import PZ IDs still
...
// V44.6: Sales Card freshness is controlled by the global Revision gate.
...
// V44.6: a completed sale is a whole-card, permanent local fact. Keep it in
...
async function loadProductLinksIntoEditorV206(type){
  ...
  // V44.6 Local First: cached Sales Cards open instantly and are NOT re-read
...
function addProductLinkItemV209(type,data={}){
  ...
  // V44.6: New Sales Card is local-only and instant. If a first-time cloud
...
function renderInventoryPendingGlobalV250(){
  ["daily","live","fair"].forEach(pageType=>{
    ...
    const list=inventoryPendingCacheV250.filter(item=>{
      // V44.6: a draft card must never flash an Import-inventory reminder.
...
/* ================= V44.6 once-per-day unconfirmed draft reminder =================
...
async function checkUnconfirmedDraftReminderV369(){
  ...
  try{
    ...
    // V44.6: only the explicit X button dismisses today's reminder. Leaving,
...
async function addTurnoverEntryV376(type){
  ...
  // V44.6: after an Apps Script timeout the optimistic entry intentionally
...
renderAll=function(){
  ...
  try{renderTurnoverComposerV376('daily');renderTurnoverComposerV376('fair');renderTurnoverComposerV376('live')}catch(e){console.warn('V44.6 post-render turnover',e)}
  return result;
};
...
/* ================= V44.6 system status + system information ================= */
...
function renderSystemInformationV442(data){
  ...
  set('systemInfoApiV442',data?.apiVersion?`V${String(data.apiVersion).replace(/^V/i,'').replace(/^4460$/,'44.6')}`:'连接异常');
...
async function runSystemHealthCheckV442(userTriggered=false){
  ...
  try{
    const data=await jsonp({action:'healthV443',clientVersion:'4460'},{timeoutMs:15000});
    if(!data?.ok)throw new Error(data?.message||'系统检查失败');
    const issues=[];
    if(String(data.apiVersion||'')!=='4460')issues.push(`Frontend / API 版本不一致（Frontend 4460 / API ${data.apiVersion||'未知'}）`);
    (Array.isArray(data.issues)?data.issues:[]).forEach(x=>issues.push(String(x)));
    const severe=Boolean(data.severe)||!data.sheetConnected||String(data.apiVersion||'')!=='4460';
