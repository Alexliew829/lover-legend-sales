...
function showPage(name,el){
  ...
  // V26.3: every time Live is opened, start from today's date.
  ...
  // V26.3: page switching never waits for or triggers cloud sync.
...
// V26.3: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V26.3: historical record is lazy. Top 5 opens instantly from local rows;
...
function renderTodayCompanyStatus(){
  ...
  // V26.3: status colour represents actual business amount, not merely
...
function applyCloudCommissionSettings(settings){
  ...
  // V26.3: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V26.3: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V26.3: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V26.3: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V26.3: local Fair values are direct replacements, never additions. The server
...
// V26.3: paint Home immediately, restore local cache, then perform only a
...
// V26.3: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V26.3: active host list is independent from historical Live records.
...
function restoreLastLiveSession(){
  ...
  // V26.3: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
/* ================= V26.3 Import Cost System product search ================= */
...
function unorderedImportProductMatchV214(sourceValue,queryValue){
  ...
  if(isExactProductCodeQueryV214(query)){
    // V26.3: code search supports case-insensitive prefix/partial entry.
...
function setupImportProductSearchV214(item,nameInput,resultsBox,closeButton){
  // V26.3: iPhone/iOS safe search state.
  ...
  const render=async()=>{
    ...
    try{
      ...
      matches.forEach(record=>{
        ...
        // V26.3: distinguish an intentional tap from list scrolling on iPhone/iOS.
  ...
  if(closeButton){
    ...
    closeButton.addEventListener("click",e=>{
      ...
      // V26.3: X means CLEAR CURRENT PRODUCT SELECTION.
...
/* ================= V26.3 Sales Card unsaved-change protection ================= */
...
/* ================= V26.3 optional bonsai product association ================= */
...
function toggleProductLinkBoxV206(type){
  ...
  if(opening){
    // V26.3: Sales Cards and Daily Profit are mutually exclusive.
    ...
    // V26.3: saved cards first. Do not flash a fake 0.00 card while cloud data loads.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V26.3 product item container missing",type);return false}
...
// Explicit globals keep both legacy and V26.3 button bindings reliable.
...
/* ================= V26.3 multi-product Sales Card ================= */
...
function buildProductSubItemV239(type,card,data={},order=1){
  ...
  [c,p].forEach(el=>el.addEventListener("blur",()=>{
    ...
    // V26.3: blur fires before the Save button click. Re-mark the card dirty here so
...
function bindSingleTapDeleteProductV254(button,handler){
  ...
  // V26.3: for touch/pen, pointerup is the one and only real activation.
...
function buildSalesCardTransactionV239(type,dataList=[]){
  ...
  setTimeout(()=>{
    // V26.3: initialization may finish after a very fast first edit.
...
function addProductLinkItemV209(type,data={}){
  const pre=productLinkPreV208(type),wrap=document.getElementById(pre+"ProductItems");
  if(!wrap){console.error("V26.3 sales card container missing",type);return false}
...
/* ================= V26.3 Sales Card unit-price + stable optimistic UI ================= */
...
async function saveProductLinksV206(type){
  ...
  // V26.3 optimistic local cache: keep the current UI stable; do not redraw from an older async response.
...
/* ================= V26.3 same-day linked bonsai profit summary ================= */
...
async function toggleProductProfitSummaryV216(type,button){
  ...
  try{
    // V26.3: opening Daily Profit closes Sales Cards first, then paints local cache immediately.
    const pre=productLinkPreV208(type),box=document.getElementById(pre+"ProductLinkBox"),body=document.getElementById(pre+"ProductLinkBody");
...
/* ================= V26.3 expandable yearly monthly breakdown ================= */
...
function yearBreakdownRowsV224(kind){
  // V26.3: Balakong / Belimbing / Fair / Live show months inside the selected
...
function renderAll(){
  // V26.3: one complete render path. This replaces the older partial duplicate
...
// V26.3: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V26.3: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V26.3: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V26.3: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V26.3: deleting the last special commission rule leaves candidate and
...
/* ================= V26.3 Sales Card transaction integrity / instant cache ================= */
...
/* ================= V26.3 Live optimistic revision guard ================= */
...
async function refreshOpenSalesChangeLogAfterSaveV243(type,date){
  ...
  }catch(e){console.warn("V26.3 修改/销售记录后台刷新失败",e)}
}
...
saveLiveSales=async function(){
  ...
  // Critical V26.3 guard: any cloud request started before this local mutation
...
/* ================= V26.3 Profit list copy + compact totals ================= */
...
/* ================= V26.3 profit layout + weighted average margin ================= */
...
/* ================= V26.3 cross-date inventory reminders =================
...
function renderInventoryPendingGlobalV250(){
  ["live","fair"].forEach(pageType=>{
    ...
    const list=inventoryPendingCacheV250.filter(item=>String(item.type||"").toLowerCase()===pageType);
    ...
    box.innerHTML=`<div class="inventory-global-head-v250"><b>⚠️ 有 ${list.length} 项库存待处理</b><span>请到 Import Cost System 手动修改库存</span></div>`+
      [...groups.values()].map(group=>{
        const first=group[0],old=inventoryPendingDaysOldV250(first.date),stale=old>=1;
        return `<div class="inventory-global-item-v250${stale?" stale-v250":""}">
          <div class="inventory-global-main-v250">
            <b>${stale?"⚠️ ":""}${escapeChangeLogHtmlV200(first.date)} · ${first.type==="live"?"Live":"Fair"} · ${escapeChangeLogHtmlV200(first.location)}</b>
            ${group.map(item=>{
              const name=String(item.productName||"").trim(),qty=Math.max(1,Number(item.quantity||1));
              return `<div class="inventory-product-row-v263">
...
async function confirmPendingInventoryV250(raw,btn){
  ...
  if(!linkId){
    alert("这项产品缺少 Link ID，无法独立确认。请先更新 Apps Script / Code.gs 到 V26.3 后重新部署。");
    return;
  }
  if(!confirm(`请确认：你已经在 Import Cost System 手动修改以下库存。\n\n${item.date} · ${item.type==="live"?"Live":"Fair"} · ${item.location}\n${inventoryPendingProductTextV250(item)}\n\n确认后，只会停止提醒这一项产品。`))return;
  try{
    if(btn){btn.disabled=true;btn.textContent="确认中…"}
    await confirmSalesCardInventoryV249({type:item.type,date:item.date,location:item.location,transactionId:item.transactionId,linkId});
    inventoryPendingCacheV250=inventoryPendingCacheV250.filter(x=>String(x.linkId||"")!==linkId);
