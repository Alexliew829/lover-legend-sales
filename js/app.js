...
function showPage(name,el){
  ...
  // V20.1: every time Live is opened, start from today's date.
  ...
  // V20.1: page switching never waits for or triggers cloud sync.
...
// V20.1: Top 5 business performance. Uses rows already loaded in memory only;
...
function toggleTop3(id,btn){
  ...
  if(show){
    ...
    // V20.1: historical record is lazy. Top 5 opens instantly from local rows;
...
function applyCloudCommissionSettings(settings){
  ...
  // V20.1: Fair and Live each have their own revision.
...
function getCommissionSettingsForMonth(month){
  ...
  // V20.1: historical Fair rates come from that month's snapshot, while the
...
以后在 Live 再输入同一名字，会自动复职并对应原历史资料。`))return;
  ...
  try{
    ...
    // V20.1: timeout must not undo the user's local action.
...
async function saveDailySales(){
  ...
  // V20.1: normal Save uses exactly one cloud write.
...
async function saveFairSales(){const fairLocationValue=String(document.getElementById("fairLocation")?.value||"").trim();if(!fairLocationValue){alert("请先输入 Fair 地点");return;}
  ...
  try{
    ...
    // V20.1: local Fair values are direct replacements, never additions. The server
...
let systemState={currentMonth:monthISO(),closedMonths:[],commissionSnapshots:{},dataVersion:"2001"};
...
function applySystemState(state){if(state){systemState.currentMonth=state.currentMonth||monthISO();systemState.closedMonths=sanitizeClosedMonthsClientV197(state.closedMonths,systemState.currentMonth);systemState.commissionSnapshots=state.commissionSnapshots||{};systemState.dataVersion=state.dataVersion||"2001"}updateReadOnlyMode()}
...
// V20.1: paint Home immediately, restore local cache, then perform only a
...
// V20.1: start cached Home immediately, then warm the current year's historical
...
function collectLiveHosts(){
  ...
  // V20.1: active host list is independent from historical Live records.
...
function renderChangeLogTimelineV200(type,date,items){
  ...
  // V20.1 three cases remain unchanged:
  // 1) only one first/original save (0 -> amount): no add/modify record.
  // 2) once a later change exists, show the first/original amount as part of the full timeline.
  ...
  const changedGroups=groups.filter(group=>{
    ...
    if(ordered.length===1){
      ...
      return Number(first.oldAmount||0)>0; // pre-audit amount modified after upgrade
    }
    return true;
  });
...
function restoreLastLiveSession(){
  ...
  // V20.1: do not restore the previously saved date.
  setDateControl("liveDate",todayISO());
  updateLiveInputFromSelectedDate();
}
...
function renderAll(){
  // V20.1: one complete render path. This replaces the older partial duplicate
...
// V20.1: expandable daily total list. It uses cached rows immediately and only
...
async function loadDailyTotalsMonth(month){
  ...
  // V20.1: current month already follows the normal Home sync flow.
...
async function toggleMonthlySummary(force){
  ...
  // V20.1: show cache immediately and complete historical months in background.
  renderMonthlySummary();
...
async function saveFairCommissionSettings(){
  ...
  try{
    ...
    // V20.1: save locally immediately. Do not make the user wait for Apps Script.
...
async function saveLiveCommissionSettings(){
  ...
  try{
    ...
    // V20.1: deleting the last special commission rule leaves candidate and
...
function backupAllData(){
  try{
    ...
    downloadFile(`Lover_Legend_Sales_V20_1_Backup_${stamp}.json`,JSON.stringify(payload,null,2),"application/json;charset=utf-8");
  }catch(e){
