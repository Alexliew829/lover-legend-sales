...
// V39.7: pending rows are durable retry instructions, not proof that the cloud
...
async function loadFromSheet(options = {}) {
  ...
  cloudLoadPromise = (async () => {
    ...
    try {
      ...
      // V39.7: never let the startup sync terminate at the lightweight revision
      ...
      // V39.7: verify durable pending rows against their own authoritative
...
async function syncPendingRows() {
  ...
  try {
    ...
    // V39.7: every retry path (startup, timer, focus, manual recovery) first
...
async function sendFairBatchToSheetV343(location, records, foregroundSave=false) {
  ...
  const json = await jsonp({
    ...
    // V39.7: interactive Fair saves must return the cloud ACK before any push work.
  ...
  applyLocalDataRevision(json.dataRevision);
  if(json.turnoverRevision!==undefined){const p=getPrioritySyncLocalV315();setPrioritySyncLocalV315({...p,turnoverRevision:Number(json.turnoverRevision||0),at:Date.now()})}
  // V39.7 foreground Fair saves return the cloud ACK first; push is dispatched asynchronously afterward.
...
/* ================= V39.7 Fair/Live turnover entry details ================= */
