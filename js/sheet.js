...
// V37.7: pending rows are durable retry instructions, not proof that the cloud
...
async function loadFromSheet(options = {}) {
  ...
  cloudLoadPromise = (async () => {
    ...
    try {
      ...
      // V37.7: never let the startup sync terminate at the lightweight revision
      ...
      // V37.7: verify durable pending rows against their own authoritative
...
async function syncPendingRows() {
  ...
  try {
    ...
    // V37.7: every retry path (startup, timer, focus, manual recovery) first
...
/* ================= V37.7 Fair/Live turnover entry details ================= */
