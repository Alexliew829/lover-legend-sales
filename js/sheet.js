...
// V21.3: notification dispatch uses the existing keepalive transport.
...
/* V21.3: first paint must not wait for the full system render. */
let localCacheRenderedOnce = false;
let deferredFullRenderTimer = null;

function renderHomeFirst() {
  // V21.3: first paint must stay lightweight. Cloud merge performs dedupe later.
...
function loadLocalDataCache() {
  try {
    ...
    // V21.3: damaged/partial cache must never trap startup.
...
// V21.3: best-effort immediate cloud dispatch for mobile saves.
...
async function loadYearInBackground(year) {
  ...
  const task = (async () => {
    ...
    try {
      ...
      // V21.3: if Fair is currently open, repaint its date inputs from the
...
async function loadFromSheet(options = {}) {
  ...
  cloudLoadPromise = (async () => {
    ...
    try {
      ...
      // V21.3: opening/resuming first checks one tiny revision value.
      ...
      if (!force && hasLocalData && options.skipRevisionCheck !== true) {
        try {
          ...
          // V21.3: when local data exists, a slow/failed revision check must not
      ...
      renderHomeFirst();
      scheduleDeferredFullRender(0);
      // V21.3: keep Fair's visible daily amount inputs consistent with rows after
      ...
      // V21.3 mobile performance: startup loads only the selected month.
...
async function loadSalesProductLinksV206(type,date,location) {
  const json = await jsonp({ action:"getSalesProductLinks", type, date, location }, { timeoutMs:20000 });
...
async function loadAllSalesProductLinksV203() {
  const json = await jsonp({ action:"getAllSalesProductLinks" }, { timeoutMs:20000 });
  if (!json.ok) throw new Error(json.message || "读取盆栽关联资料失败");
  return Array.isArray(json.links) ? json.links : [];
}
...
async function restoreBackupToSheet(payload){
  ...
  let result=await jsonp({action:"restoreBegin",restoreId:id,totalChunks:total},{timeoutMs:30000});if(!result.ok)throw new Error(result.message||"无法开始恢复");
  for(let i=0;i<total;i++){result=await jsonp({action:"restoreChunk",restoreId:id,index:i,data:raw.slice(i*chunkSize,(i+1)*chunkSize)},{timeoutMs:30000});if(!result.ok)throw new Error(result.message||`恢复区块 ${i+1} 失败`);setSync(`正在恢复 Backup ${i+1}/${total}...`)}
  setSync("正在写回 Google Sheet，请勿关闭页面…");
  result=await jsonp({action:"restoreCommit",restoreId:id},{timeoutMs:180000});if(!result.ok)throw new Error(result.message||"恢复失败");return result
}
