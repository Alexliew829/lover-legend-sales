...
// V30.3: notification dispatch uses the existing keepalive transport.
...
/* V30.3: first paint must not wait for the full system render. */
let localCacheRenderedOnce = false;
let deferredFullRenderTimer = null;

function renderHomeFirst() {
  // V30.3: first paint must stay lightweight. Cloud merge performs dedupe later.
...
function loadLocalDataCache() {
  try {
    ...
    // V30.3: damaged/partial cache must never trap startup.
...
// V30.3: best-effort immediate cloud dispatch for mobile saves.
...
async function loadYearInBackground(year) {
  ...
  const task = (async () => {
    ...
    try {
      ...
      // V30.3: if Fair is currently open, repaint its date inputs from the
...
async function loadFromSheet(options = {}) {
  ...
  cloudLoadPromise = (async () => {
    ...
    try {
      ...
      // V30.3: opening/resuming first checks one tiny revision value.
      ...
      if (!force && hasLocalData && options.skipRevisionCheck !== true) {
        try {
          ...
          // V30.3: when local data exists, a slow/failed revision check must not
      ...
      renderHomeFirst();
      scheduleDeferredFullRender(0);
      // V30.3: keep Fair's visible daily amount inputs consistent with rows after
      ...
      // V30.3 mobile performance: startup loads only the selected month.
...
async function syncPendingRows() {
  ...
  try {
    ...
    // V30.3: send all queued Sales/Fair/Live turnover rows in ONE Apps Script
    ...
    console.warn("V30.3 batch pending sync delayed",err);
...
/* ================= V30.3 Profit / Change Log persistent cache ================= */
...
// V30.3: queued writes are already dispatched immediately after each save.
...
// V30.3 stable API alias: UI save function must never shadow the transport function.
async function deleteSalesTransactionV256(saleId){
  ...
  // V30.3: deleted cards must disappear from profit/editor caches immediately.
  salesProductLinksCacheV216.clear();
  allSalesProductLinksCacheV216={links:null,at:0};
  clearDailyProfitCacheV237();
  clearSalesCardPersistentCacheV232();
