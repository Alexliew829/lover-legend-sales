...
// V26.2: notification dispatch uses the existing keepalive transport.
...
/* V26.2: first paint must not wait for the full system render. */
let localCacheRenderedOnce = false;
let deferredFullRenderTimer = null;

function renderHomeFirst() {
  // V26.2: first paint must stay lightweight. Cloud merge performs dedupe later.
...
function loadLocalDataCache() {
  try {
    ...
    // V26.2: damaged/partial cache must never trap startup.
...
// V26.2: best-effort immediate cloud dispatch for mobile saves.
...
async function loadYearInBackground(year) {
  ...
  const task = (async () => {
    ...
    try {
      ...
      // V26.2: if Fair is currently open, repaint its date inputs from the
...
async function loadFromSheet(options = {}) {
  ...
  cloudLoadPromise = (async () => {
    ...
    try {
      ...
      // V26.2: opening/resuming first checks one tiny revision value.
      ...
      if (!force && hasLocalData && options.skipRevisionCheck !== true) {
        try {
          ...
          // V26.2: when local data exists, a slow/failed revision check must not
      ...
      renderHomeFirst();
      scheduleDeferredFullRender(0);
      // V26.2: keep Fair's visible daily amount inputs consistent with rows after
      ...
      // V26.2 mobile performance: startup loads only the selected month.
...
/* ================= V26.2 Profit / Change Log persistent cache ================= */
...
// V26.2 stable API alias: UI save function must never shadow the transport function.
