...
// V29.3: notification dispatch uses the existing keepalive transport.
...
/* V29.3: first paint must not wait for the full system render. */
let localCacheRenderedOnce = false;
let deferredFullRenderTimer = null;

function renderHomeFirst() {
  // V29.3: first paint must stay lightweight. Cloud merge performs dedupe later.
...
function loadLocalDataCache() {
  try {
    ...
    // V29.3: damaged/partial cache must never trap startup.
...
// V29.3: best-effort immediate cloud dispatch for mobile saves.
...
async function loadYearInBackground(year) {
  ...
  const task = (async () => {
    ...
    try {
      ...
      // V29.3: if Fair is currently open, repaint its date inputs from the
...
async function loadFromSheet(options = {}) {
  ...
  cloudLoadPromise = (async () => {
    ...
    try {
      ...
      // V29.3: opening/resuming first checks one tiny revision value.
      ...
      if (!force && hasLocalData && options.skipRevisionCheck !== true) {
        try {
          ...
          // V29.3: when local data exists, a slow/failed revision check must not
      ...
      renderHomeFirst();
      scheduleDeferredFullRender(0);
      // V29.3: keep Fair's visible daily amount inputs consistent with rows after
      ...
      // V29.3 mobile performance: startup loads only the selected month.
...
async function confirmSalesTransactionsV290(saleIds){const json=await jsonp({action:"confirmSalesTransactionsV290",saleIdsJson:JSON.stringify(saleIds||[])},{timeoutMs:20000});if(!json.ok)throw new Error(json.message||json.error||"确认销售失败");salesProductLinksCacheV216.clear();allSalesProductLinksCacheV216={links:null,at:0};clearSalesCardPersistentCacheV232();return json}
async function completeManualInventoryCorrectionV290(saleId){const json=await jsonp({action:"completeManualInventoryCorrectionV290",saleId:String(saleId||"")},{timeoutMs:20000});if(!json.ok)throw new Error(json.message||json.error||"库存修正状态保存失败");salesProductLinksCacheV216.clear();allSalesProductLinksCacheV216={links:null,at:0};clearSalesCardPersistentCacheV232();return json}
window.confirmSalesTransactionsV290=confirmSalesTransactionsV290;window.completeManualInventoryCorrectionV290=completeManualInventoryCorrectionV290;



/* ================= V29.3 Profit / Change Log persistent cache ================= */
...
// V29.3 stable API alias: UI save function must never shadow the transport function.
async function deleteSalesTransactionV256(saleId){
  ...
  // V29.3: deleted cards must disappear from profit/editor caches immediately.
  salesProductLinksCacheV216.clear();
  allSalesProductLinksCacheV216={links:null,at:0};
  clearDailyProfitCacheV237();
  clearSalesCardPersistentCacheV232();
