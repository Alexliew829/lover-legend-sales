...
// V28.0: notification dispatch uses the existing keepalive transport.
...
/* V28.0: first paint must not wait for the full system render. */
let localCacheRenderedOnce = false;
let deferredFullRenderTimer = null;

function renderHomeFirst() {
  // V28.0: first paint must stay lightweight. Cloud merge performs dedupe later.
...
function loadLocalDataCache() {
  try {
    ...
    // V28.0: damaged/partial cache must never trap startup.
...
// V28.0: best-effort immediate cloud dispatch for mobile saves.
...
async function loadYearInBackground(year) {
  ...
  const task = (async () => {
    ...
    try {
      ...
      // V28.0: if Fair is currently open, repaint its date inputs from the
...
async function loadFromSheet(options = {}) {
  ...
  cloudLoadPromise = (async () => {
    ...
    try {
      ...
      // V28.0: opening/resuming first checks one tiny revision value.
      ...
      if (!force && hasLocalData && options.skipRevisionCheck !== true) {
        try {
          ...
          // V28.0: when local data exists, a slow/failed revision check must not
      ...
      renderHomeFirst();
      scheduleDeferredFullRender(0);
      // V28.0: keep Fair's visible daily amount inputs consistent with rows after
      ...
      // V28.0 mobile performance: startup loads only the selected month.
...
/* ================= V28.0 Profit / Change Log persistent cache ================= */
...
// V28.0 stable API alias: UI save function must never shadow the transport function.
async function deleteSalesTransactionV256(saleId){
  ...
  // V28.0: deleted cards must disappear from profit/editor caches immediately.
  salesProductLinksCacheV216.clear();
  allSalesProductLinksCacheV216={links:null,at:0};
  clearDailyProfitCacheV237();
  clearSalesCardPersistentCacheV232();
