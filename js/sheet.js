...
// V27.7: notification dispatch uses the existing keepalive transport.
...
/* V27.7: first paint must not wait for the full system render. */
let localCacheRenderedOnce = false;
let deferredFullRenderTimer = null;

function renderHomeFirst() {
  // V27.7: first paint must stay lightweight. Cloud merge performs dedupe later.
...
function loadLocalDataCache() {
  try {
    ...
    // V27.7: damaged/partial cache must never trap startup.
...
// V27.7: best-effort immediate cloud dispatch for mobile saves.
...
async function loadYearInBackground(year) {
  ...
  const task = (async () => {
    ...
    try {
      ...
      // V27.7: if Fair is currently open, repaint its date inputs from the
...
async function loadFromSheet(options = {}) {
  ...
  cloudLoadPromise = (async () => {
    ...
    try {
      ...
      // V27.7: opening/resuming first checks one tiny revision value.
      ...
      if (!force && hasLocalData && options.skipRevisionCheck !== true) {
        try {
          ...
          // V27.7: when local data exists, a slow/failed revision check must not
      ...
      renderHomeFirst();
      scheduleDeferredFullRender(0);
      // V27.7: keep Fair's visible daily amount inputs consistent with rows after
      ...
      // V27.7 mobile performance: startup loads only the selected month.
...
/* ================= V27.7 Profit / Change Log persistent cache ================= */
...
// V27.7 stable API alias: UI save function must never shadow the transport function.
async function deleteSalesTransactionV256(saleId){
  ...
  // V27.7: deleted cards must disappear from profit/editor caches immediately.
  salesProductLinksCacheV216.clear();
  allSalesProductLinksCacheV216={links:null,at:0};
  clearDailyProfitCacheV237();
  clearSalesCardPersistentCacheV232();
