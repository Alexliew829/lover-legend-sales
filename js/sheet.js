...
// V38.2: pending rows are durable retry instructions, not proof that the cloud
...
async function loadFromSheet(options = {}) {
  ...
  cloudLoadPromise = (async () => {
    ...
    try {
      ...
      // V38.2: never let the startup sync terminate at the lightweight revision
      ...
      // V38.2: verify durable pending rows against their own authoritative
...
async function syncPendingRows() {
  ...
  try {
    ...
    // V38.2: every retry path (startup, timer, focus, manual recovery) first
...
/* ================= V38.2 Fair/Live turnover entry details ================= */
