...
// V18.3: notifications use the same proven JSONP transport as business saves.
...
function dispatchSalesNotificationAsync(envelope){
  if(!envelope||!envelope.payload||!envelope.signature){
    setLastNotificationDispatchStatus({ok:true,skipped:true,message:"没有营业额变化，无需通知"});
    return;
  }
...
/* V18.3: first paint must not wait for the full system render. */
let localCacheRenderedOnce = false;
let deferredFullRenderTimer = null;

function renderHomeFirst() {
  // V18.3: first paint must stay lightweight. Cloud merge performs dedupe later.
...
function loadLocalDataCache() {
  try {
    ...
    // V18.3: damaged/partial cache must never trap startup.
...
// V18.3: best-effort immediate cloud dispatch for mobile saves.
...
async function loadYearInBackground(year) {
  ...
  const task = (async () => {
    ...
    try {
      ...
      // V18.3: if Fair is currently open, repaint its date inputs from the
...
async function loadFromSheet(options = {}) {
  ...
  cloudLoadPromise = (async () => {
    ...
    try {
      ...
      // V18.3: opening/resuming first checks one tiny revision value.
      ...
      if (!force && hasLocalData && options.skipRevisionCheck !== true) {
        try {
          ...
          // V18.3: when local data exists, a slow/failed revision check must not
      ...
      renderHomeFirst();
      scheduleDeferredFullRender(0);
      // V18.3: keep Fair's visible daily amount inputs consistent with rows after
      ...
      // V18.3 mobile performance: startup loads only the selected month.
