...
// V29.4: notification dispatch uses the existing keepalive transport.
...
/* V29.4: first paint must not wait for the full system render. */
let localCacheRenderedOnce = false;
let deferredFullRenderTimer = null;

function renderHomeFirst() {
  // V29.4: first paint must stay lightweight. Cloud merge performs dedupe later.
...
function loadLocalDataCache() {
  try {
    ...
    // V29.4: damaged/partial cache must never trap startup.
...
// V29.4: best-effort immediate cloud dispatch for mobile saves.
...
async function loadYearInBackground(year) {
  ...
  const task = (async () => {
    ...
    try {
      ...
      // V29.4: if Fair is currently open, repaint its date inputs from the
...
async function loadFromSheet(options = {}) {
  ...
  cloudLoadPromise = (async () => {
    ...
    try {
      ...
      // V29.4: opening/resuming first checks one tiny revision value.
      ...
      if (!force && hasLocalData && options.skipRevisionCheck !== true) {
        try {
          ...
          // V29.4: when local data exists, a slow/failed revision check must not
      ...
      renderHomeFirst();
      scheduleDeferredFullRender(0);
      // V29.4: keep Fair's visible daily amount inputs consistent with rows after
      ...
      // V29.4 mobile performance: startup loads only the selected month.
...
/* ================= V29.4 Profit / Change Log persistent cache ================= */
...
// V29.4 stable API alias: UI save function must never shadow the transport function.
async function deleteSalesTransactionV256(saleId){
  ...
  // V29.4: deleted cards must disappear from profit/editor caches immediately.
  salesProductLinksCacheV216.clear();
  allSalesProductLinksCacheV216={links:null,at:0};
  clearDailyProfitCacheV237();
  clearSalesCardPersistentCacheV232();
