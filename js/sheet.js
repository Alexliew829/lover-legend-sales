...
// V38.0: pending rows are durable retry instructions, not proof that the cloud
...
async function loadFromSheet(options = {}) {
  ...
  cloudLoadPromise = (async () => {
    ...
    try {
      ...
      // V38.0: never let the startup sync terminate at the lightweight revision
      ...
      // V38.0: verify durable pending rows against their own authoritative
...
async function syncPendingRows() {
  ...
  try {
    ...
    // V38.0: every retry path (startup, timer, focus, manual recovery) first
...
/* ================= V38.0 Fair/Live turnover entry details ================= */
