...
// V21.1: notification dispatch uses the existing keepalive transport.
...
/* V21.1: first paint must not wait for the full system render. */
let localCacheRenderedOnce = false;
let deferredFullRenderTimer = null;

function renderHomeFirst() {
  // V21.1: first paint must stay lightweight. Cloud merge performs dedupe later.
...
function loadLocalDataCache() {
  try {
    ...
    // V21.1: damaged/partial cache must never trap startup.
...
// V21.1: best-effort immediate cloud dispatch for mobile saves.
...
async function loadYearInBackground(year) {
  ...
  const task = (async () => {
    ...
    try {
      ...
      // V21.1: if Fair is currently open, repaint its date inputs from the
...
async function loadFromSheet(options = {}) {
  ...
  cloudLoadPromise = (async () => {
    ...
    try {
      ...
      // V21.1: opening/resuming first checks one tiny revision value.
      ...
      if (!force && hasLocalData && options.skipRevisionCheck !== true) {
        try {
          ...
          // V21.1: when local data exists, a slow/failed revision check must not
      ...
      renderHomeFirst();
      scheduleDeferredFullRender(0);
      // V21.1: keep Fair's visible daily amount inputs consistent with rows after
      ...
      // V21.1 mobile performance: startup loads only the selected month.
