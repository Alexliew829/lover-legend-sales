...
// V30.1: notification dispatch uses the existing keepalive transport.
...
/* V30.1: first paint must not wait for the full system render. */
let localCacheRenderedOnce = false;
let deferredFullRenderTimer = null;

function renderHomeFirst() {
  // V30.1: first paint must stay lightweight. Cloud merge performs dedupe later.
...
function loadLocalDataCache() {
  try {
    ...
    // V30.1: damaged/partial cache must never trap startup.
...
// V30.1: best-effort immediate cloud dispatch for mobile saves.
...
async function loadYearInBackground(year) {
  ...
  const task = (async () => {
    ...
    try {
      ...
      // V30.1: if Fair is currently open, repaint its date inputs from the
...
async function loadFromSheet(options = {}) {
  ...
  cloudLoadPromise = (async () => {
    ...
    try {
      ...
      // V30.1: opening/resuming first checks one tiny revision value.
      ...
      if (!force && hasLocalData && options.skipRevisionCheck !== true) {
        try {
          ...
          // V30.1: when local data exists, a slow/failed revision check must not
      ...
      renderHomeFirst();
      scheduleDeferredFullRender(0);
      // V30.1: keep Fair's visible daily amount inputs consistent with rows after
      ...
      // V30.1 mobile performance: startup loads only the selected month.
...
async function savePendingRowsBatchToSheetV300(records){
  const json=await jsonp({
    action:"savePendingRowsBatchV300",
    rows:JSON.stringify(Array.isArray(records)?records:[])
  },{timeoutMs:20000});
  if(!json.ok)throw new Error(json.message||"批量同步失败");
  applyLocalDataRevision(json.dataRevision);
...
async function syncPendingRows() {
  ...
  try {
    loadPendingRows();
    if (pendingRows.length === 0) {
      if (initialCloudSyncFinished && !cloudLoadPromise) setSync("已同步", true);
      return;
    }

    // V30.1: send all queued Sales/Fair/Live turnover rows in ONE Apps Script
    // request. Older versions sent Daily/Live one-by-one, which made a queue of
    // 10 records visibly crawl through repeated network round-trips.
    const snapshot=dedupeRows(Array.isArray(pendingRows)?pendingRows:[]);
    setSync(`后台快速同步 ${snapshot.length} 笔资料...`);
    const result=await savePendingRowsBatchToSheetV300(snapshot);
    const savedRows=Array.isArray(result&&result.rows)?result.rows:[];

    savedRows.forEach(saved=>{
      const amount=Number(saved.amount||0);
      if(amount>0)upsertLocalRow(saved);
      else rows=rows.filter(x=>syncKey(x)!==syncKey(saved));
    });
    snapshot.forEach(clearPendingRow);

    renderAll();
    saveLocalDataCache();
    setSync("已同步", true);
  } catch (err) {
    console.warn("V30.1 batch pending sync delayed",err);
...
/* ================= V30.1 Profit / Change Log persistent cache ================= */
...
// V30.1: queued writes are already dispatched immediately after each save.
// Retry quietly once per minute instead of waking the network every 30 seconds.
setInterval(() => {
  loadPendingRows();
  if (pendingRows.length > 0 && navigator.onLine !== false) syncPendingRows();
}, 60000);
...
// V30.1 stable API alias: UI save function must never shadow the transport function.
async function deleteSalesTransactionV256(saleId){
  ...
  // V30.1: deleted cards must disappear from profit/editor caches immediately.
  salesProductLinksCacheV216.clear();
  allSalesProductLinksCacheV216={links:null,at:0};
  clearDailyProfitCacheV237();
  clearSalesCardPersistentCacheV232();
