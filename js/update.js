(() => {
  ...
  async function registerAndCheckForUpdates() {
    ...
    try {
      const registration = await navigator.serviceWorker.register("./sw.js?v=45.5", { updateViaCache: "none" });
  ...
  window.addEventListener("pageshow", event => {
    ...
    // Reload the document itself so the current V45.5 HTML is used, not only
