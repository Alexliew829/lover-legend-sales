(() => {
  ...
  async function refreshCloudData(reason = "resume", force = false) {
    ...
    if (running) {
      ...
      // V49.6: manual refresh is accepted even while an automatic probe is active.
    ...
    refreshPromise = loadFromSheet({
      // V49.6: even manual pull uses the V48.8 selective revision gate first.
  ...
  function startAutomaticRefreshAfterInitialSync() {
    ...
    // V49.6: visible devices run only the tiny V46.6/V48.3 revision gate every 10s.
  ...
  async function registerAndCheckForUpdates() {
    ...
    try {
      const registration = await navigator.serviceWorker.register("./sw.js?v=49.6", { updateViaCache: "none" });
  ...
  function refreshAfterReopen(reason = "resume") {
    ...
    // V49.6: keep only a very small reopen debounce. A 30s recent-sync guard
    ...
    resumePromise = refreshCloudData(reason, false).then(result => {
      ...
      // V49.6: do not schedule a second resume retry here. sheet.js already owns
  ...
  function setupPullToRefresh() {
    ...
    document.addEventListener("touchstart", event => {
      // V49.6: manual pull-to-refresh must remain available even while a background
