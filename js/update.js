(() => {
  ...
  async function registerAndCheckForUpdates() {
    ...
    try {
      const registration = await navigator.serviceWorker.register("./sw.js?v=43.8", { updateViaCache: "none" });
  ...
  window.addEventListener("pageshow", event => {
    ...
    // Reload the document itself so the current V43.8 HTML is used, not only
