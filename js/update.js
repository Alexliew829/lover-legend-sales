(() => {
  ...
  async function registerAndCheckForUpdates() {
    ...
    try {
      const registration = await navigator.serviceWorker.register("./sw.js?v=44.0", { updateViaCache: "none" });
  ...
  window.addEventListener("pageshow", event => {
    ...
    // Reload the document itself so the current V44.0 HTML is used, not only
