(() => {
  ...
  // V45.8: once the core sync state says 已同步, treat this refresh cycle as
  ...
  async function registerAndCheckForUpdates() {
    ...
    try {
      const registration = await navigator.serviceWorker.register("./sw.js?v=45.8", { updateViaCache: "none" });
  ...
  window.addEventListener("pageshow", event => {
    ...
    // Reload the document itself so the current V45.8 HTML is used, not only
  ...
  function setupPullToRefresh() {
    ...
    // V45.8: if any automatic/manual cloud path has already reached a
