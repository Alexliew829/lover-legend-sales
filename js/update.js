(() => {
  ...
  async function registerAndCheckForUpdates() {
    ...
    try {
      const registration = await navigator.serviceWorker.register("./sw.js?v=44.9", { updateViaCache: "none" });
  ...
  window.addEventListener("pageshow", event => {
    ...
    // Reload the document itself so the current V44.9 HTML is used, not only
