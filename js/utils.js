...
function isoToDisplay(s){if(!s)return"";const t=String(s).trim();if(/^\d{2}-\d{2}-\d{4}$/.test(t))return t;if(/^\d{4}-\d{2}-\d{2}$/.test(t)){const[y,m,d]=t.split("-");return `${d}-${m}-${y}`}return t}
function displayToISO(s){if(!s)return"";const t=String(s).trim();if(/^\d{4}-\d{2}-\d{2}$/.test(t))return t;if(/^\d{2}-\d{2}-\d{4}$/.test(t)){const[d,m,y]=t.split("-");return `${y}-${m}-${d}`}return""}
function isValidDisplayDate(s){if(!/^\d{2}-\d{2}-\d{4}$/.test(String(s||"")))return false;const iso=displayToISO(s);const[y,m,d]=iso.split("-").map(Number);const x=new Date(y,m-1,d);return x.getFullYear()===y&&x.getMonth()===m-1&&x.getDate()===d}
...
function dateRange(startISO,endISO){const a=[];const[sy,sm,sd]=startISO.split("-").map(Number);const[ey,em,ed]=endISO.split("-").map(Number);let c=new Date(sy,sm-1,sd),l=new Date(ey,em-1,ed);while(c<=l){a.push(isoToDisplay(`${c.getFullYear()}-${String(c.getMonth()+1).padStart(2,"0")}-${String(c.getDate()).padStart(2,"0")}`));c.setDate(c.getDate()+1)}return a}
function downloadFile(filename,content,type){const b=new Blob([content],{type}),u=URL.createObjectURL(b),l=document.createElement("a");l.href=u;l.download=filename;document.body.appendChild(l);l.click();document.body.removeChild(l);URL.revokeObjectURL(u)}
...
function monthAfter(m){const[y,mo]=m.split("-").map(Number),d=new Date(y,mo,1);return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`}
function yearAfter(y){return String(Number(y)+1)}
function syncKey(row){return [row.type,row.date,row.company,canonicalLocation(row.location||"")].join("|")}
function openDatePicker(id){const el=document.getElementById(id);if(el.showPicker)el.showPicker();else el.click()}
function setDateControl(id,iso){document.getElementById(id).value=iso;const d=document.getElementById(id+"Display");if(d)d.value=isoToDisplay(iso)}
function bindDateControl(id,onChange){const hidden=document.getElementById(id),display=document.getElementById(id+"Display");hidden.addEventListener("change",()=>{display.value=isoToDisplay(hidden.value);if(onChange)onChange()});display.addEventListener("change",()=>{if(!isValidDisplayDate(display.value)){alert("日期格式必须是 dd-mm-yyyy，例如 10-07-2026");display.value=isoToDisplay(hidden.value);return}hidden.value=displayToISO(display.value);if(onChange)onChange()})}
